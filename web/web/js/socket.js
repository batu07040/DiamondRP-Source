...
/******/ 	return __webpack_require__(__webpack_require__.s = "./src/web/src/addons/socket.io/index.ts");
/******/ })
/************************************************************************/
/******/ ({
...
/***/ "./src/web/src/addons/socket.io/index.ts":
/*!***********************************************!*\
  !*** ./src/web/src/addons/socket.io/index.ts ***!
  \***********************************************/
/*! no exports provided */
...
try {
    let socket;
    window.initSocketSystem = (url, id, key) => {
        log('Start initSocketSystem');
        socket = socket_io_client__WEBPACK_IMPORTED_MODULE_0___default()(url, {
            reconnection: true,
        ...
        socket.on('eventClient', (name, jsArgs) => mp.trigger('socketEvent', name, jsArgs));
        log('init connect');
        socket.on('connect', () => {
            log('socket:connected');
            mp.trigger('socket:connected');
        });
        window.callRemote = (name, jsArgs) => socket.emit("eventServer", name, jsArgs);
    };
