...
export let carWash = {
  ...
  loadAll: function() {
      ...
      carWash.list.forEach(function (item) {
          ...
          methods.createStaticCheckpoint(shopPos.x, shopPos.y, shopPos.z, "Kullanmak için ~g~E~s~ tuşlarına basın", 4, 0, [0, 0, 0, 0]);
      });
  },
      ...
      try {
          ...
          if (player.vehicle) {
              if (user.getCashMoney(player) < 100) {
                  player.notify('~r~Yıkamak için 100 $a sahip olmak gerekir ');
                  return;
              }
              ...
              setTimeout(function () {
                  ...
                  player.notify('~g~Taşıtınız artık temiz');
                  player.notify('~g~Servis ücreti: ~s~ $100');
