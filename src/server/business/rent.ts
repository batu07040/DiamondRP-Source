...
export let rent = {
  ...
  loadAll: function() {
    ...
    rent.listBike.forEach(function(item) {
      ...
      methods.createBlip(shopPos, 226, 33, 0.7, 'Bisiklet/motosiklet kiralama merkezi');
      methods.createStaticCheckpoint(
        shopPos.x,
        shopPos.y,
        shopPos.z,
        'Menüyü açmak için ~g~E~s~ tuşlarına basın',
        0.8,
        -1,
        [33, 150, 243, 100],
        0.3
  ...
  buy: function(
    ...
    if (find) {
      player.notify('~y~Zaten bir araç kiraladınız.');
      return;
    }
    ...
    switch (className) {
      ...
      case 'Boats':
        if (!user.get(player, 'ship_lic')) {
          player.notify('~r~У Su taşımacılığı ruhsatınız yok');
          return;
        }
        break;
      case 'Commercials':
      case 'Industrial':
        if (!user.get(player, 'c_lic')) {
          player.notify('~r~C  ehliyetiniz yok');
          return;
        }
        break;
      case 'Compacts':
      ...
      case 'Vans':
        if (!user.get(player, 'b_lic')) {
          player.notify('~r~B  ehliyetiniz yok');
          return;
        }
        break;
      case 'Motorcycles':
        if (hash != -1842748181 && !user.get(player, 'a_lic')) {
          player.notify('~r~A  ehliyetiniz yok. Faggio yu alabilirsin');
          return;
        }
        break;
    }
    ...
    player.notify('~g~Bir araç kiraladınız');
    player.notify('~g~Açmak için, tuşuna basın ~s~L');
