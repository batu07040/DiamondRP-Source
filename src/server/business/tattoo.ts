...
export let tattoo = {
    openMenu: (player: PlayerMp, shopId:number) => {
        if(user.hasAnyWeapon(player)) return player.notify(`~r~Silahlarınızı saklayın, usta gergin.`)
        let m = menu.new(player, "", "Kategoriler");
        switch (shopId) {
            case 94:
        ...
        let categorys: [string, string][] = [
            ["Baş", "ZONE_HEAD"],
            ["Göğüs", "ZONE_TORSO"],
            ["Sol El", "ZONE_LEFT_ARM"],
            ["Sağ El", "ZONE_RIGHT_ARM"],
            ["Sol Bacak", "ZONE_LEFT_LEG"],
            ["Sağ Bacak", "ZONE_RIGHT_LEG"],
        ]
        categorys.map(qs => {
            ...
            m.newItem({
                ...
                onpress: () => {
                    ...
                    const op = () => {
                        ...
                        let submenu = menu.new(player, "Dövme Salonu", "Dövmelerin Listesi");
                        submenu.onclose = () => { user.resetCustomization(player);}
                        submenu.newItem({
                            name: "Başlığa Göre Ara",
                            onpress: () => {
                                menu.input(player, "Arama İçin Bir İsim Girin").then(name => {
                                    if (!name) srch = ""
                            ...
                            list.map(item => {
                                if (user.hasTattoo(player, (item[1] as string), ((user.getSex(player) ? item[3] : item[2]) as string))){
                                    submenu.newItem({
                                        ...
                                        more: "~g~Doldurulmuş",
                                        desc: "Birleştirin: ~g~$" + ((item[5] as number) * 2),
                                        onpress: () => {
                                            if (!user.hasTattoo(player, (item[1] as string), ((user.getSex(player) ? item[3] : item[2]) as string))) return player.notify(`~r~Söz konusu dövme sende zaten var.`)
                                            if (user.getCashMoney(player) < ((item[5] as number) * 2)) return player.notify(`~r~Yeterli paranız yok!`)
                                            user.removeCashMoney(player, ((item[5] as number) * 2));
                                            player.notify(`~g~Dövme küçültüldü.`)
                                            user.log(player, "Tattoo", `Dövme ${mp.joaat(item[1] as string)} ${mp.joaat((user.getSex(player) ? item[3] : item[2]) as string)} $${((item[5] as number) * 2)} ${shopId}`)
                                    ...
                                    submenu.newItem({
                                        name: item[0] as string,
                                        type: "list",
                                        list: ["Ön İzleme", "Eşyalar: ~g~$" + item[5]],
                                        onpress: (it) => {
                                            if (user.hasTattoo(player, (item[1] as string), ((user.getSex(player) ? item[3] : item[2]) as string))) return player.notify(`~r~Bu dövmeyi çoktan yaptırmışsın.`)
                                            if(it.listSelected == 0){
                                                ...
                                                if(user.getCashMoney(player) < item[5]) return player.notify(`~r~Ödeme yapmak için yeterli paranız yok`);
                                                if (player.tattoosList.length > 50) return player.notify(`~r~O kadar çok dövmen var ki, daha fazlası için yer yok.`);
                                                user.removeCashMoney(player, item[5] as number);
                                                player.notify(`~g~Dövme`)
                                                user.log(player, "Tattoo", `Dövme Yapıldı ${mp.joaat(item[1] as string)} ${mp.joaat((user.getSex(player) ? item[3] : item[2]) as string)} $${item[5]} ${shopId}`)
    ...
    buy: function(player:PlayerMp, collection:string, overlay:string, zone:string, price:number, shopId:number) {
        ...
        switch (zone)
        {
            case "ZONE_HEAD":
                if (user.get(player, 'tattoo_head_c') != '') {
                    player.notify("~r~Dövme önce silinmelidir");
        ...
        user.log(player, "Tattoo", `Dövme yaptırdım. ${collection} ${overlay} ${zone} ${price} ${shopId}`)
    
        user.removeMoney(player, price);
        business.addMoney(shopId, price);
        player.notify('~g~Dövmen var.');
        user.updateTattoo(player);
    },
    ...
    buyPrint: function(player:PlayerMp, collection:string, overlay:string, price:number) {
        ...
        if (user.get(player, 'tprint_c') != '') {
            player.notify("~r~Bu giysinin üzerinde zaten bir baskı var");
            user.updateTattoo(player);
            return;
        }
        ...
        user.log(player, "Print", `Bir baskı aldım ${collection} ${overlay} ${price}`)
        player.notify('~g~Bir baskı satın aldınız');
        user.updateTattoo(player);
    },
    ...
    clear: function(player:PlayerMp, zone:string, price:number, shopId:number) {
        ...
        user.removeMoney(player, price);
        business.addMoney(shopId, price);
        player.notify('~g~Dövme yaptırmışsın.');
        user.updateTattoo(player);
    }
};