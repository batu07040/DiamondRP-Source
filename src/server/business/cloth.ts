...
export let cloth = {
  ...
  loadAll: function(){
      ...
      methods.createBlip(cloth.maskShop, 362, 0, 0.8, 'Maske alışverişi yapın');
      methods.createStaticCheckpoint(cloth.maskShop.x, cloth.maskShop.y, cloth.maskShop.z, "Mağaza menüsünü açmak için ~g~E~s~ tuşlarına basın", 0.8, -1, [33, 150, 243, 100], 0.3);
          ...
          for (let i = 0; i < shopList.length; i++) {
            ...
            let type = shopList[i][2];
              ...
              if (type == 0) {
                  switch (shopType) {
                      case 0:
                          methods.createBlip(pos, 73, 68, 0.8, 'Kıyafet alışverişi "Discount Store"');
                          break;
                      case 1:
                          methods.createBlip(pos, 73, 0, 0.8, 'Kıyafet alışverişi "Suburban"');
                          break;
                      case 2:
                          methods.createBlip(pos, 73, 21, 0.8, 'Kıyafet alışverişi "Ponsonbys"');
                          break;
                      case 3:
                          methods.createBlip(pos, 73, 73, 0.8, 'Kıyafet alışverişi "AmmoNation"');
                          break;
                      case 4:
                          methods.createBlip(pos, 617, 0, 0.8, 'Kuyumcu dükkanı');
                          break;
                      case 5:
                          methods.createBlip(pos, 73, 81, 0.8, 'Kıyafet alışverişi "Binco"');
                          break;
                  }
              }
  
              methods.createStaticCheckpoint(pos.x, pos.y, pos.z, "Mağaza menüsünü açmak için ~g~E~s~ tuşlarına basın", 0.8, -1, [33, 150, 243, 100], 0.3);
          }
      } catch (e) {
          console.log(e);
          throw e;
  ...
  buyProp: function (player:PlayerMp, price:number, body:number, clothId:number, color:number, shopId:number, isFree:boolean) {
      ...
      if (!isFree) {
          ...
          player.notify("~g~Bir aksesuar satın aldınız");
          user.log(player, "PlayerBuy", `Aksesuar satın almak için ${price}`)
      }
  ...
  buy: function (player:PlayerMp, price:number, body:number, cloth:number, color:number, torso:number, torsoColor:number, parachute:number, parachuteColor:number, shopId = 0, isFree = false) {
      ...
      if (!isFree) {
          user.removeCashMoney(player, price);
          business.addMoney(shopId, price);
          player.notify("~g~Kıyafet aldınız");
          user.completeQuest(player, "Modaya uygun olun", true)
          user.log(player, "PlayerBuy", `Kıyafet satın almak ${price}`)
      }
  ...
  buyMask: function (player:PlayerMp, price:number, clothId:number, color:number, shopId:number) {
      ...
      user.removeCashMoney(player, price);
      business.addMoney(shopId, price);
      player.notify("~g~Bir maske aldın.");
  }
};
