...
export let lsc = {
    ...
    loadAll: function () {
        ...
        lsc.list.forEach(function (item) {
            ...
            methods.createBlip(shopPos, 446, 0, 0.8, 'Araba atölyesi');
        ...
        lsc.carColorPos.forEach(function (item) {
            let shopPos = new mp.Vector3(item[0], item[1], item[2]);
            methods.createStaticCheckpoint(shopPos.x, shopPos.y, shopPos.z, "~b~Aracın yeniden boyanacağı yer", 4, -1, [33, 150, 243, 100], 0.3);
        });

        lsc.carPos.forEach(function (item) {
            let shopPos = new mp.Vector3(item[0], item[1], item[2]);
            methods.createStaticCheckpoint(shopPos.x, shopPos.y, shopPos.z, "~b~Araç ayarının yapıldığı yer", 4, -1, [33, 150, 243, 100], 0.3);
        });
    },
    ...
    buy: function (player: PlayerMp, number: string, price: number, shopId: number, action: string, newNumber: string) {
        ...
        switch (action) {
            case 'setNeon': {
                ...
                player.notify('~g~Neon takma islemi basarılı.');
            ...
            case 'repair': {
                ...
                player.notify("~g~Aracı tamir ettiniz.");
                break;
            }
            case 'setNumber': {
                if (veh.getVariable('id_user') < 1) {
                ...
                if (newNumber.length < 1) {
                    player.notify('~r~Plaka en az bir karakter içermeli.');
                    return;
                }

                if (!lsc.checkValidNumber(newNumber)) {
                    player.notify('~r~Geçersiz plaka numarası girdiniz.');
                    player.notify('~r~Sadece rakamlar (0-9) ve İngilizce harfler. (A-Z)');
                    return;
                }

                if (newNumber.length == 1 && user.getMoney(player) < 5000000) {
                    player.notify('~r~1 Karakterli bir plakanın bedeli $5.000.000');
                    return;
                }

                if (newNumber.length == 2 && user.getMoney(player) < 2500000) {
                    player.notify('~r~2 Karakterli bir plakanın bedeli 2.500.000 $');
                    return;
                }

                if (newNumber.length == 3 && user.getMoney(player) < 2000000) {
                    player.notify('~r~3 Karakterli bir plakanın bedeli $2.000.000');
                    return;
                }

                if (newNumber.length == 4 && user.getMoney(player) < 1000000) {
                    player.notify('~r~4 Karakterli bir plakanın bedeli $1.000.000');
                    return;
                }
                carsEntity.findAll({
                    where: {
                    ...
                    if (rows.length === 0) {
                        ...
                        if (valid) {
                            ...
                            if (newNumber.length == 1) {
                                user.removeMoney(player, 5000000);
                                business.addMoney(shopId, 50000);
                            }
                            else if (newNumber.length == 2) {
                                user.removeMoney(player, 2500000);
                                business.addMoney(shopId, 25000);
                            }
                            else if (newNumber.length == 3) {
                                user.removeMoney(player, 2000000);
                            ...
                            player.notify('~g~Plakayı değiştirdiniz.');
                            return;
                        }
                    }
                    player.notify('~r~Plaka zaten satın alınmış.');
                });
                break;
            }
        }
    ...
    buyTun: function (player: PlayerMp, number: string, modType: number, idx: number, price: number, shopId: number) {
        ...
        if (veh.getVariable('id_user') != user.getId(player)) {
            player.notify('~r~Araç size ait degil.');
            return;
        }
        ...
        player.notify('~g~Parçayı taktınız. Fiyat: ~s~$' + methods.numberFormat(price));
    ...
    buySTun: function (player: PlayerMp, number: string, modType: number, idx: number, price: number, shopId: number) {
        ...
        player.notify('~g~Fiyatı güncellediniz: ~s~$' + methods.numberFormat(price));
        vehicles.save(veh.getVariable('container'));
    },
    ...
    resetSTun: function (player: PlayerMp, number: string, modType: number) {
        ...
        player.notify('~g~Varsayılan ayarlara güncellediniz.');
        vehicles.save(veh.getVariable('container'));
    },
    ...
    buyColor1: function (player: PlayerMp, number: string, idx: number, price: number, shopId: number) {
        ...
        player.notify('~g~Aracın rengi değiştirildi.');
