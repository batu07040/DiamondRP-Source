...
export let bank = {
  ...
  loadAll: function() {
      ...
      methods.createBlip(bank.pacificPos, 374, 65, 0.8, 'Özel banka "Pacific Standard"');
      methods.createBlip(bank.blainePos, 374, 67, 0.8, 'Özel banka "Blaine County"');
  
  
      bank.fleecaMarkers.forEach(function (item) {
          let bankPos = new mp.Vector3(item[0], item[1], item[2]);
          methods.createBlip(bankPos, 374, 69, 0.8, 'Özel banka "Fleeca"');
      });
  
      bank.markers.forEach(function (item) {
          let bankPos = new mp.Vector3(item[0], item[1], item[2] - 1);
          methods.createStaticCheckpointV(bankPos, "Menüyü açmak için ~g~E~s~ tuşlarına basın");
      });
  ...
  sendSmsBankOperation: function(player:PlayerMp, text:string, title = 'Bir hesapla yapılan işlem') {
  ...
  transferMoney: function(player:PlayerMp, bankPrefix:number, bankNumber:number, money:number) {
      ...
      if (money < 1) {
          player.notify('~r~Miktar sıfırdan büyük olmalıdır');
      ...
      if((sendMoney.get(userid) + money) > sendmoneymax) return player.notify('~r~Günlük para transferi limiti: '+sendmoneymax+'$');

      if (bankPrefix < 1) {
          player.notify('~r~Kart öneki sıfırdan büyük olmalıdır');
          user.updateClientCache(player);
          return;
      }
      if (bankNumber < 1) {
          player.notify('~r~Kart numarası sıfırdan büyük olmalıdır');
          user.updateClientCache(player);
          return;
      }
      ...
      if (pl) {
        methods.saveLog('GiveBank', `${user.getRpName(player)} (${user.getId(player)}) [${user.get(player, 'bank_prefix')}-${user.get(player, 'bank_number')}] to ${bankPrefix}-${bankNumber} count $${money} (${user.getId(pl)} online)`);
          ...
          bank.sendSmsBankOperation(player, 'Çeviri: ~g~$' + methods.numberFormat(sumFinal));
          bank.sendSmsBankOperation(pl, 'Kayıt: ~g~$' + methods.numberFormat(sumFinal));
          user.removeBankMoney(player, money);
          sendMoney.set(userid, sendMoney.get(userid) + money)
          user.addBankMoney(pl, sumFinal);
          user.loginterract(player, pl, "BankTransferMoney", 'Transfer Edilen '+methods.numberFormat(sumFinal))
      }
        ...
        userEntity.findAll({ where: {
            ...
            bank_prefix: bankPrefix,
              ...
              if(rows.length == 1){
                ...
                user.loginterract(player, item.id, "BankTransferMoney", 'Transfer Edilen '+methods.numberFormat(sumFinal))
              }
              ...
              if (!isEquip) {
                  ...
                  if(rows2.length == 1){
                      ...
                      user.log(player, "BankTransferMoney", `Karta transfer edildi @item${item2.id} ${bankPrefix}-${bankNumber} $${methods.numberFormat(sumFinal)}. Kart takılı değil. ТИП: ${item2.owner_type} ID: ${item2.owner_id}`);
                      isEquip = true;
                  };
  
                  if (!isOnline && !isEquip)
                      bank.sendSmsBankOperation(player, 'Herhangi bir hesap bulunamadı', '~r~Çeviri hatası');
            }
  ...
  changeCardNumber: function(player:PlayerMp, bankNumber:number) {
      ...
      if (bankNumber < 9999) {
          player.notify('~r~Kart numarası 4 haneden büyük olmalıdır');
          return;
      }
      ...
      userEntity.findAll({
          ...
          if (rows.length === 0) {
              ...
              let rows = inventory.allItems().filter((item) => item.number == bankNumber && item.prefix == bankPrefix)
                if (rows.length === 0) {
                    methods.saveLog("changeCardNumber", `${user.getRpName(player)} (${user.getId(player)}) Kart numarası değiştirildi ${user.get(player, 'bank_number')} на ${bankNumber}`)
                    ...
                    bank.sendSmsBankOperation(player, 'Kart numaranız değiştirildi');
                    user.saveAccount(player);
                }
                else
                    bank.sendSmsBankOperation(player, 'Kart numarası zaten mevcut', '~r~Hata');
  ...
  withdraw: function(player:PlayerMp, money:number, procent = 0) {
      ...
      //setTimeout(function () {
          ...
          if (money < 1) {
              player.notify('~r~Toplamlar sıfırdan büyük olmalıdır');
              return;
          }
          ...
          if (procent == 0) {
              bank.sendSmsBankOperation(player, 'Sonuç: ~g~$' + methods.numberFormat(money));
              ...
              bank.sendSmsBankOperation(player, 'Sonuç: ~g~$' + methods.numberFormat(sum));
  ...
  deposit: function(player:PlayerMp, money:number, procent = 0) {
      ...
      if (procent == 0) {
          bank.sendSmsBankOperation(player, 'Kaydolma: ~g~$' + methods.numberFormat(money));
          ...
          bank.sendSmsBankOperation(player, 'Kaydolma: ~g~$' + methods.numberFormat(sum));
          bank.addBusinessBankMoneyByPrefix(user.get(player, 'bank_prefix'), sumBank);
  ...
  openCard: function(player:PlayerMp, bankId:number, price:number) {
      ...
      if (user.get(player, 'bank_prefix') > 0) {
          player.notify('~r~Zaten bir banka kartınız var');
          return;
      }
  ...
  sendSmsBankCloseOperation: function(player:PlayerMp) {
      bank.sendSmsBankOperation(player, 'Banka hesabınız kapatıldı.');
  },
  
  sendSmsBankOpenOperation: function(player:PlayerMp) {
      bank.sendSmsBankOperation(player, 'Hesap açtığınız için tebrikler!');
  },
