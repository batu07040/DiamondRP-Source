...
export let fuel = {
    ...
    loadAll: function() {
        ...
        fuel.stations.forEach(function (item, idx) {
            ...
            methods.createBlip(fuelStationPos, 415, 0, 0.8, 'Yakıt doldurma');
            methods.createStaticCheckpoint(fuelStationShopPos.x, fuelStationShopPos.y, fuelStationShopPos.z, "Menüyü açmak için ~g~E~s~ tuşlarına basın");
            if (item[3] == 1)
                methods.createBlip(fuelStationShopPos, 52, 0, 0.8, '7/24 Alışveriş');
        });
    },
