...
export let autosalon = {
  ...
  list: <[string, number, number, number, number, number, number][]>[
    ['Araç Galerisi (Ekonomik)', -44.23451232910156, -1660.8045654296875, 29.283042907714844, 225, 173, 5],
    ['Araç Galerisi (Lüks)', -32.23559951782226, -1111.933471679687, 26.4223518371582, 225, 174, 5],
    ['Araç Galerisi (Elit)', -69.3691177, 62.7385559, 71.8934402, 225, 175, 5],
    ['Motor Galerisi', 268.23211669921875, -1155.246826171875, 29.290678024291992, 226, 176, 5],
    ['Tekne Galerisi', -786.2677001953125, -1355.430419921875, 5.150264739990234, 427, 177, 5],
    ['Hava Aracı Galerisi', -1071.0123291015625, -2868.7138671875, 13.951811790466309, 307, 178, 5],
    ['Araç Galerisi (Nakliye)', 796.626953125, -2988.70166015625, 6.020936012268066, 318, 179, 5],
  ],
  ...
  loadAll: async () => {
    ...
    autosalon.list.map(async (item, idx) => {
      ...
      methods.createStaticCheckpoint(
        ...
        shopPos.y,
        shopPos.z - 1,
        'Galeriye girmek için ~g~Е~s~ basın'
      );
    });
    autosalon.loadCars();
  ...
  checkPosForOpenMenu: (player: PlayerMp) => {
    ...
    const m = menu.new(player, business.getName(asIdRemote), 'Eylemler', [
      {
        name: 'Satın Almak',
        onpress: () => {
          if(!enabledSystem.buycar) return player.notify("Salon geçici olarak bakıma alınmıştır. Daha sonra tekrar kontrol edin.")
    ...
    if (asIdRemote != 179) {
      m.newItem({
        name: 'Araç Kiralama',
        onpress: () => {
          if(!enabledSystem.rent) return player.notify("Salon geçici olarak bakıma alınmıştır. Daha sonra tekrar kontrol edin.")
  ...
  buyCar: async (player: PlayerMp, slot: number, car: CarInstance, color: number) => {
    ...
    try {
      ...
      if (!rows.length) {
        user.bigAlert(player, 'Mevcut değil.', 'warning');
        return false;
      }
      car = rows[0];
      if (car.user_name || car.id_user) {
        user.bigAlert(player, 'Çoktan satın alındı.', 'warning');
        return false;
      }
      ...
      user.bigAlert(player, `Araç alımınız için tebrikler. ${user.getLevel(player) < 4 ? 'Aracınızı sizin için uygun olan yere yeniden park etmeyi unutmayın.' : ''}`, 'success', 10000);
      user.saveAccount(player);
      user.log(player, "PlayerBuy", `Satın Alındı.${car.name} @veh${car.id} için $${car.price}`)
  ...
  rentCar: async (player: PlayerMp, model: string, price: number, color: number) => {
    ...
    if (find) {
      player.notify('~y~Zaten kiraladınız, Kira sözleşmenizi bitirin! ~b~(/unrent)');
      return;
    }
    ...
    switch (className) {
      ...
      case 'Helicopters':
        if (!user.get(player, 'air_lic')) {
          user.bigAlert(player, 'Pilot lisansınız yok!', 'warning');
          return;
        }
        break;
      case 'Boats':
        if (!user.get(player, 'ship_lic')) {
          user.bigAlert(player, 'Su taşımacılığı ruhsatınız yok!', 'warning');
          return;
        }
        break;
      case 'Commercials':
      case 'Industrial':
        if (!user.get(player, 'c_lic')) {
          user.bigAlert(player, 'C kategorisi ehliyetiniz yok!', 'warning');
          return;
        }
        break;
      case 'Compacts':
      ...
      case 'Vans':
        if (!user.get(player, 'b_lic')) {
          user.bigAlert(player, 'B kategorisi ehliyetiniz yok!', 'warning');
          return;
        }
        break;
      case 'Motorcycles':
        if (hash != -1842748181 && !user.get(player, 'a_lic')) {
          user.bigAlert(player, 'A kategorisi ehliyetiniz yok. Faggio yu alabilirsiniz', 'warning');
          return;
        }
        break;
    }
    ...
    user.log(player, "PlayerRent", `Kira ile alındı. ${model} за $${price}`)
    const asId = autosalon.list[player.currentAutosalonId][5];
    ...
    user.bigAlert(player, `Araç kiraladınız, birkaç saniye bekleyin`, 'success');
    player.notify('~b~Gir ~y~/unrent ~s~Kira sözleşmesini reddedin.');
    let parkpos = getParkPosition(new mp.Vector3(pos[0], pos[1], pos[2]));
    if(parkpos && parkpos.pos){
  ...
  changeRentPriceMenu: (player: PlayerMp, remoteId: number) => {
    ...
    const m = menu.new(player, 'Kiralık', 'Seçiniz');
    cars.forEach((car) => {
      m.newItem({
  ...
  changeRentPrice: async (player: PlayerMp, id: number) => {
    ...
    if (!mp.players.exists(player)) return;
    let input = await menu.input(player, 'Yeni Fiyat', '', 5, 'int');
    ...
    if (price < 10 || price > 50000) {
      autosalon.changeRentPrice(player, id);
      return player.notify('~r~Tutarı girin ($10-$50.000)');
    }
    ...
    player.notify('~g~Kira bedelini güncellediniz');
    return;
  },
...
setInterval(() => {
  ...
  buyWaiting.forEach((item, playerId) => {
    ...
    buy(player, item.id, item.colorId).then((status) => {
      ...
      if (!status) {
        user.alert(player, 'Tekrar deneyin', 'error');
      }
...
mp.events.add('server:autosalon:buyCar', (player: PlayerMp, id: number, colorId: number) => {
  ...
  player.notify('Depoda arıyorum, birkaç saniye bekleyin.');
});
...
function buy(player: PlayerMp, id: number, colorId: number) {
  ...
  return new Promise(async (resolve) => {
    ...
    if (car.id_user || car.user_name) return user.alert(player, 'Satın alındı!', 'warning'), resolve(false);
    if (user.get(player, 'money') < car.price) {
      user.bigAlert(player, 'Para yetersiz.', 'warning');
    ...
    if (!user.get(player, 'car_id1')) {
      ...
      if (
        ...
        return user.bigAlert(
          player,
          'İkinci bir araç satın almak için bir eviniz/dükkanınız/daireniz olmalıdır',
          'warning'
        ), resolve(false);
      }
      ...
      if (user.get(player, 'id_house') == 0) {
        return user.bigAlert(player, 'Üçüncü bir araç satın almak için bir eviniz olmalı.', 'warning'), resolve(false);
      }
      if (houses.get(user.get(player, 'id_house'), 'price') < 1000000) {
        return user.bigAlert(
          player,
          'Üçüncü bir araç satın almak için evinizin değeri en az 1.000.000 $ olmalıdır',
          'warning'
        ), resolve(false);
      }
      ...
      if (user.get(player, 'id_house') == 0) {
        return user.bigAlert(player, 'Dördüncü bir araç satın almak için bir eviniz olmalı', 'warning'), resolve(false);
      }
      if (houses.get(user.get(player, 'id_house'), 'price') < 2500000) {
        return user.bigAlert(
          player,
          'Dördüncü bir araç satın almak için evinizin değeri en az 2.500.000 $ olmalıdır',
          'warning'
        ), resolve(false);
      }
      ...
      if (user.get(player, 'id_house') == 0) {
        return user.bigAlert(player, 'Beşinci bir araç satın almak için bir eviniz olmalı', 'warning'), resolve(false);
      }
      if (houses.get(user.get(player, 'id_house'), 'price') < 5000000) {
        return user.bigAlert(
          player,
          '5inci bir araç satın almak için evinizin en az 5.000.000 $ değerinde olması gerekir',
          'warning'
        ), resolve(false);
      }
      ...
      return user.bigAlert(player, '6, 7, 8 taşıma ile satın alınabilir Appi Coins', 'info'), resolve(false);
    }
  })
}
...
mp.events.add(
  ...
  (player: PlayerMp, id: number, colorId: number) => {
    ...
    if (!car) return;
    if (user.get(player, 'money') < car.price) {
      return user.bigAlert(player, 'Para yetersiz.', 'warning');
    }
    const color = autosalon.colors[colorId][0];
    // todo
...
mp.events.add('playerStartEnterVehicle', (player: PlayerMp, vehicle: VehicleMp, seat: number) => {
  ...
  switch (className) {
    ...
    case 'Helicopters':
      if (!user.get(player, 'air_lic')) {
        player.notify('~r~Pilot lisansınız yok');
    ...
    case 'Boats':
      if (!user.get(player, 'ship_lic')) {
        player.notify('~r~Su taşımacılığı ruhsatınız yok');
        player.stopAnimation();
        return;
      }
      break;
...
setTimeout(() => {
  ...
  chat.registerCommand('unrent', (player: PlayerMp) => {
    ...
    if (!vehicle) return player.notify('~b~Araç kiralayamazsın');
    if (!mp.vehicles.exists(vehicle)) return;
    vehicle.destroy();
    player.notify('~g~Araç kiralamayı reddettiniz');
  });
