...
export let shop = {
    lotoMenu: (player: PlayerMp, shopId: number, addedNalog:number = 0) => {
        ...
        lotoList.map(loto => {
            m.newItem({
                name: `Çiziliyor ~g~$${methods.numberFormat(loto.prize)}`,
                more: `$${methods.numberFormat(loto.cost)}`,
                desc: `Bilet kalmadı: ${(loto.count - itemsLoto.filter(item => item.item_id == itemLoto && item.prefix == loto.prize && item.key_id == 0).length)}`,
                onpress: () => {
                    m.close();
                    ...
                    if (user.getCashMoney(player) < loto.cost) return player.notify("~r~Bilet almak için yeterli paranız yok")
                    if ((loto.count - itemsLoto.filter(item => item.item_id == itemLoto && item.prefix == loto.prize && item.key_id == 0).length) <= 0) return player.notify("~r~Biletlerin süresi doldu, çekilişi bekleyin")
                    
                    
                    if (inventory.currentAmount(1, user.getId(player)) + items.getItemWeightById(itemLoto) > inventory.maxAmount(1, user.getId(player))) {
                        player.notify('~r~Üzerinde yer yok');
                        return;
                    }
                    ...
                    * */
                    ...
                    player.notify('~g~Piyango bileti aldınız: ~s~$' + price);
        ...
        m.newItem({
            name: "~y~Kazançlar için tüm biletleri değiştirin.",
            onpress: () => {
                m.close();
                if (player.spamProtect) return;
                player.spamProtect = true;
                ...
                if (rows.length > 0) {
                    rows.forEach((item) => {
                        ...
                        methods.notifyWithPictureToPlayer(
                            ...
                            'Tebrikler!',
                            `#${item['number']}`,
                            `~g~Biletinizi değiştirdiniz: ~s~$${methods.numberFormat(item['prefix'])}`,
                            'WEB_PILLPHARM'
                        );
                    });
                } else {
                    player.notify('~r~У Biletiniz yok.');
                }
            }
        })
        m.newItem({
            name: "~y~Tüm biletleri teslim edin",
            onpress: () => {
                m.close();
                if (player.spamProtect) return;
                player.spamProtect = true;
                ...
                methods.notifyWithPictureToPlayer(
                    ...
                    'American Loto',
                    `Teşekkür ederim!`,
                    `~g~Süresi geçen biletleri teslim edin`,
                    'WEB_PILLPHARM'
                );
            }
        })
    ...
    loadAll: function() {
        ...
        shop.list.forEach(function (item) {
            ...
            methods.createBlip(shopPos, 52, 0, 0.8, 'Alışveriş 24/7');
            methods.createStaticCheckpoint(shopPos.x, shopPos.y, shopPos.z, "Menüyü açmak için ~g~E~s~ tuşlarına basın");
        });
    
        methods.createBlip(pickups.PrintShopPos, 72, 0, 0.8, 'Kıyafet baskıları için alışveriş yapın');
    
        methods.createBlip(shop.digitalDen, 521, 0, 0.8, 'Digital Den elektronik mağazası');
        methods.createStaticCheckpoint(shop.digitalDen.x, shop.digitalDen.y, shop.digitalDen.z, "Menüyü açmak için ~g~E~s~ tuşlarına basın");
    
        methods.createBlip(shop.digitalDenMirror, 521, 0, 0.8, 'Digital Den elektronik mağazası');
        methods.createStaticCheckpoint(shop.digitalDenMirror.x, shop.digitalDenMirror.y, shop.digitalDenMirror.z, "Menüyü açmak için ~g~E~s~ tuşlarına basın");
    
        methods.createBlip(shop.apteka, 153, 69, 0.8, 'Eczane');
        methods.createStaticCheckpoint(shop.apteka.x, shop.apteka.y, shop.apteka.z, "Menüyü açmak için ~g~E~s~ tuşlarına basın");
    
        methods.createBlip(shop.apteka2, 153, 69, 0.8, 'Eczane');
        methods.createStaticCheckpoint(shop.apteka2.x, shop.apteka2.y, shop.apteka2.z, "Menüyü açmak için ~g~E~s~ tuşlarına basın");
    
        methods.createBlip(shop.apteka3, 153, 69, 0.8, 'Eczane');
        methods.createStaticCheckpoint(shop.apteka3.x, shop.apteka3.y, shop.apteka3.z, "Menüyü açmak için ~g~E~s~ tuşlarına basın");
    },
    ...
    buy: function(player:PlayerMp, itemId:number, price:number, shopId:number, addedNalog = 0) {
        ...
        if ([inventory.types.BagTransfer, inventory.types.Bag, inventory.types.BagFull, inventory.types.BagSmall].includes(itemId)){
            if (user.hasBag(player)) return player.notify(`~r~Üzerinizde zaten bir çanta var`);
            else {
                setTimeout(() => {
        ...
        switch (itemId) {
            case 221:
                if(!user.get(player, "allow_antipohmel")) return player.notify("~r~Antipne için reçeteniz yok")
            ...
            case 8:
                if (user.get(player, 'phone_code') > 0 && user.get(player, 'phone') > 0) {
                    player.notify('~r~İlk olarak, cebinizden kıyafetinden çıkarın');
                    return;
                }
                ...
                player.notify('~g~Telefonu şu kadar ücret karşılığında aldınız: ~s~$' + price);
            ...
            case 2820000:
                if (user.get(player, 'tablet_equip') > 0) {
                    player.notify('~r~İlk olarak, tableti cebinizden çıkarın');
                    return;
                }
    
                user.set(player, 'tablet_equip', 1);
    
                player.notify('~g~Bu fiyata bir tablet aldın.: ~s~$' + price);
            ...
            case 7:
                if (user.get(player, 'item_clock')) {
                    player.notify('~r~İlk olarak saati cebinizden çıkarın');
                    return;
                }
    
                player.notify('~g~Saati şu fiyata aldınız: ~s~$' + price);
            ...
            case 47:
                if (user.get(player, 'is_buy_walkietalkie')) {
                    player.notify('~r~Zaten bir telsiziniz var');
                    return;
                }
    
                player.notify('~g~Şu fiyata bir telsiz satın aldınız: ~s~$' + price);
            ...
            case 277:
            {
                ...
                if (amount + items.getItemAmountById(itemId) > 55000) {
                    player.notify('~r~Üzerinizde yer yok');
                    return;
                }
                ...
                * */
                ...
                player.notify('~g~Şu fiyata bir piyango bileti aldınız: ~s~$' + price);
            ...
            default:
                ...
                player.notify('~g~Bu ürünü ücret karşılığında aldınız: ~s~$' + price);

                user.log(player, "PlayerBuy", `Bir mağazadan satın almak için $${price} ${items.getItemNameById(itemId)}`)
                user.removeMoney(player, price);
                if(addedNalog) business.addMoney(shopId, price - (price / 100 * addedNalog));
                else business.addMoney(shopId, price);
