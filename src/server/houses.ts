...
export let houses = {
    ...
    sell: function (player: PlayerMp) {
        ...
        if (hInfo.get('id_user') != user.get(player, 'id')) {
            player.notify('~r~Bu ev senin değil.');
            return;
        }
        ...
        user.log(player, "PlayerSell", `Evi sattım ${hInfo.get('address')} @house${hInfo.get('id')} за $${nalog}`)
        setTimeout(function () {
            if (!user.isLogin(player))
                return;
            user.addHistory(player, 3, 'Evi sattım ' + hInfo.get('address') + ' №' + hInfo.get('id') + '. Цена: $' + methods.numberFormat(nalog));
            player.notify('~g~Bir mülk sattınız');
            player.notify(`~g~Vergi:~s~ ${coffer.get('cofferNalog')}%\n~g~Erişim adresi:~s~ $${methods.numberFormat(nalog)}`);
    ...
    buy: function (player: PlayerMp, id: number) {
        ...
        if (user.get(player, 'id_house') > 0) {
            player.notify('~r~Zaten bir eve sahipsin.');
            return false;
        }
        ...
        user.log(player, "PlayerBuy", `Bir ev satın aldın ${hInfo.get('address')} @house${hInfo.get('id')} за $${hInfo.get('price')}`)
        setTimeout(function () {
            if (!user.isLogin(player))
                return;
            user.addHistory(player, 3, 'Bir ev satın aldın ' + hInfo.get('address') + ' №' + hInfo.get('id') + '. Fiyat: $' + methods.numberFormat(hInfo.get('price')));
            user.saveAccount(player);
