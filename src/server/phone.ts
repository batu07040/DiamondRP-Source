...
export let phone = {
    ...
    sendSms: function (player: PlayerMp, number: string, text: string) {
        ...
        try {
            ...
            player.notify("~g~Başarılı~s~\nAlıcıya bir kısa mesaj gönderildi.");
        } catch (e) {
            methods.debug(e);
        }
    },
    ...
    addContact: function (player: PlayerMp, phone: string, title: string, num: string) {
        ...
        if (!user.isLogin(player))
            return;
        try {
            if (title.length >= 50) return player.notify(`~r~Kisi adının uzunlugu maks. 50`)
            phoneContactEntity.create({
                number: phone,
                title: title,
                text_number: num
            })
            player.notify("~g~Kişi rehbere eklendi.");
        } catch (e) {
            methods.debug(e);
        }
    },
    ...
    renameContact: function (player: PlayerMp, id: number, newName: string) {
        ...
        if (!user.isLogin(player))
            return;
        try {
            player.notify("~g~Kişinin adı değiştirildi.");
        } catch (e) {
            methods.debug(e);
        }
    },
