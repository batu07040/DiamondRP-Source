...
setTimeout(() => {
  mp.events.register('autosale:buyVeh', (player: PlayerMp, cost: number) => {
    if (!player.vehicle) return player.notify("~r~Nakliye isinde olmalısınız");
    if (user.isDriver(player)) return player.notify("~r~Araba kullanmamalısın");
    const point = vehSalesPointsData.find(q => q && q[0] == player.vehicle);
    if (!point) return player.notify("~r~Satılık ");
    const pointIndex = vehSalesPointsData.findIndex(q => q && q[0] == player.vehicle);
    if (!mp.players.exists(point[1])) return player.notify("~r~Satıcı cevrimici degil");
    if (point[1].vehicle !== player.vehicle) return player.notify("~r~Satıcı su özelliklere sahip olmalıdır в ТС");
    if (vehicles.getVehicleOwnerId(player.vehicle) !== user.getId(point[1])) return player.notify("~r~Arac artık satıcıya ait degildir");
    if (point[3] !== cost) return player.notify(`~r~Satıs fiyatı degisti`);
  ...
  vehicleSalePoints.map((item, index) => {
    ...
    const canControl = (player: PlayerMp) => {
      if (!player.vehicle) {
        player.notify("~r~Nakliye isinde olmalısınız");
        return false;
      }
      if (!vehSalesPointsData[index]) {
        player.notify("~r~Koltuk bos");
        return false;
      }
      if (checkFree()) {
        player.notify("~r~Koltuk bos");
      ...
      if (vehSalesPointsData[index][1].id !== player.id) {
        player.notify("~r~Bu koltuk baska bir kisi tarafından isgal edilmistir");
        return false;
      }
      if (vehSalesPointsData[index][0].id !== player.vehicle.id) {
        player.notify("~r~Bu koltuk baska bir arac tarafından isgal edilmistir");
        return false;
      }
      if (!user.isDriver(player)) {
        player.notify("~r~Direksiyonun basında olmalısın.");
        return false;
      }
      return true;
    }
    methods.createDynamicCheckpoint(new mp.Vector3(item[0].x, item[0].y, item[0].z + 0.5), "Tıklayın ~g~E~w~ acıga cıkarmak icin ТС Satılık", player => {
      if (!player.vehicle) return player.notify("~r~Nakliye isinde olmalısınız");
      if (!user.isDriver(player)) return player.notify("~r~Direksiyonun basında olmalısın.");

      // if (vehSalesPointsData.find((q, i) => !!q && index !== i && q[1] && q[1].id === player.id)) return player.notify("~r~Вы уже продаёте ТС в другом месте");
      if (vehSalesPointsData.find((q, i) => !!q && index !== i && q[0] && q[0].id === player.vehicle.id)) return player.notify("~r~Sizin aracınız zaten baska yerde satıldı");
      if (!checkFree()) {
        let m = menu.new(player, "Araba pazarı", "Eylemler");
        m.newItem({
          name: "~y~Degisim maliyeti",
          onpress: () => {
            if (!canControl(player)) return;
            menu.input(player, "Satıs degerini belirtiniz", "", 9, "int").then(sum => {
              if (!canControl(player)) return;
              if (!sum) return;
              if (sum < 100) return player.notify("~r~Maliyet daha düsük olamaz 100$")
              if (sum > 999999999) return player.notify("~r~Dogru degeri girin");
        ...
        m.newItem({
          name: "~g~Bir yolcuya satın alma teklifi",
          onpress: () => {
            if (!canControl(player)) return;
            const occupants = player.vehicle.getOccupants();
            if (occupants.length < 2) return player.notify("~r~Tasımacılıkta yolcu yoktur");
            if (occupants.length > 2) return player.notify("~r~Aracta sadece siz ve müsteri bulunmalıdır");
            ...
            m.close();
            player.notify("~g~Bir satın alma teklifi gönderildi");
      ...
      if (!owner) return player.notify("~r~Aracın sahibi siz olmalısınız");
      if (player.vehicle.velocity.x + player.vehicle.velocity.y + player.vehicle.velocity.z > 0.1) return player.notify("~r~Satısa cıkarmadan önce TC'yi durdurun")
      menu.input(player, "Satıs degerini belirtiniz", "", 9, "int").then(sum => {
        if (!sum) return;
        if (sum < 100) return player.notify("~r~Maliyet daha düsük olamaz 100$")
        if (sum > 999999999) return player.notify("~r~Dogru degeri girin");
        if (!player.vehicle) return player.notify("~r~Nakliye isinde olmalısınız");
        if (!user.isDriver(player)) return player.notify("~r~Direksiyonun basında olmalısın.");
        if (!checkFree()) return player.notify("~r~Bu koltuk dolu");
        ...
        player.notify("~g~aracınızı satısa çıkardınız. aracınız satıs yerinden ayrılırsa - duyuru geri cekilecektir")
      })
    }, 2.0, 0, [0, 0, 255, 90], 1, 27)
    let showCheckpoint = methods.createDynamicCheckpoint(new mp.Vector3(item[1].x, item[1].y, item[1].z), "Tıklayın ~g~E~w~ aracınızı tanımak icin", player => {
      if (checkFree()) return player.notify("~r~Koltuk bos");
      const veh = vehSalesPointsData[index][0]
      let car = vehicles.getData(veh.getVariable('container'));
      let upgrade = JSON.parse(car.get('upgrade'));
      const name = vehSalesPointsData[index][4];
...
export let vehicles = {
  ...
  respBySlot: function (player: PlayerMp, slot: any, tome = false) {
    ...
    mp.vehicles.forEach((veh) => {
      ...
      if (veh.getVariable('container') == containerId) {
        if (veh.getOccupants().length > 0) {
          player.notify('~r~Calınan araclar');
          isFind = true;
          jacked = true;
          return;
        }
    ...
    if (vehicles.get(containerId, 'sell_price') > 0) {
      player.notify('~r~Önce nakliyeyi pdmden kaldırmanız gerekir');
      return false;
    }
    ...
    if (!tome) {
      player.notifyWithPicture(
        'Nakliye teslimatı',
        'Tamirci',
        'Bir dakika icinde, nakliyeniz orada olacak',
        'CHAR_MECHANIC',
        2
      );
    }
    ...
    setTimeout(async function () {
      if (tome) {
        ...
        if (player.position.z - status > 5) return player.notifyWithPicture(
          'Nakliye teslimatı',
          'Tamirci',
          '~r~Nakliyenizi bulundugunuz yere teslim edemedim',
          'CHAR_MECHANIC',
          2
        );
  ...
  findBySlot: function (player: PlayerMp, slot: any) {
    ...
    if (isFind == false) player.notify('~r~Nakil bulunamadı, onu kurtarın');
  },
  ...
  sell: async (player: PlayerMp, slot: number) => {
    ...
    if (user.get(player, 'car_id' + slot) == 0) {
      player.notify('~r~Ulasımınız yok');
      return;
    }

    let containerId = user.get(player, 'car_id' + slot);
    const veh = mp.vehicles.toArray().find((veh) => veh.getVariable('container') == containerId);
    if (!vehicles.exists(veh) || !veh) return player.notify('~r~Aracı yeniden sıralayarak baslayın');
    ...
    veh.destroy();
    user.log(player, "PlayerSellCar", 'Arabayı sattım @veh' + vInfo.get('id') + ' Fiyat: $' + methods.numberFormat(nalog));
    if (!user.isLogin(player)) return;
    user.addHistory(
      player,
      3,
      'Nakliyeyi sattı ' + vInfo.get('name') + '. Fiyat: $' + methods.numberFormat(nalog)
    );
    player.notify('~g~Aracı sattınız');
    player.notify(
    ...
    try {
      ...
      if (player) {
        ...
        if (player.inGreenZone) return player.notifyWithPicture(
          'Nakliye teslimatı',
          'Tamirci',
          'Yesil bölgeye ulasım saglayamadım',
          'CHAR_MECHANIC',
          2
        );
        ...
        if (player.vehicle) return player.notifyWithPicture(
          'Nakliye teslimatı',
          'Tamirci',
          'Nakliyeyi teslim edemedim zaten bir tür aractasınız?',
          'CHAR_MECHANIC',
          2
        );
        ...
        user.log(player, "SpawnVehicle", `Siparis edilen arac teslimatı @veh${id}`)
      }
      ...
      vehicles.newOrdered(
        (veh) => {
          ...
          if (isRespawn) {
            ...
            methods.debug(
              `[${car.get('id')}] Spawned ${car.get('name')} at ${position.x}, ${position.y}, ${position.z
              }`
            );
          } else {
            setTimeout(function () {
  ...
  spawnCar: (
    ...
    methods.debug("Hash Kodu : " + vInfo.hash)
  ...
  lockStatus: (player: PlayerMp, vehicle: VehicleMp, status: boolean = null) => {
    ...
    try {
      ...
      if (!vehicle.locked && vehicle.engineHealth > 0) {
        player.notify('~g~Aracın kilidi açılldı.');
      }
      else if (vehicle.locked && vehicle.engineHealth > 0) {
        player.notify('~r~Araç kilitendi.');
      } else {
        player.notify('~r~Araç imha edildi.');
        return;
      }
      if (!player.vehicle) {
        user.playAnimation(player, "anim@mp_player_intmenu@key_fob@", "fob_click", 48);
        player.addAttachment('pickPick', false);
        setTimeout(function () {
          try {
  ...
  engineStatus: (player: PlayerMp, vehicle: VehicleMp, status: boolean = null) => {
    ...
    try {
      if (vehicle.getVariable('fuel') == 0) {
        if (status != null) player.notify('~r~Tasımacılıkta benzin bitti');
        if (status != null) player.notify('~b~Yakıt ikmali etiketi ayarlandı');
        if (status != null) {
      ...
      if (status != null) return;
      if (eStatus) player.notify('~g~Motoru calıstırdınız');
      else player.notify('~r~Motoru kapattınız');
    } catch (e) {
      console.log(e);
    }
  },
