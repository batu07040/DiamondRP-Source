...
setTimeout(() => {
  mp.events.register('atm:getCash', (player, sum: number) => {
    if (user.getBankMoney(player) < sum) return player.notify('~r~Bu islem icin yeterli kaynagınız yok')
  ...
  mp.events.register('atm:putCash', (player, sum: number) => {
    if (user.getCashMoney(player) <= sum) return player.notify('~r~Bu islem icin yeterli kaynagınız yok')
...
function warnUser(target: PlayerMp | number, reason: string, admin: string, days = 0) {
  return new Promise(async resolve => {
    ...
    if (days > 0) chat.sendToAll(
      admin,
      `${name} (${id})!{${chat.clRed}} Bir uyarı aldım !{${chat.clWhite}}[${(countWarns + 1)}/3]!{${chat.clRed}} pahasına: !{${chat.clWhite}}${reason}`,
      chat.clRed
    );
    ...
    if (needBan) {
      ...
      if (typeof target !== "number") user.kick(target, "Yönetim tarafından uyarıldınız: " + reason);
      else if (user.getPlayerById(target)) user.kick(user.getPlayerById(target), "Yönetim tarafından uyarıldınız: " + reason);
    }
...
setTimeout(() => {
  chat.registerCommand('cheat', (player, ids: string) => {
    if (!user.isAdminNow(player, 5)) return player.notify("~r~Erisim yok");
    let id = methods.parseInt(ids);
    let target = user.getPlayerById(id);
    if (!target) return player.notify("~r~Oyuncu sunucuda degil");
    user.accept(player, "Emin misin? (ID: " + user.getRpName(target) + " (" + id + "))?").then(status => {
...
export let user = {
  ...
  banCheater: (player: PlayerMp, who?: PlayerMp) => {
    ...
    setTimeout(() => {
      blackListEntity.create({
        lic: serial,
        reason: `Hileleri kullanma [Ödünc ${who ? `${user.getRpName(player)} (${user.getId(player)})` : 'Sistem'}]`,
        guid: social,
        rgscId: rgscId
      }).then(() => {
        if (who) who.notify("~g~Kullanıcı karalistede")
        methods.saveLog("addBlackList", (who ? user.getId(who) : "[Sistem]") + " Girildi в BlackList " + id + " " + social + " " + serial + ' ' + rgscId)
        if (who) user.log(who, "AdminJob", "Aldı BlackList @user" + id + " " + social + " " + serial + ' ' + rgscId)
        if (mp.players.exists(player))
          user.kickAntiCheat(player, "Hileleri Kullanma")
        user.banuser(id, who ? who : "Система", 60000 * 60 * 24 * 30 * 6, "Hileleri Kullanma", true, 'm', 6)
      })
  ...
  clearCheater: (player: PlayerMp, who?: PlayerMp) => {
    ...
    blackListEntity.create({
      lic: serial,
      reason: `Использование читов [Занёс ${who ? `${user.getRpName(player)} (${user.getId(player)})` : 'Sistem'}]`,
      guid: social,
      rgscId: rgscId
    }).then(() => {
      if (who) who.notify("~g~Kullanıcı Karalistede")
      methods.saveLog("addBlackList", (who ? user.getId(who) : "[Sistem]") + " Karalisteye alındı " + id + " " + social + " " + serial + ' ' + rgscId)
      if (who) user.log(who, "AdminJob", "Kara listeye alındı @user" + id + " " + social + " " + serial + ' ' + rgscId)
    })
    setTimeout(() => {
  ...
  giveItem: (player: PlayerMp, item_id: number, amount = 1, notify = false, prefix = -1, key = -1) => {
    ...
    if (notify) player.notify(`~g~Almıs oldugunuz x${amount} ${items.getItemNameById(item_id)}`)
  },
  notify: notify,
  ...
  giveVipStatus: (player: PlayerMp, name: vipName, days: number) => {
    ...
    player.outputChatBox(`!{#2ded60}almıs oldugunuz ${vipData.name} at ${days} Gunler`);
    user.updateClientCache(player);
  },
  clearVipStatus: (player: PlayerMp) => {
    if (user.getVipStatusData(player)) {
      player.outputChatBox(`!{#FFC107}Ваш ${user.getVipStatusData(player).name} sona erdi :c`);
    }
  ...
  isDriver: (player: PlayerMp) => {
    ...
    return RAGE_BETA ? player.seat == 0 : player.seat == 0
  },
  ...
  loginterract: (player: PlayerMp | number, target: PlayerMp | number, type: LogType, reason: string) => {
    ...
    log.new(type, log.convertIdPlayerToData(player) + " ile etkilesim " + log.convertIdPlayerToData(target) + ": " + reason);
  },
  ...
  generateCustomizationSettings: (player: PlayerMp, admin = false): Promise<PlayerCustomizationObject> => {
    return new Promise((resolve, reject) => {
      ...
      components.forEach((item, id) => {
        ...
        m.newItem({
          name: "Kıyafet numarası" + id + " | Texture: ",
          type: "range",
          rangeselect: [0, 1001],
          onchange: (value) => {
      ...
      m.newItem({
        ...
        onpress: () => {
          ...
          menu.input(player, "Verileri kopyalayın", JSON.stringify(<PlayerCustomizationObject>{
            components: c,
            props: p,
          }), 100000, "text")
        }
  ...
  giveQuest: (player: PlayerMp, name: string, notify = false) => {
    ...
    if (notify) player.notify("Yeni bir görev alındı: " + name)
    return true;
  },
  /** Удаление квеста */
  ...
  completeQuest: (player: PlayerMp, name: string, nocheck = false) => {
    ...
    if (nocheck || (getQuest(item.name) && getQuest(item.name).result(player))) {
      item.complete = true;
      player.notify("Görev Tamamlandı: " + name)
  ...
  testPC: async (player: PlayerMp) => {
    if (player.testPc) return player.notify("~r~Bu test her 2 dakikada bir calıstırılabilir");
    let m = menu.new(player, "Kontrol Et", "Eylemler");
    m.newItem({
      name: "~g~Baslayın",
      desc: "Sisteminizin bazı unsurlarını kontrol edecek karmasık bir kod çalıstırması yapacagız ve bunun sonucunda herhangi bir sorun varsa gelistiricilere bildirmeniz icin size bir sistem endeksi verecegiz",
      onpress: () => {
        if (player.testPc) return player.notify("~r~Bu test her 2 dakikada bir calıstırılabilir");
        if (player.vehicle) return player.notify("~r~Nakliyeyi Bırakın");
        player.testPc = true;
        setTimeout(() => {
          player.testPc = false;
        }, 120000)
        ...
        player.notify("Prosedür baslatıldı, tamamlanması bekleniyor");
        mp.events.callClient(player, 'pc:test').then((result: { [x: string]: number }) => {
          ...
          player.notify("Bu bilgileri gelistiriciye saglayın")
          let q = menu.new(player, "Sonuc", "Gönderilecek veriler");
          let data: [string, any][] = []
          ...
          q.newItem({
            name: "~g~Kopyalama",
            onpress: () => {
              menu.input(player, "Sonuc", data.map(q => { return q.join(' > ') }).join(', '), 5000, "text");
            }
          })
          q.newItem({
            name: "~r~Kapat",
  ...
  testNet: async (player: PlayerMp) => {
    if (player.testPing && !user.isAdminNow(player)) return player.notify("~r~Bu test her 2 dakikada bir calıstırılabilir");
    ...
    player.notify("Baglantı kontrolu gerçeklestirin, tamamlanmasını bekleyin...")
    let res: number[] = [];
    let sum = 0;
    let min = 0;
    let max = 0;
    ...
    player.notify("Test tamamlandı, sonuc sohbet odasında")
    if (user.isAdminNow(player)) return;
    setTimeout(() => {
      player.testPing = false;
    }, 120000)
  },
  accept: (player: PlayerMp, text: string = null, acceptBtn = "~g~Kabul Etmek", declineBtn = "~r~Reddet", acceptAnyTime = false): Promise<boolean> => {
    return new Promise((resolve) => {
      let m = menu.new(player, "istek", text);
  ...
  startSpectate: (player: PlayerMp, target: PlayerMp) => {
    if (player.spectateTarget) return player.notify("Zaten birini izliyorsun.")
    if (player == target) return player.notify("Kendinize göz kulak mı oluyorsunuz")
    ...
    player.notify("Gözlem modundan çıkma - /sp")
  },
  ...
  unbanuser: (id: number, who?: PlayerMp | string, reason?: string) => {
    user.getUserDataFromDB(id).then(data => {
      ...
      banListEntity.create({
        ...
        format: "",
        reason: reason ? reason : "Yasaklanmamıs",
        full_reason: "",
        ip: "",
      })
      user.notify(id, `${((typeof who == "string") ? who : user.getRpName(who))} ${((typeof who == "string") ? '' : `(${user.getId(who)})`)} hesabınızın yasagını kaldırdı`, 'Yönetim', "chat", "error");
      data.date_ban = 0;
      data.save();
    })
  },
  ...
  banuser: (id: PlayerMp | number, who: PlayerMp | string, time: number, reason: string, notifyAndRepeate = true, texttype?: string, texttime?: number) => {
    ...
    const targetid = id
    if (typeof who != "string" && id == user.getId(who)) return who.notify("Kendini yasakla");
    ...
    user.checkIdUser(targetid).then(fnd => {
      if (fnd == -1 && who) return typeof who != "string" ? who.notify("Oyuncu tespit edilmedi") : "";
      if (fnd != 0 && who) return typeof who != "string" ? who.notify("Oyuncu bir yöneticidir") : "";
      if (notifyAndRepeate && nplayer && who) {

        chat.sendToAll(
          `Yönetici  ${((typeof who == "string") ? who : user.getRpName(who))} ${((typeof who == "string") ? '' : `(${user.getId(who)})`)}`,
          `${user.getRpName(nplayer)} (${user.getId(nplayer)})!{${chat.clRed}} adlı kişiyi ${formattedTime} ${date.getDate()}/${date.getMonth() +
          1}/${date.getFullYear()} tarihine kadar banladı. !{${chat.clWhite
          }} Sebep: ${reason}`,
          chat.clRed
        );
      }
      if (nplayer) user.kick(nplayer, `Bir yönetici tarafından yasaklandınız ${((typeof who == "string") ? who : user.getRpName(who))}, Sebebi: ${reason}`);

      userEntity.update({
        date_ban: endtime
      }, { where: { id: targetid } })
      if (!notifyAndRepeate) return;
      user.log(id, "PlayerBan", (who ? (typeof who != "string" ? log.convertIdPlayerToData(who) : who) + " yasaklandı" : "yasaklandı") + " " + log.convertIdPlayerToData(id) + ` до ${formattedTime} ${date.getDate()}/${date.getMonth() +
        1}/${date.getFullYear()} с neden: ${reason}`)
      ...
      typeof who != "string" ? who.notify("Kullanıcı engellendi.") : "";
      setTimeout(() => {
        userEntity.update({
          date_ban: endtime
        }, { where: { id: targetid } })
  ...
  authAccount: async (
    ...
    if (fndacc) return player.notify("~r~Bu hesap zaten yetkilendirilmistir");
    if (user.get(player, 'is_online') == 1) {
      player.notify('~r~Hesap zaten yetkilendirilmis')
      mp.events.triggerBrowser(player, 'cef:login:updateSendStatus');
      return;
    }
    if (db_user.date_ban > methods.getTimeStamp()) {
      player.notify(
        '~r~Hesabınız şu tarihe kadar yasaklandı: ~s~' + methods.unixTimeStampToDateTime(db_user.date_ban)
      );
    ...
    if (user.isLogin(player)) return;
    player.notify('~b~Hesabınıza giriş yapılıyor...');
    user.set(player, 'id', db_user.id);
    // player.db_user = db_user;
    user.log(db_user.id, "PlayerJoin", "Giriş yapıldı.")
    ...
    user.init(player).then(() => {
      ...
      // }, 1000)
      setTimeout(function () {
        if (mp.players.exists(player)) user.removeQuest(player, "Kalıntıları toplamak")
      }, 5000)
      if (user.get(player, "exp_age") > 0) {
        player.outputChatBox(
          '!{#A83242}Menüyü Acın (М) -> Ayarlar ve karakterinizin RP yasını belirleme.'
        );
      }
      ...
      user.getActiveWarns(db_user.id).then(items => {
        ...
        items.forEach(item => {
          ...
          player.outputChatBox(`!{#eb3636}Yeni bir uyarınız var${timetext}. Tarafından yayınlandı: ${item.admin}, Sebebi: ${item.reason}`);
          item.notified = true;
          item.save();
        })
      })
  ...
  register: async (
    ...
    if (lastIdentityRegister.has(player.ip)) {
      player.notify('~r~Yakın zamanda IP adresinizle bir hesap kaydedildi. Lütfen daha sonra tekrar deneyin');
      mp.events.triggerBrowser(player, 'cef:register:updateSendStatus');
      return;
    }
    if (lastIdentityRegister.has(player.serial)) {
      player.notify('~r~Cihazınıza yakın zamanda bir hesap kaydedilmistir. Lütfen daha sonra tekrar deneyin');
      mp.events.triggerBrowser(player, 'cef:register:updateSendStatus');
      return;
    }
    if (lastIdentityRegister.has(player.socialClub)) {
      player.notify('~r~SocialClub hesabınız kısa süre önce kaydedildi. Lütfen daha sonra tekrar deneyin');
      mp.events.triggerBrowser(player, 'cef:register:updateSendStatus');
      return;
    }
    ...
    player.notify('~b~Veri kontrolü...');
    const nicknames = await userEntity.count({
      where: { rp_name: nick }
    })
    if (nicknames) {
      player.notify('~r~RP adınız sistemde zaten kayıtlı!');
      mp.events.triggerBrowser(player, 'cef:register:updateSendStatus');
      return;
    }
    ...
    if (accounts >= 3 && !methods.isTestServer()) {
      player.notify('~r~Aynı SocialClub ile en fazla 3 hesap oluşturabilirsiniz.');
      mp.events.triggerBrowser(player, 'cef:register:updateSendStatus');
      return;
    }
    ...
    if (promocode) {
      ...
      if (promocodes == 0) {
        player.notify('~r~Özel promosyon kodu bulunamadı');
        player.notify('~r~Normal promosyon kodları M - Ayarlar - Promosyon kodu aracılıgıyla girilir');
        mp.events.triggerBrowser(player, 'cef:register:updateSendStatus');
        return;
      }
    }
    ...
    player.notify('~b~Hesap olusturma...');
    ...
    userEntity.create({
      ...
      user.log(db_user.id, "PlayerRegister", "kayıtlı")
      try {
        mp.players.toArray().filter(target => user.isAdmin(target) && target.registerAlert).forEach(target => {
          target.outputChatBox(`!{#C300FF}Yeni Kayıt: ${db_user.id} | ${db_user.name}`);
        })
      } catch (error) {
        console.error(error);
      }
      ...
      if (!mp.players.exists(player)) return;
      player.notify('~b~Yetkilendirme icin hazırlık...');
  ...
  donePersonage: async (player: PlayerMp) => {
    ...
    user.init(player).then(() => {
      ...
      if (is_reg) {
        let desc = 'Yaz mevsimidir.';
        if (weather.getMonth() == 12 || weather.getMonth() < 3)
          desc = 'Kış mevsimidir.';
        else if (weather.getMonth() >= 3 && weather.getMonth() < 6) desc = 'Bahar.';
        else if (weather.getMonth() >= 9 && weather.getMonth() < 12) desc = 'Sonbahar. Yağmur mevsimi.';

        player.outputChatBox(
          '!{#FFC107}Uçağınız iniş yaptı. Los Santos, yolculuğunuza başlamak için Belediye binasına giden otobüse bindiniz. Keyifli yolculuklar.'
        );

        player.outputChatBox(`!{#FFC107}Şimdi dışarıda mevsim ${desc}`);
        player.outputChatBox(`!{#FFC107}Hava Sıcaklığı: ${weather.getWeatherTemp().toFixed(1)}°C`);
        player.outputChatBox(`!{#FFC107}Yerel Saat ve Tarih: ${weather.getRpDateTime()}`);
      }
      setTimeout(function () {
        user.resetCustomization(player)
      }, 1000)
  ...
  init: async (player: PlayerMp) => {
    return new Promise<void>(async (resolve, reject) => {
      try {
        ...
        if (!id || id == -1) return user.kick(player, "Bir hata olustu");
        ...
        if (!db_user) return user.kick(player, "Bir hata olustu");
        ...
        setTimeout(function () {
          if (!mp.players.exists(player)) return;
          if (customParams.to20) player.outputChatBox('!{#6BFD81}En iyi 20 aktif oyuncu arasına girin, en yuksek 100000$');
          if (customParams.fiveAndTenBonus) player.outputChatBox('!{#6BFD81}Bugunun promosyonu gecerlidir: Bes saat boyunca oynayın = 50,000$, 10 часов = 100,000$');
          if (customParams.donatex2) player.outputChatBox('!{#6BFD81}Bugunun promosyonu geçerlidir: X2 Bagısla');
          if (customParams.donatex3) player.outputChatBox('!{#6BFD81}Bugunun promosyonu gecerlidir: X3 Bagısla');
          if (customParams.agex2) player.outputChatBox('!{#6BFD81}Dahil olan tum oyuncular icin X2 deneyim');
          if (customParams.agex2new) player.outputChatBox('!{#6BFD81}Oyuncular icin 1-' + levelAccess.x2lvlNewPlayers + ' seviye dahil X2 deneyim');
          if (customParams.jobx2) player.outputChatBox('!{#6BFD81}На sunucu su anda bir promosyon yurutuyor: X2 calısmalarda');
          if (customParams.gosx2) player.outputChatBox('!{#6BFD81}Sunucu su anda bir promosyon yurutuyor: X2 kamu kurumlarının calısanlarının maaslarına');
          if (user.getPlayerFraction(player) && !user.get(player, "tablet_equip")) player.outputChatBox('!{#FF0711}Grup menülerine erismek icin bir tablet edinin ve donatın. Bunu bir elektronik magazasında yapabilirsiniz');
        ...
        if (backlist) {
          setTimeout(() => {
            ...
            user.log(player, "TryBlackList", `ID: ${backlist.id}, Sebebi: ${backlist.reason ? `: ${backlist.reason}` : ''}, Social: ${player.socialClub}, RGSCID: ${player.clientSocial}, Lic: ${player.serial}`)
            user.bigAlert(player, `Proje kara listesindesiniz${backlist.reason ? `: ${backlist.reason}` : ''}. Giris numarası ${backlist.id}`, "error", 100000);
  ...
  saveAccount: function (player: PlayerMp, withReset = false): Promise<string> {
    return new Promise((resolve, reject) => {
      try {
        ...
        if (!id || id == -1) return resolve("Kullanıcı kimligi alınamadı");


        if (!user.get(playerid, 'age')) return resolve("Oyuncu yası tespit edilemedi");
        ...
        usr.hp = player.health
        usr.pos = player.position
    ...
    userEntity.findAll({
      ...
      for (let q in rows) {
        ...
        if (
          ...
          let lastpos = JSON.parse(row.pos);
          spawns.push([
            'Cıkıs noktası',
            lastpos.x,
            lastpos.y,
        ...
        const fslist: Object = {
          1: { x: 23.20, y: -1396, z: 33 },
          2: { x: 23.20, y: -1396, z: 33 },
          3: { x: 23.20, y: -1396, z: 33 },
          4: { x: 23.20, y: -1396, z: 33 },
          7: { x: 23.20, y: -1396, z: 33 },
          8: { x: 23.20, y: -1396, z: 33 },
          9: { x: 23.20, y: -1396, z: 33 },
          10: { x: 23.20, y: -1396, z: 33 },
          11: { x: 23.20, y: -1396, z: 33 },
          16: { x: 23.20, y: -1396, z: 33 },
          18: { x: 23.20, y: -1396, z: 33 },
          19: { x: 23.20, y: -1396, z: 33 },
          20: { x: 23.20, y: -1396, z: 33 },
          21: { x: 23.20, y: -1396, z: 33 },
        }
        if (row.fraction_id) {
          spawns.push([
            'Organizasyon',
            fslist[row.fraction_id].x,
            fslist[row.fraction_id].y,
            fslist[row.fraction_id].z,
            0,
            0,
            75,
          ]);
        ...
        if (row.fraction_id2 == 500) {
          spawns.push(['Catı katı', 988.42, 71.50, 115.18, 0]);
        }
        for (let i = 1; i < 9; i++) {
          ...
          if (carId > 0) {
            ...
            if (rows.length) {
              ...
              if (hash == 1876516712 || hash == -120287622) {
                ...
                spawns.push(['Fragman', posX, posY, posZ, 0]);
              }
            }
          }
        }
        ...
        if (row.condo_id > 0) {
          ...
          if (hData) spawns.push(['Duz', hData.get('x'), hData.get('y'), hData.get('z'), 0, 0]);
        }

        if (row.id_house > 0) {
          let hData = houses.getHouseData(row.id_house);
          if (hData) spawns.push(['Ev', hData.get('x'), hData.get('y'), hData.get('z'), 0, 0]);
        }

        if (row.stock_id > 0) {
          let hData = stock.getData(row.stock_id);
          if (hData) spawns.push(['Depo', hData.get('x'), hData.get('y'), hData.get('z'), 0, 0]);
        }

        switch (methods.getRandomInt(0, 3)) {
          case 0:
            spawns.push(['Standart yumurtlama', 124.8076, -1215.845, 28.33152, 0]);
            break;
          case 1:
            spawns.push(['Standart yumurtlama', 1.66987, -1225.569, 28.29525, 0]);
            break;
          case 2:
            spawns.push(['Standart yumurtlama', 462.8509, -850.47, 26.12981, 0]);
            break;
        }
  ...
  mutePlayer: (target: PlayerMp, who: PlayerMp, minutes: number, type: "voice" | "chat", reason?: string) => {
    ...
    chat.sendToAll(
      ...
      `${user.getRpName(target)} (${user.getId(target)})!{${chat.clRed}} alındı ${(muteContainer[type].get(id) ? 'mutha uzantili' : 'мут')} ${(type == "voice") ? 'sesli sohbet' : ''} до ${formattedTime} ${date.getDate()}/${date.getMonth() +
      1}/${date.getFullYear()} с причиной!{${chat.clWhite
      }} ${reason}`,
      chat.clRed
    );
    setTimeout(() => {
  ...
  getRegStatusName: function (player: PlayerMp) {
    ...
    if (!mp.players.exists(player)) return '~r~Hayır';
    switch (user.get(player, 'reg_status')) {
      case 1:
        return 'gecici';
      case 2:
        return 'vatandaslık kazanımı';
      case 3:
        return 'ABD vatandaslıgı';
      default:
        return '~r~Hayır';
    }
  },
  ...
  savePos: function (player: PlayerMp) {
    const id = user.getId(player);
    userEntity.update({
      pos: player.position,
      dimension: 0, //player.dimension
      hp: player.health
    }, {
      where: {
        id
  ...
  setChips: function (player: PlayerMp, money: number, save = true) {
    ...
    if (save) user.log(player, "PlayerSystem", "ile kumarhane fislerinin dengesini degistirmek " + player.chipsBalance + " на " + money)
  ...
  addMoneyOffline: function (id: number, money: number, title?: string, notify?: string) {
    if (notify) user.notify(id, `${notify}: ~g~$${methods.numberFormat(money)}`, `${title}`, "chat", "success");
    let target = user.getPlayerById(id);
    if (target) {
  ...
  setCashMoney: function (player: PlayerMp, money: number) {
    // if (money <= 0) return;
    user.log(player, "PlayerSystem", "ile eldeki bakiyeyi degistirmek " + user.getCashMoney(player) + "$ at " + money + "$")
  ...
  setBankMoney: function (player: PlayerMp, money: number) {
    user.log(player, "PlayerSystem", "banka bi̇lancosunun degi̇sti̇ri̇lmesi̇ с " + user.getBankMoney(player) + "$ на " + money + "$")
  ...
  giveTaxiJobMoney: function (player: PlayerMp, money: number) {
    ...
    if (user.get(player, 'bank_prefix') < 1) {
      ...
      player.notify('Kazancınız: ~g~$' + taxiMoney);
    } else {
      user.addBankMoney(player, taxiMoney);
      bank.sendSmsBankOperation(player, `Ücretiniz Yatırıldı: ~g~$${taxiMoney}`);
    }
  ...
  giveJobMoney: function (player: PlayerMp, money: number) {
    ...
    if (user.get(player, 'bank_prefix') < 1) {
      user.addCashMoney(player, money);
      player.notify('Kazancınız: ~g~$' + money);
    } else {
      user.addBankMoney(player, money);
      bank.sendSmsBankOperation(player, `Maaşınız Yatırıldı: ~g~$${money}`);
    }
  ...
  arrest: function (player: PlayerMp) {
    user.log(player, "GosJail", 'icin hapsedildi. ' + user.get(player, 'wanted_level') + ' лет')
    ...
    user.addHistory(
      player,
      1,
      'icin hapsedildi. ' + user.get(player, 'wanted_level') + ' лет'
    );
  ...
  arrestAdmin: (player: PlayerMp | number, admin: PlayerMp | string, time: number, reason: string, name?: string): Promise<boolean> => {
    return new Promise(resolve => {
      ...
      if (isNaN(time) || time < 0) return (typeof admin == "string" ? null : admin.notify("Saat dogru degil")), resolve(true);
      if (!reason || reason.length < 2) return (typeof admin == "string" ? null : admin.notify("Tutuklama nedenini belirtiniz")), resolve(true);
      user.checkIdUser(id).then(fnd => {
        if (fnd == -1 && typeof admin != "string") return (admin.notify("Oyuncu tespit edilemedi.")), resolve(true);
        if (fnd == -1) return resolve(true);
        if (fnd > 0 && typeof admin != "string" && time > 0) return (admin.notify("Yöneticiyi hapse atamazsınız.")), resolve(true);
        ...
        if (typeof player != "number" && time > 0) chat.sendToAll(
          `Yönetici ${typeof admin == "string" ? admin : `${user.getRpName(admin)} (${user.getId(admin)})`}`,
          `${user.getRpName(player)} (${user.getId(player)})!{${chat.clRed}} bir sebepten dolayı hapsedildi!{${chat.clWhite
          }} ${reason} at ${time} min.`,
          chat.clRed
        );
        else if (name && time > 0) {
          chat.sendToAll(
            `Yönetici ${typeof admin == "string" ? admin : `${user.getRpName(admin)} (${user.getId(admin)})`}`,
            `${name} (${id})!{${chat.clRed}} bir sebepten dolayı hapsedildi!{${chat.clWhite
            }} ${reason} at ${time} min.`,
            chat.clRed
          );
        }
        else if (typeof admin != "string" && time > 0) admin.outputChatBox(
          `!{#f44336}Sen ektin!{FFFFFF} Oyuncu ID: ${id} bir sebepten dolayı hapse: !{${chat.clWhite
          }}${reason} at ${time} min.`
        );
        if (time == 0) {
          chat.broadcastAdmins(
            `Yönetici ${typeof admin == "string" ? admin : `${user.getRpName(admin)} (${user.getId(admin)})`}`,
            `${name ? name : "Oyuncu"} (${id})!{${chat.clRed}} hapishaneden serbest bırakıldı`,
            chat.clRed
          );
        }
        if (typeof admin == "string") {
          user.log(player, "AdminJail", `Bir yönetici tarafından hapsedildi ${typeof admin == "string" ? admin : `${user.getRpName(admin)} (${user.getId(admin)})`} at ` + time + ' nedenli yıllar: ' + reason)
        } else {
          user.loginterract(player, admin, "AdminJail", `Bir yönetici tarafından hapsedildi ${typeof admin == "string" ? admin : `${user.getRpName(admin)} (${user.getId(admin)})`} at ` + time + ' nedenli yıllar: ' + reason)
        }
        user.notify(id, `Bir yönetici tarafından hapsedildiniz ${typeof admin == "string" ? admin : `${user.getRpName(admin)} (${user.getId(admin)})`} at ${time} nedenli dakikalar: ${reason}`, "Yönetim", "chat", "error")
  ...
  giveWanted: function (player: PlayerMp, level: number, reason: string) {
    ...
    if (reason == 'clear') {
      user.log(player, "GosUnWanted", `arananlar li̇stesi̇ kaldirildi`)
      user.set(player, 'wanted_level', 0);
      user.set(player, 'wanted_reason', 0);
      mp.events.triggerBrowser(player, 'cef:hud:setWantedLevel', 0);
      user.addHistory(player, 1, 'Bir arama temizlendi');
    } else {
      ...
      user.log(player, "GosWanted", `aranıyor ilanı verildi. ${level}, neden: ${reason}`)
      ...
      if (currentLvl + level >= 10) {
        ...
        mp.events.triggerBrowser(player, 'cef:hud:setWantedLevel', 10);
      } else user.set(player, 'wanted_level', currentLvl + level);
      user.set(player, 'wanted_reason', reason);
      mp.events.triggerBrowser(player, 'cef:hud:setWantedLevel', currentLvl + level);
      user.addHistory(player, 1, 'Bir aranıyor ilanı yayınlandı ' + level + '. Sebebi: ' + reason);
    }
    user.updateClientCache(player);
  },
  ...
  setIzol: function (player: PlayerMp, number: number, reason: string) {
    ...
    player.notify('~r~Tecrit edildiniz №' + number + ' с neden:');
  ...
  giveJailRun: function (player: PlayerMp) {
    ...
    player.notify('~g~Size bir yürüyüs izni verildi');
  },
  ...
  takeJailRun: function (player: PlayerMp) {
    ...
    player.notify('~g~Yürüyüs izniniz elinizden alındı');
  },
  ...
  kick: function (player: PlayerMp, reason: string, title = 'Kovuldun.') {
    if (!mp.players.exists(player)) return;
    methods.debug('user.kick ' + player.socialClub + ' ' + reason);
    player.outputChatBox('!{f44336}' + title);
    player.outputChatBox('!{f44336}Sebebi: !{FFFFFF}' + reason);
    user.log(player, "PlayerKick", `${title}, neden: ${reason}`)
    user.notify(player, reason, title, 'chat', 'error')
    player.kick(reason);
  },

  kickAntiCheat: function (player: PlayerMp, reason: string, title = 'Kovuldun..') {
    if (!mp.players.exists(player)) return;
    player.weaponsAll = [];
    methods.debug('user.kickAntiCheat');
    user.kick(player, reason, title);
    user.log(player, "PlayerKick", `[Antichit] ${title}, neden: ${reason}`)
    if (user.isLogin(player)) {
      methods.saveLog('AntiCheat', `${user.getRpName(player)} (${user.getId(player)}) - ${reason}`);
      chat.sendToAll(
        'Anti-Cheat System',
        `${user.getRpName(player)} (${user.getId(player)})!{${chat.clRed}} tekmelendi с neden!{${chat.clWhite
        }} ${reason}`,
        chat.clRed
      );
    }
  ...
  grab: function (player: PlayerMp, shopId: any) {
    ...
    if (mp.players.length < 5) {
      player.notify('~r~Sunucuda 5 ten fazla kisi cevrimici olmalıdır');
      return;
    }
    ...
    if (player.dimension != 0) {
      player.notify('~r~Sanal bir dünyada soygun yapamazsınız');
      return;
    }

    if (Container.Has(shopId, 'isGrabShop')) {
      player.notify('~r~Söz konusu dükkan çoktan soyuldu');
      return;
    }

    if (Container.Has(shopId, 'isGrabGunShop')) {
      player.notify('~r~Dükkan çoktan soyulmuştu.');
      return;
    }
    ...
    if (methods.getRandomInt(0, 1) == 0) {
      dispatcher.sendPos('KOD 3', 'Tüm birimler şuanda soygun gerçekleşiyor.', pos, true, null, [2, 7, 3]);
      player.notify('~y~Alarm tetiklendi.');
    }

    setTimeout(function () {
      if (!user.isLogin(player)) return;
      if (methods.distanceToPos(pos, player.position) > 2) {
        return player.notify("~r~Cok uzaga gittin")
      }
      user.isPlayAnimation(player, 'anim@heists@money_grab@duffel', 'loop').then(stas => {
        if (!stas) return player.notify("~r~Animasyonu yarıda kestiniz")
        user.minigame(player, "gr6").then(status => {
          if (methods.distanceToPos(pos, player.position) > 2) {
            player.notify("~r~Cok uzaga gittin")
          } else {
            user.reset(player, 'isGrabingShop');
            user.giveWanted(player, 4, 'Soygun');
            const sum = methods.getRandomInt(4500, 5000)
            if (!player.bankgrabmoney) player.bankgrabmoney = 0;
            player.notify(`~g~Bir dükkanı soydunuz. $${sum}`)
            player.bankgrabmoney += sum;
            user.setComponentVariation(player, 5, 45, 0);
            if (user.isGang(player) || user.isMafia(player)) {
              player.notify('~y~Simdi para cantasını fraksiyonun kasasına götürmeniz gerekiyor');
            } else {
  ...
  payDay: async function (player: PlayerMp) {
    ...
    if (user.get(player, 'reg_time') <= 0 && user.get(player, 'reg_status') == 2) {
      ...
      player.notifyWithPicture(
        '323-555-0001',
        'Avukat',
        'Tebrikler, ABD vatandaşlığına kabul edildiniz.',
        'CHAR_BARRY',
        2
      );
    }
    ...
    if (user.getExp(player) >= user.getMaxExp(player)) {
      ...
      bank.sendSmsBankOperation(
        player,
        'Yeni bir seviyeye ulasmanın serefine (' + user.getLevel(player) + '), мы дарим Вам ~g~$' + moneylevel,
        'Yeni bir seviye'
      );
      ...
      if (user.getLevel(player) == 3 && user.get(player, 'referer') != '' && user.get(player, "exp_age") == 0) {
        ...
        player.notify(`~g~Almıs oldugunuz $25,000 sevk sistemi hakkında`);
        player.notify(
          `~g~Davetkar ${user.get(player, 'referer')} alındı 200ac kisisel hesap`
        );
      ...
      if (user.getLevel(player) == 3 && user.get(player, 'promocode') != '' && user.get(player, "exp_age") == 0) {
        player.notify(
          `~g~Almıs oldugunuz ~s~$25000 ~g~по promosyon kodu ~s~${user.get(player, 'promocode')}`
        );
        user.addCashMoney(player, 25000);
    ...
    if (user.isAfk(player)) {
      if (!user.isLogin(player)) return;
      return player.notify(`~r~Asagıdaki nedenlerden dolayı maaslarınızı alamadınız AFK`)
    }
    ...
    if (user.get(player, 'is_old_money')) {
      ...
      else player.notify('~r~Emeklilik fonu gelirinizi buldu, emekli maaşı alamazsınız');
    } else if (user.get(player, 'posob')) {
      if (money == 0) money += coffer.getPosob()
      else player.notify('~r~Sosyal yardım fonu geliriniz olduğunu tespit ederse, yardım alamazsınız');
    }

    if (customParams.fiveAndTenBonus) {
      if (player.played_time == 5) moneyBonus += 50000, player.notify('~g~Oynadığınız 5 saat için bir bonus aldınız');
      if (player.played_time == 10) moneyBonus += 50000, player.notify('~g~Oynadıgınız 10 saat için bir bonus aldınız');
    }
    ...
    if (money > 0 || moneyBonus > 0) {
      ...
      if (user.get(player, 'bank_prefix') > 0) {
        ...
        player.notify(`~r~Bir banka kartına başvurmanızı siddetle tavsiye ederiz`)
      }
      bank.sendSmsBankOperation(player, `~g~~b~Miktar: ~g~$${money}${(!user.get(player, 'is_old_money') && money > 0) ? `\n~b~Vergi: ~g~$${nalog} ~w~${coffer.getNalog()}%` : ``}${(moneyBonus > 0) ? `\n~b~Artıs: ~g~$${moneyBonus}` : ``}`, 'Fonların kredilendirilmesi');
    }
      ...
      if (user.has(player, 'skill_' + job)) {
        ...
        if (currentSkill == skillCount - 1) {
          ...
          methods.getSkillCountPlayers(job, currentSkill, function (count) {
            if (count < 3) {
              chat.sendToAll('Rekabet', `${user.getRpName(player)} !{${chat.clBlue}} San Andreas eyaletinin en iyi calısanlarından biri oldu ve ödül aldı $10,000`, chat.clBlue);
          ...
          player.notify('~g~Mesleğinizin becerisi geliştirildi');
        }
      }
    }
  },
  ...
  checkCanHandCuff: (player: PlayerMp, target: PlayerMp) => {
    ...
    if (user.isCuff(target)) {
      player.notify('~r~Oyuncu zaten kelepceli');
      return false;
    }
    if (user.isCuff(player) || user.isTie(player)) {
      player.notify("~r~Ya kelepcelisiniz ya da baglısınız");
      return false;
    }
    if (target.vehicle) {
      player.notify("~r~Taşıma sırasında bir kişiyi kelepçelememelisiniz");
      return;
    }
    if (target.health == 0) {
      player.notify("~r~Komadaki bir insanı kelepçeleyemezsiniz.");
      return false;
    }
    if (player.health == 0) {
      player.notify("~r~Komadasın.");
      return false;
    }
    if (player.vehicle) {
      player.notify("~r~Taşıma sırasında bir kişiyi kelepçelememelisiniz");
    ...
    if (target.inGreenZone && !user.isGosPD(player)) {
      player.notify("~r~Yeşil alanda bir kişiye kelepçe takamazsınız");
      return false;
    }
    if (!user.isGosPD(player) && !user.isMafia(player) && !user.isGang(player)) {
      player.notify("~r~Kelepçe kullanamazsınız");
      return false;
    }
    if (user.isInMp(target) && target.dimension != 0) {
      player.notify("~r~Burada kelepçelenemezsin.");
      return false;
    }
    return true;
  },
  ...
  tieBandage: function (player: PlayerMp) {
    ...
    player.notify('~r~Gözbağı takıyorsun');
  },
  ...
  unTieBandage: function (player: PlayerMp) {
    ...
    player.notify('~g~Gözbağın çıkarıldı.');
  },
...
export function banUser(player: PlayerMp, id: number, name: string, lvl: number, reason: string) {
  if (!mp.players.exists(player)) return;
  if (user.getAdminLevel(player) < 2) return player.notify("~r~Oyuncu yasaklarına erişiminiz yok")
  let target = user.getPlayerById(id);
  let block = menu.new(player, `Kullanıcı yasağı`, `${name} (${id}) ${target ? 'Online' : 'Offline'}`);
  let month = 0;
  let days = 0;
  let hours = 0;

  block.newItem({ name: "Ay sayısı", type: "range", rangeselect: [0, 56], onchange: (value) => month = value })
  block.newItem({ name: "Gün sayısı", type: "range", rangeselect: [0, 31], onchange: (value) => days = value })
  block.newItem({ name: "Saat sayısı", type: "range", rangeselect: [0, 24], onchange: (value) => hours = value })


  block.newItem({
    name: "~r~Yasaklan", type: "select", onpress: () => {
      let time = (hours * 60 * 60) + (days * 60 * 60 * 24) + (month * 60 * 60 * 24 * 30);
      if (time <= 0) return player.notify("~r~Zaman belirtilmemiş");
  ...
  block.newItem({ name: "Geri", type: "select", onpress: () => workUser(player, id, name, lvl) });
...
export async function workUser(player: PlayerMp, id: number, name?: string, lvl?: number) {
  ...
  if (methods.isTestServer() && !user.isAdminNow(player) && target) {
    let block = menu.new(player, `Eylem`);
    block.newItem({
      name: "Oyuncuya teknik destek",
      onpress: () => {
        if (target.teleportBlock && user.getAdminLevel(player) < 5) return player.notify(`~r~Bu yönetici şu adrese ışınlanamaz`);
  ...
  if (!user.isAdminNow(player)) return player.notify("Yalnızca yöneticilerin kullanabileceği eylemler");

  let block = menu.new(player, `Eylem`, `${name} (${id}) ${target ? 'Online' : 'Offline'}`);
  if (target) {
    if (adminLvl > lvl) block.newItem({
      name: "Tekme",
      onpress: () => {
        if (lvl > 0) return workUser(player, id, name, lvl), player.notify("Bir yöneticiyi atamazsınız");
        menu.input(player, "Bir neden girin (en az 3 karakter)").then(res => {
          if (!res || res.length < 3) return workUser(player, id, name, lvl), player.notify("Sebep belirtilmedi");
          user.kick(target, res, 'Yönetici tarafından tekmelendi');
          user.loginterract(player, target, "AdminJob", "Tekmelendi")
          player.notify("Bir oyuncu atılır");
        })
      }
    })
    block.newItem({
      name: "Kelepçeleri çıkar / tak",
      onpress: () => {
        if (mp.players.exists(target)) {
          if (user.isCuff(target)) {
            user.unCuff(target)
          ...
          player.notify('~r~Bir oyuncu sunucudan ayrıldı')
        }
      }
    })
    block.newItem({
      ...
      onpress: () => {
        ...
        player.dimension = target.dimension
        if (player.vehicle) player.vehicle.dimension = target.dimension
        user.loginterract(player, target, "AdminJob", "Bir oyuncuya ışınlandı")
      }
    })
    block.newItem({
      name: "Bir oyuncunun TP'si",
      onpress: () => {
        if (target.teleportBlock && user.getAdminLevel(player) < 5) return player.notify(`~r~Bu yönetici ısınlanamaz`);
        ...
        user.loginterract(player, target, "AdminJob", "Bir oyuncuyu ışınladı")
      }
    })
    block.newItem({
      name: "İzleme",
      onpress: () => {
        if (target.teleportBlock && user.getAdminLevel(player) < 5) return player.notify(`~r~Bu yönetici izlenemez`);
        user.startSpectate(player, target)
      }
    })
    block.newItem({
      name: "~y~Mevcut oyuncuyu takip etmekten çık",
      onpress: () => user.stopSpectate(player, target)
    })
    block.newItem({
      name: "~y~Mevcut gözetimi durdurun",
    ...
    if (user.isAdminNow(player, 5)) {
      block.newItem({
        ...
        onpress: () => {
          target.armour = 50;
          user.loginterract(player, target, "AdminJob", "Tarafından yayınlandı 50% армор")
        }
      })
      block.newItem({
        name: "~g~100% армор",
        onpress: () => {
          target.armour = 100;
          user.loginterract(player, target, "AdminJob", "Tarafından yayınlandı 100% армор")
        }
      })
    }
    block.newItem({
      ...
      onpress: () => {
        ...
        user.loginterract(player, target, "AdminJob", "İyilesti")
      }
    })
    block.newItem({
      name: "~r~Öldürmek",
      onpress: () => {
        target.health = 0;
        user.loginterract(player, target, "AdminJob", "Öldürüldü")
      }
    })
    block.newItem({
      name: "~y~Bir oyuncuyu veritabanına kaydetme",
      desc: "Web sitesinde güncel verileri görüntülemek icin gerekli",
      onpress: () => {
        if (mp.players.exists(target)) return;
        if (player.spamProtect) return player.notify("~r~Bu dügmeyi spam yapmayın")
        ...
        user.loginterract(player, target, "AdminJob", "Bir oyuncuyu veritabanına zorladı")
      }
    })
    if (adminLvl >= 4) {
      if (target) {
        block.newItem({
          ...
          onpress: () => {
            menu.selectFraction(player).then(fract => {
              ...
              user.loginterract(player, target, "AdminJob", "Bir fraksiyon yayınladı " + fract)
              user.set(target, "fraction_id", fract);
              user.set(target, "rank", 1);
              methods.saveLog('adminJob', user.getId(player) + " Bir fraksiyon yayınladı " + fract + "kullanıcıya " + user.getId(target));
              user.updateClientCache(target);
              player.notify("~g~Kesir ayarlanır")
              user.saveAccount(target)
        ...
        if (user.getPlayerFraction(target)) {
          block.newItem({
            ...
            onpress: () => {
              ...
              menu.selector(player, "Bir rütbe secin", ["~r~Отмена", ...fractionranks], true).then(rank => {
                if (!rank) return workUser(player, id, name, lvl)
                user.set(target, 'rank', rank);
                player.notify(`~g~Basarıyla`);
                user.updateClientCache(target);
                user.saveAccount(target)
                methods.saveLog('adminJob', user.getId(player) + "bir rütbe verdi " + rank + " kullanıcıya " + user.getId(target));
                return workUser(player, id, name, lvl)
              })
            }
          })
    ...
    if (adminLvl > 5) {
      ...
      block.newItem({
        name: "~o~VIP Durum",
        desc: (user.getVipStatusData(target) ? user.getVipStatusData(target).name + " Ta ki " + methods.unixTimeStampToDateTime(user.get(target, 'vip_time')) : "~r~Hayır"),
        onpress: () => {
          menu.selector(player, "Seciniz VIP", ["~r~Iptal", "~b~Kalkıs", ...vipStatus.data.filter(item => !item.switch).map(item => { return item.name })], true).then(vipid => {
            if (typeof vipid != "number" || !vipid) return workUser(player, id, name, lvl);
            if (vipid == 1) {
              user.accept(player, 'Vipka yı cıkaralım mı?').then(status => {
                if (!status) return workUser(player, id, name, lvl);
                if (!user.getVipStatusData(target)) {
                  player.notify('~r~Oyuncunun VIP statüsü yok')
                  return workUser(player, id, name, lvl);
                }
                user.loginterract(player, target, "AdminJob", "VIP statusu kaldırıldı " + user.getVipStatusData(target).name)
                user.clearVipStatus(target);
                player.notify('~g~VIP statusunu geri cektiniz')
                workUser(player, id, name, lvl);
              })
              return;
            }
            ...
            user.accept(player, "Sorun " + vipcfg.name + "?").then(status => {
              if (!status) return workUser(player, id, name, lvl);
              menu.input(player, "Gun sayısını gırın", "30", 6, "int").then(m => {
                let days = methods.parseInt(m);
                if (isNaN(days) || days < 0) return player.notify("Son teslım tarihi yanlıs")
                user.loginterract(player, target, "AdminJob", "Выдал випку " + vipcfg.name + " на " + days)
                player.notify("Vipa yayınlandı")
                workUser(player, id, name, lvl)
      ...
      block.newItem({
        ...
        onpress: () => {
          menu.input(player, "Seviyeyi girin", user.getLevel(target).toString()).then(m => {
            let money = methods.parseInt(m);
            if (isNaN(money) || money < 0) return player.notify("Yeni denge dogru degil")
            user.loginterract(player, target, "AdminJob", "Bir seviye verdi " + money)
            user.setLevel(target, money);
            player.notify("Denge ayarlandı")
            workUser(player, id, name, lvl)
          })
        }
      })
      block.newItem({
        ...
        onpress: () => {
          let list = ["~r~Hayır", "gecici", "vatandaslık kazanımı", "ABD vatandaslıgı"]
          let submenu = menu.new(player, "Yeni bir durum secin");
          list.map((q, ind) => {
            submenu.newItem({
              name: q,
              onpress: () => {
                workUser(player, id, name, lvl)
                if (!mp.players.exists(target)) return player.notify(`~r~Bir oyuncu sunucudan ayrıldı`);
                user.set(target, 'reg_status', ind)
                user.updateClientCache(target);
                player.notify('~g~Durum değisti');
              }
            })
          })
          submenu.open();
      ...
      block.newItem({
        name: "~o~Casino cipleri",
        desc: "~g~" + user.getChips(target),
        onpress: () => {
          menu.input(player, "Bakiyenizi girin", user.getChips(target).toString()).then(m => {
            let money = methods.parseInt(m);
            if (isNaN(money) || money < 0) return player.notify("Yeni bakiye dogru degil")
            user.loginterract(player, target, "AdminJob", "Tarafından yayınlandı " + (user.getMoney(target) - money) + " kumarhane cipleri ")
            methods.saveLog('adminJob', user.getId(player) + " Tarafından yayınlandı " + (user.getMoney(target) - money) + " kumarhane cipleri " + user.getId(target));
      ...
      block.newItem({
        ...
        onpress: () => {
          menu.input(player, "Bakiyenizi girin", user.getMoney(target).toString()).then(m => {
            let money = methods.parseInt(m);
            if (isNaN(money) || money < 0) return player.notify("Yeni bakiye dogru degil")
            user.loginterract(player, target, "AdminJob", "Tarafından yayınlandı " + (user.getMoney(target) - money) + "$ elimde ")
            methods.saveLog('adminJob', user.getId(player) + " Tarafından yayınlandı " + (user.getMoney(target) - money) + "$ elimde " + user.getId(target));
      ...
      block.newItem({
        name: "~o~Banka bakiyesi",
        desc: "$~g~" + user.getBankMoney(target),
        onpress: () => {
          menu.input(player, "Bakiyenizi girin", user.getBankMoney(target).toString()).then(m => {
            let money = methods.parseInt(m);
            if (isNaN(money) || money < 0) return player.notify("Yeni bakiye dogru degil")
            user.loginterract(player, target, "AdminJob", "Tarafından yayınlandı " + (user.getMoney(target) - money) + "$ bankada ")
            methods.saveLog('adminJob', user.getId(player) + " Tarafından yayınlandı " + (user.getMoney(target) - money) + "$ bankada " + user.getId(target));
      ...
      if (adminLvl >= 6) {
        block.newItem({
          ...
          onpress: () => {
            user.payDay(target);
            player.notify(`~g~PayDay tarafından yayınlandı`)
            user.loginterract(player, target, "AdminJob", "Tarafından yayınlandı PayDay")
          }
        })
        block.newItem({
          name: `~b~Yönetici seviyesi (${user.getAdminLevel(target)})`,
          onpress: () => {
            menu.input(player, "Seviyeyi girin (0-5)", user.getAdminLevel(target).toString()).then(m => {
              let lvl = methods.parseInt(m);
              if (isNaN(lvl) || lvl < 0 || lvl > 6 || (lvl > 5 && !methods.isTestServer())) return player.notify("Seviye dogru degil")
              user.loginterract(player, target, "AdminJob", "Bir yönetici yayınladı " + lvl)
              methods.saveLog('adminJob', user.getId(player) + " Bir yönetici yayınladı " + lvl + " " + user.getId(target));
              user.setAdminLevel(target, lvl);
              player.notify("Yönetici seviyesi ayarlandı")
              workUser(player, id, name, lvl)
            })
          }
        })
        block.newItem({
          name: `~b~Yardımcı seviyesi (${user.getHelperLevel(target)})`,
          onpress: () => {
            menu.input(player, "Seviyeyi girin (0-3)", user.getHelperLevel(target).toString()).then(m => {
              let lvl = methods.parseInt(m);
              if (isNaN(lvl) || lvl < 0 || lvl > 3) return player.notify("Seviye dogru degil")
              user.loginterract(player, target, "AdminJob", "Yardımcıyı verdi " + lvl)
              methods.saveLog('adminJob', user.getId(player) + " Yardımcıyı verdi" + lvl + " " + user.getId(target));
              user.setHelperLevel(target, lvl);
              player.notify("Yardımcı seviye seti")
              workUser(player, id, name, lvl)
            })
          }
        })
    ...
    if (lvl == 0) block.newItem({
      name: "Metin sohbetini sessize alma",
      onpress: () => {
        menu.input(player, "Bir neden girin (en az 3 karakter)").then(res => {
          if (!res || res.length < 3) return workUser(player, id, name, lvl), player.notify("Sebep belirtilmedi");
          menu.input(player, "Dakika sayısını girin").then(m => {
            let minute = methods.parseInt(m);
            if (isNaN(minute) || minute < 0) return player.notify("Dakika sayısı dogru degil")
            user.mutePlayer(target, player, minute, "chat", res)
            user.loginterract(player, target, "AdminJob", "Bir metin sohbetini sessize almak icin " + minute)
            player.notify("Tartışma kabul edildi");
            userList(player)
          })
        })
      }
    ...
    if (lvl == 0) block.newItem({
      name: "Sesli sohbeti sessize alma",
      onpress: () => {
        menu.input(player, "Bir neden girin (en az 3 karakter)").then(res => {
          if (!res || res.length < 3) return workUser(player, id, name, lvl), player.notify("Sebep belirtilmedi");
          menu.input(player, "Dakika sayısını girin").then(m => {
            ...
            user.loginterract(player, target, "AdminJob", "Sesli sohbeti sessize almak icin " + minute)
            player.notify("Tartısma kabul edildi");
            userList(player)
          })
        })
      }
    ...
    block.newItem({
      name: "Metin sohbeti tartısmasını kaldırın",
      onpress: () => {
        user.unmutePlayer(target, "chat");
        player.notify("Tartısma kaldırıldı");
        user.loginterract(player, target, "AdminJob", "Metin sohbetinin sesini kapattım")
      }
    })
    block.newItem({
      name: "Sesli sohbetin sesini kapatma özelligini kaldırma",
      onpress: () => {
        user.unmutePlayer(target, "voice");
        player.notify("Tartısma kaldırıldı");
        user.loginterract(player, target, "AdminJob", "Sesli sohbetin sesini kapattı")
      }
    })
  }
  block.newItem({
    ...
    onpress: () => {
      const q = () => {
        carsEntity.findAll({ where: { id_user: id } }).then(cars => {
          let submenu = menu.new(player, "Tasımacılık listesi");
          cars.map(car => {
            let excar = mp.vehicles.toArray().find(veh => veh.dbid == car.id);
            submenu.newItem({
              name: `(${car.id}) ${car.name}`,
              more: car.number,
              desc: excar ? "~g~Sunucu uzerinde" : "~r~Sunucu uzerinde",
              onpress: () => {
                excar = mp.vehicles.toArray().find(veh => veh.dbid == car.id);
                let submenu2 = menu.new(player, "Eylemler", `(${car.id}) ${car.name} | ${car.number}`);
                submenu2.onclose = () => { q(); }
                submenu2.newItem({
                  name: "Cizme",
                  onpress: () => {
                    excar = mp.vehicles.toArray().find(veh => veh.dbid == car.id);
                    if (user.getAdminLevel(player) < 5) return player.notify(`~r~ aracınızın invertoru uzaktan goruntuleyemezsiniz`);
                    if (excar) return player.notify(`~r~aracınız sunucuda, envantere erismek icin normal yontemi kullanın`)
                    menu.close(player);
                    inventory.openInventory(player, 8, methods.convertNumberToHash(car.number));
                  }
                })
                submenu2.newItem({
                  name: "Makinenin TA'sı kendinize",
                  onpress: () => {
                    excar = mp.vehicles.toArray().find(veh => veh.dbid == car.id);
                    if (!excar) return player.notify(`~r~aracınız sunucuda degil`)
                    try {
  ...
  if (user.getAdminLevel(player) >= 5) {
    block.newItem({
      name: "~b~Envantere bakın",
      onpress: () => {
        if (target) return player.notify(`~r~Cevrimici oyuncu, envantere erismek icin normal yontemi kullanın`)
        menu.close(player);
  ...
  if (adminLvl > lvl) block.newItem({
    ...
    onpress: () => {
      if (user.getAdminLevel(player) == 1) {
        if (mp.players.toArray().find(t => user.getAdminLevel(t) > 1)) return player.notify(`~r~Daha yüksek bir yönetici yoksa yalnızca kendi yöneticilik seviyenizdeki bir oyuncuyu yasaklayabilirsiniz`);
        else player.notify(`~g~Agda daha üst düzey yöneticiler olmadıgı icin, gecici olarak oyuncuları yasaklama erisimine sahipsiniz`);
      }
      if (lvl > 0) return workUser(player, id, name, lvl), player.notify("Bir yöneticiyi yasaklayamazsınız");
      menu.input(player, "Bir neden girin (en az 3 karakter)").then(res => {
        if (!res || res.length < 3) return workUser(player, id, name, lvl), player.notify("Sebep belirtilmedi");
  ...
  if (warns.length > 0 && adminLvl > lvl) {
    block.newItem({
      name: "Aktif savascılar",
      more: warns.length + " parca",
      onpress: () => {
        let submenu = menu.new(player, "Aktif savascıların listesi");
        warns.map(itm => {
          submenu.newItem({
            ...
            desc: itm.reason,
            onpress: () => {
              if (user.getAdminLevel(player) <= 4) return player.notify(`~r~Kaldırma islemi yalnızca 5+ LVL'ye sahip yöneticiler icin gecerlidir`)
              user.accept(player, "Silmek mi?").then(status => {
                if (status) {
                  user.loginterract(player, target, "AdminJob", `Vernik kaldırıldı ${itm.id}, verdi ${itm.admin} до ${methods.unixTimeStampToDateTime(itm.timestamp)} pahasına ${itm.reason}`)
                  itm.destroy();
                }
                workUser(player, id, name, lvl)
              })
  ...
  if (user.getAdminLevel(player) > 5) {
    ...
    block.newItem({
      name: "~o~Madeni Paralar",
      more: "~g~" + coins,
      onpress: () => {
        menu.input(player, "Madeni para sayısını girin", coins.toString()).then(m => {
          let money = methods.parseInt(m);
          if (isNaN(money) || money < 0) return player.notify("Yeni denge dogru degil")
          user.loginterract(player, id, "AdminJob", "Koinin değerini belirledi " + money)
  ...
  if (warnsall.length > 0 && user.getAdminLevel(player) > 5 && adminLvl > lvl) {
    block.newItem({
      name: "Tüm varnalar",
      more: warnsall.length + "parca",
      onpress: () => {
        let submenu = menu.new(player, "Tüm savascıların listesi");
        warns.map(itm => {
          submenu.newItem({
            name: (itm.timestamp > methods.getTimeStamp() ? "~r~[AKTIF] " : "~g~[KAYNAK] ") + methods.unixTimeStampToDateTime(itm.timestamp),
            more: itm.admin,
            desc: itm.reason,
            onpress: () => {
              user.accept(player, "Silmek mi?").then(status => {
  ...
  block.newItem({
    name: "Bir uyarı yayınlayın",
    more: `Aktif: [${warns.length}/3]`,
    onpress: () => {
      if (user.getAdminLevel(player) <= 1 || adminLvl <= lvl) return player.notify(`~r~Bir uyarı yayınlayamazsınız`), workUser(player, id, name, lvl)
      if (warns.length >= 3) return player.notify(`~r~Uyarı sayısı 3'ten büyük veya 3'ü aşıyor`), workUser(player, id, name, lvl)
      menu.input(player, "Bir neden girin (en az 3 karakter)").then(res => {
        if (!res || res.length < 3) return workUser(player, id, name, lvl), player.notify("Sebep belirtilmemis");
        user.warn(id, res, "Yönetici " + user.getRpName(player), 7)
        player.notify("~g~Uyarı yayınlandı")
        user.loginterract(player, id, "AdminJob", "Bir oyuncuya uyarı verdi: " + res)
      })
    }
  })
  block.newItem({
    name: "Yasagı Kaldır",
    onpress: () => {
      menu.input(player, "Bir neden girin (en az 3 karakter)", '[Yasaklanmamıs] ').then(res => {
        if (!res || res.length < 3) return workUser(player, id, name, lvl), player.notify("Sebep belirtilmemis");
        user.unbanuser(id, player, res);
        user.loginterract(player, id, "AdminJob", "Bir oyuncu yasaklanmadı")
      })
  ...
  if (lvl == 0) block.newItem({
    name: "Hapse gir.",
    onpress: () => {
      if (lvl > 0) return workUser(player, id, name, lvl), player.notify("Bir yöneticiyi hapse atamazsınız");
      menu.input(player, "Bir neden girin (en az 3 karakter)").then(res => {
        if (!res || res.length < 3) return workUser(player, id, name, lvl), player.notify("Sebep belirtilmemis");
        menu.input(player, "Dakika sayısını girin").then(m => {
          let minute = methods.parseInt(m);
  ...
  block.newItem({
    name: "Cezaevinden tahliye",
    onpress: () => {
      user.arrestAdmin(id, player, 0, "Serbest bırakıldı")
      player.notify("Игрок освобождён");
      userList(player)
      user.loginterract(player, id, "AdminJob", "Hapisten cıktı")
    }
  })
  if (adminLvl > 5) {
    block.newItem({
      name: "Sunlara göre bir öge yayınlayın ID",
      onpress: () => {
        menu.input(player, "Urun kimligini girin").then(m => {
          let itemq = methods.parseInt(m);
          if (isNaN(itemq) || itemq < 0) return player.notify("~r~ID konu yanlıs"), workUser(player, id, name, lvl)
          if (items.getItemNameById(itemq) == "UNKNOWN") return player.notify(`~r~Böyle bir konu yok`), workUser(player, id, name, lvl)
          user.accept(player, "Sorun " + items.getItemNameById(itemq) + "?").then(status => {
            if (!status) return workUser(player, id, name, lvl);
            menu.input(player, "Numarasını girin", "1", 5, "int").then(count => {
              if (!count) return player.notify(`~r~Miktarlar dogru degil`), workUser(player, id, name, lvl)
              if (count > 50) return player.notify(`~r~Miktar maks. 50`), workUser(player, id, name, lvl)
              inventory.createManyItem(itemq, 0, 1, id, count)
              user.loginterract(player, id, "AdminJob", "Bir esya verdi " + items.getItemNameById(itemq) + " x" + count)
              workUser(player, id, name, lvl)
            })
          })
    ...
    block.newItem({
      name: "Silah sorunu",
      onpress: () => {
        menu.selector(player, "Silahınızı secin", weaponsUtil.hashesMap.map(([name, hashhalh]) => name), true).then(async ids => {
          ...
          if (!ammo || ammo < 1) return player.notify(`~r~Geçersiz mermi sayısı girildi.`), workUser(player, id, name, lvl);
          user.giveWeaponByHash(player, mp.joaat("WEAPON_" + weapon[0].toUpperCase()), ammo)
          player.notify('~g~Verilen silahlar');
          return workUser(player, id, name, lvl);
        });
      }
    })
    if (target) {
      block.newItem({
        name: "Mülk sahibinin",
        onpress: () => {
          let list = ["Ev", "Depo", "Duz", "Daire"];
          menu.selector(player, "Tip secin", list, true).then(ids => {
            if (ids == null) return;
            ...
            menu.input(player, "Girin ID").then(m => {
              let itemq = methods.parseInt(m);
              if (isNaN(itemq) || itemq <= 0) return player.notify("~r~ID dogru degil")
              itq.updateOwnerInfo(itemq, id, user.getRpName(target));
              user.set(target, itqs, itemq)
              user.updateClientCache(target)
              if (itqs == "id_house") user.loginterract(player, id, "AdminJob", "Bir ev sahibi yaptı @house" + itemq)
              else if (itqs == "apartment_id") user.loginterract(player, id, "AdminJob", "Aparatların sahibini yaptı @apartment" + itemq)
              else if (itqs == "condo_id") user.loginterract(player, id, "AdminJob", "Bir dairenin ev sahibi yapıldı @condo" + itemq)
              else if (itqs == "stock_id") user.loginterract(player, id, "AdminJob", "Depo sahibini @stock" + itemq)
      ...
      block.newItem({
        name: "Bir isletme sahibi olun",
        onpress: () => {
          menu.input(player, "İsletme kimligini girin").then(m => {
            let itemq = methods.parseInt(m);
            if (isNaN(itemq) || itemq <= 0) return player.notify("~r~ID dogru degil")
            ...
            user.loginterract(player, id, "AdminJob", "Bir isletme sahibi yaptı @business" + itemq)
            user.updateClientCache(target)
            player.notify("~g~Basarıyla");
          });
        }
  ...
  block.newItem({ name: "Geri", type: "select", onpress: () => userList(player) });
  block.open();
}

async function statUserEdit(player: PlayerMp, id: number, name: string, lvl: number) {
  let adminLvl = user.getAdminLevel(player)
  if (!user.isAdminNow(player)) return player.notify("Yalnızca yoneticilerin kullanabilecegi eylemler");
  let target = user.getPlayerById(id);
  let block = menu.new(player, `İstatistik editoru`, `${name} (${id}) ${target ? 'Online' : 'Offline'}`);
  let statData: any;
  if (!target) {
    let q = await userEntity.findOne({ where: { id: id } })
    if (!q) return player.notify("Yanlıs ID");
    statData = q;
  }
  const statList = [
    "age",
  ...
  statList.map(item => {
    ...
    block.newItem({
      name: `${item} (${value})`,
      onpress: () => {
        menu.input(player, "Yeni deger", value, 20, "int").then(async res => {
          if (res == null) return;
          ...
          methods.saveLog('adminJob', user.getId(player) + " parametreyi degiştirdi " + item + " => " + value + " на " + newitem + " " + id);
          user.loginterract(player, id, "AdminJob", "Parametre degiştirildi " + item + " => " + value + " на " + newitem)
          statUserEdit(player, id, name, lvl)
        })
      }
    })
  ...
  statListText.map(item => {
    ...
    block.newItem({
      name: `${item} (${value})`,
      onpress: () => {
        menu.input(player, "Yeni deger", value).then(async res => {
          let newitem = res;
          if (target) {
            user.set(target, item, newitem);
            user.saveAccount(target);
          ...
          methods.saveLog('adminJob', user.getId(player) + " parametreyi değistirdi " + item + " => " + value + " на " + newitem + " " + id);
          user.loginterract(player, id, "AdminJob", "Parametre degistirildi " + item + " => " + value + " на " + newitem)
          statUserEdit(player, id, name, lvl)
        })
      }
    })
...
async function userList(player: PlayerMp, offline = false, search = "") {
  menu.close(player)
  if (offline && !user.isAdminNow(player)) return player.notify("Yalnızca yoneticiler tarafından kullanılabilir");
  let block = menu.new(player, "Kullanıcılar");
  let isAdmin = user.isAdminNow(player);
  let id = methods.parseInt(search);
  block.newItem({ name: `~y~Kapat`, onpress: () => block.close() })
  if (isNaN(id) || id < 0) id = 0;
  block.newItem({
    name: `Bir oyuncu aranıyor ${search ? `(${search})` : ''}`,
    onpress: () => {
      menu.input(player, "Bir ad girin veya ID", search).then(res => {
        if (!res) res = ""
        userList(player, offline, res);
      })
    }
  })
  block.newItem({ name: `~g~Cevrimici oyuncular` })
  ...
  players.forEach(target => {
    ...
    block.newItem({
      name: (isAdmin ? `${name} | ` : '') + (user.getDating(player, targetid) ? user.getDating(player, targetid) : (myid == targetid ? "Sen" : "Yabancı")) + ` (${targetid})`,
      more: "Seviye: " + user.getLevel(target),
  ...
  if (isAdmin) {
    block.newItem({
      name: offline ? `~r~Oyuncular offline ~g~(Dahil)~w~ (500 max)` : `~r~Oyuncular offline (Devre dısı bırakıldı)`,
      onpress: () => {
...
mp.events.add("users:top", (player: PlayerMp) => {
  let m = menu.new(player, "En iyi oyuncular", "Kategoriler");
  m.newItem({
    name: "Топ 20 bir gunde aktif",
    onpress: () => {
      let submenu = menu.new(player, "Liste");
...
mp.events.add("setAge", (player: PlayerMp) => {
  if (user.get(player, "exp_age") == 0) return player.notify("~r~Bir yaş belirtemezsiniz");
  // if(user.getLevel(player) > 1) return player.notify("~r~Вы не можете указывать возраст, поскольку ваш уровень выше первого");

  menu.input(player, "Yası belirtin (16 - 90)", user.get(player, "age"), 3, "int").then(age => {
    if (user.get(player, "exp_age") == 0) return player.notify("~r~Bir yas belirtemezsiniz");
    // if(user.getLevel(player) > 1) return player.notify("~r~Вы не можете указывать возраст, поскольку ваш уровень выше первого");
    if (age == null) return player.notify("~r~İptal");
    age = methods.parseInt(age);
    if (isNaN(age)) return player.notify("~r~Yaş doğru değil");
    if (age < 16 || age > 90) return player.notify("~r~Yaş 16 ile 90 arasında olmalıdır");
    ...
    user.updateCharacterCloth(player);
    user.updateClientCache(player);
    player.notify("~g~Belirtilen yaş, karakterinizin şu anki seviyesi: " + user.getLevel(player))
...
const dropBag = (player: PlayerMp) => {
  ...
  user.log(player, "Grab", `Bir canta dolusu para dusurdu.: ${player.bankgrabmoney}$`);
  let existSum = true;
  const countSum = methods.parseInt(`${player.bankgrabmoney}`);
  player.bankgrabmoney = 0;
  user.checkComponents(player)
  let check = methods.createDynamicCheckpoint(new mp.Vector3(player.position.x, player.position.y, player.position.z - 1), "Tıklayın ~g~Е~s~ para çantasını almak için", target => {
    if (!user.isGos(target) && !user.isMafia(target) && !user.isGang(target)) return target.notify(`~r~Вам эта сумка не нужна`);
    if (!existSum) return target.notify(`~r~Canta coktan kaldırıldı`);
    ...
    setTimeout(() => {
      ...
      if (!existSum) return target.notify(`~r~Çanta çoktan kaldırıldı`);
      if (!target.bankgrabmoney) target.bankgrabmoney = 0;
      target.bankgrabmoney += countSum;
      player.notify(`~g~Bir çanta dolusu para kaldırdınız. $${methods.numberFormat(countSum)}`)
...
mp.events.add('playerDeath', (player: PlayerMp, reason: number, killer: PlayerMp) => {
  ...
  let reasonText = ``;
  if (killer) {
    reasonText += `Olduruldu ${user.getRpName(killer)} @user${user.getId(killer)} `
  }
  if (reason) {
...
setInterval(() => {
  ...
  if (used) {
    ...
    mp.players.forEach(player => {
      ...
      if (onlineUser.get(user.getId(player)) < 25) return player.notify(`~r~PayDay alamadınız çünkü 25 dakikadan az oynadınız`);
      if (user.isAfk(player)) return player.notify(`~r~Ayakta olduğunuz için PayDay almadınız AFK`);
      user.payDay(player)
    ...
    if (d.getHours() == 18) {
      ...
      methods.notifyToAll('~b~Cizim sonucları ~b~' + user.getVipStatusData("Bonus").name + ' 4 gun boyunca\r~g~Tebrikler! Kazananlar: ~s~' + winners.map(player => { return user.getRpName(player) }).join(', '));
    }
  }
}, 10000)
...
playerDataSQL.init().then(() => {
  setInterval(() => {
    ...
    if (newday) {
      ...
      res.map((itm, index) => {
        user.addMoneyOffline(itm.id, dayTopReward[index], 'Вознаграждение', `Gunun en aktif oyuncusu olarak bir bonus aldınız (${index + 1} yer)`)
      })
      playerDataSQL.clear();
    }
