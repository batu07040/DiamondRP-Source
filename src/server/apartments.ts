...
export let apartments = {
  ...
  sell: function (player:PlayerMp) {
      ...
      user.log(player, "PlayerSell", `Daireyi sattım ${hInfo.get('address')} @apartment${hInfo.get('id')} за $${nalog}`)
      setTimeout(function () {
  
          if (!user.isLogin(player))
              return;
  
          user.addHistory(player, 3, 'Daireyi sattım №' + hInfo.get('id') + '. Fiyat: $' + methods.numberFormat(nalog));
  ...
  buy: function (player:PlayerMp, id:number) {
      ...
      user.log(player, "PlayerBuy", `Bir daire satın aldım ${hInfo.get('address')} @apartment${hInfo.get('id')} за $${hInfo.get('price')}`)
      setTimeout(function () {
          if (!user.isLogin(player))
              return;
          user.addHistory(player, 3, 'Bir daire satın aldım №' + hInfo.get('id') + '. Цена: $' + methods.numberFormat(hInfo.get('price')));
          user.saveAccount(player);
