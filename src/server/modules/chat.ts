...
export const chat = {
    ...
    sendBCommand: function (player: PlayerMp, text: string) {
        if (user.isLogin(player)) {
            let _rpName = user.getRpName(player);
            mp.players.forEach((nplayer) => {
                if (nplayer.dist(player.position) < range && nplayer.dimension == player.dimension) {
                    let _dating = user.getDating(player, user.getId(nplayer));
                    nplayer.outputChatBox(
                        `[${chat.getTime()}] !{2196F3} ${_dating || player == nplayer ? _rpName : "Yabancı"} !{C2A2DA}(${user.getShowingIdString(nplayer, player)}) : !{FFFFFF}(( ${escape(text)} )) `
                    )
                }
            })
    ...
    sendTryCommand: function (player: PlayerMp, text: string) {
        if (user.isLogin(player)) {
            let _rpName = user.getRpName(player);
            let lucky = methods.getRandomInt(0, 2) == 0 ? 'Başarısız' : 'Başarılı'
            mp.players.forEach((nplayer) => {
                if (nplayer.dist(player.position) < range && nplayer.dimension == player.dimension) {
                    let _dating = user.getDating(player, user.getId(nplayer));
                    nplayer.outputChatBox(
                        `[${chat.getTime()}] !{C2A2DA} ${lucky
                        } ${_dating || player == nplayer ? _rpName : "Yabancı"} !{C2A2DA}(${user.getShowingIdString(nplayer, player)}) :  ${escape(text)}`
                    )
                }
            })
    ...
    sendDoCommand: function (player: PlayerMp, text: string) {
        if (user.isLogin(player)) {
            ...
            mp.players.forEach((nplayer) => {
                if (nplayer.dist(player.position) < range && nplayer.dimension == player.dimension) {
                    let _dating = user.getDating(player, user.getId(nplayer));
                    nplayer.outputChatBox(`[${chat.getTime()}] !{C2A2DA} (( ${escape(text)} )) ${_dating || player == nplayer ? _rpName : "Yabancı"} !{C2A2DA}(${user.getShowingIdString(nplayer, player)})`)
                }
            })
        ...
        if (user.isLogin(player)) {
            ...
            mp.players.forEach((nplayer) => {
                if (nplayer.dist(player.position) < range && nplayer.dimension == player.dimension) {
                    ...
                    nplayer.outputChatBox(
                        `[${chat.getTime()}] ${_dating || player == nplayer ? _rpName : "Yabancı"} !{C2A2DA}(${user.getShowingIdString(nplayer, player)}) : ${escape(text)}`
                    )
                }
            })
        ...
        if (user.isLogin(player)) {
            ...
            mp.players.forEach((nplayer) => {
                if (nplayer.dist(player.position) < range && nplayer.dimension == player.dimension) {
                    let _dating = user.getDating(player, user.getId(nplayer));
                    nplayer.outputChatBox(
                        `[${chat.getTime()}] ${_dating || player == nplayer ? _rpName : "Yabancı"} !{C2A2DA}(${user.getShowingIdString(nplayer, player)}) Zar Attı !{FF9800}(( Sayı: ${dice} ))`
                    )
        ...
        if (user.isLogin(player)) {
            ...
            mp.players.forEach((nplayer) => {
                if (nplayer.dist(player.position) < range && nplayer.dimension == player.dimension) {
                    ...
                    nplayer.outputChatBox(
                        `[${chat.getTime()}] !{2196F3}${_dating || player == nplayer ? _rpName : "Yabancı"} (${user.getShowingIdString(nplayer, player)}) : !{FFFFFF}${escape(text)}`
                    )
                }
            })
    ...
    sendGosRank: function (sender: PlayerMp, text: string, rank: number, color = '7FADED') {
        let fractionId = user.get(sender, "fraction_id");
        let rankName = user.getRankName(fractionId, rank);

        mp.players.forEach(nuser => {
            if (user.isGos(nuser)) nuser.outputChatBox(`[${chat.getTime()}] !{${color}} [GOS] ${rankName} ${user.getRpName(sender)} (${user.getFractionName(user.get(sender, 'fraction_id'))}):!{FFFFFF} ${escape(text)}`);
        })
    },
    getTime: function () {
...
mp.events.add('server:chat:sendMeCommand', function (player: PlayerMp, text: string) {
    ...
    if (mute) {
        ...
        let formattedTime = ('0' + date.getHours()).substr(-2) + ':' + ('0' + date.getMinutes()).substr(-2) + ` ${date.getDate()}/${date.getMonth() +
            1}/${date.getFullYear()}`;
        return player.outputChatBox("Yönetici tarafından yazılı sohbetten engellendiniz." + formattedTime)
    }
...
mp.events.add('playerCommand', (player: PlayerMp, command: string) => {
    ...
    if (commands[commandName]) commands[commandName](player, ...args);
        ...
        player.outputChatBox(
            `!{FFC107}Kullanılabillir komutların listesi /me /do /try /b`
        );
        if (user.isAdminNow(player)) player.outputChatBox(
            `!{FFC107}Yönetici komutlarını görebilmek için /ahelp `
        );
        if (user.isHelper(player)) player.outputChatBox(
            `!{FFC107}Yardımcılar(Helper) arasında iletişim için /h [TEXT]`
        );
        if (user.isGos(player) && user.get(player, 'rank') >= 7) player.outputChatBox(
            `!{FFC107}Devlet fraksiyonları arasında iletişim için  /g [TEXT]`
        );
    }
});
...
chat.registerCommand('g', (player, ...textarr) => {
    ...
    let rank = user.get(player, 'rank');
    if (rank) return;
    let text = chat.text(textarr);
    chat.sendGosRank(player, text, rank);
});
...
chat.registerCommand('ahelp', (player) => {
    ...
    player.outputChatBox('/ban [id] [reason] - Kullanıcı banlama.');
    // player.outputChatBox('/jail [id] [time] [reason] - Посадить в тюрьму');
    player.outputChatBox('/unban [id] - Kullanıcı ban kaldırma.');
    player.outputChatBox('/cmute [id] [zaman] [neden] - Oyuncuyu metin sohbetinden yasakla');
    player.outputChatBox('/vmute [id] [zaman] [neden] - Выдать мут игроку в минутах в голосовом чате');
    player.outputChatBox('/sp [id] - Bir oyuncuyu takip edin.');
    player.outputChatBox('/d [id] - Boyut değiştir.');
    player.outputChatBox('/admins - Sunucudaki aktif yöneticiller.');
    player.outputChatBox('/helpers - Sunucudaki aktif yardımcı yöneticiler.');
    player.outputChatBox('/a [TEXT] - Yöneticilere mesaj gönder.');
    player.outputChatBox('/pos - Bulunduğunuz yerin koordinatlarını görüntüleyin.');
    player.outputChatBox('/tp [id] | [x] [y] [z] - Oyuncuya ışınlanın.');
    player.outputChatBox('/tpm [id] - Oyuncuyu kendinize çekin.');
    player.outputChatBox('/tpveh [number] - Aracı kendinize çekin.');
