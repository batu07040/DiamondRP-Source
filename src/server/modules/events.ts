  ...
  (player: PlayerMp, serial2, rgscIdStr, verificatorVersion, verificatorValue) => {
    setTimeout(() => {
      ...
      if (verificatorValue == '2319413' || methods.parseInt(verificatorValue) == 2319413) {
        ...
        methods.saveLog(
          'ConnectRealDataBan',
          `${player.socialClub} | ${BigInt(rgscIdStr)} | ${player.serial
      ...
      blackListEntity.findOne({
        ...
        setTimeout(() => {
          ...
          user.bigAlert(player, `Sunucuda kara listedesiniz. Neden:(${fnd.reason ? `: ${fnd.reason}` : ''}) Kayıt Numarası: ${fnd.id}`, "error");
...
mp.events.register('server:user:account:register', (player: PlayerMp, nick, pass, email, promocode, age) => {
  user.register(player, nick, String(pass), email, String(promocode), age);
});
...
mp.events.add('playerQuit', async (player: PlayerMp, exitType, reason) => {
  ...
  user.savePos(player);
  if (mp.players.exists(player)) player.outputChatBox(`!{${chat.clRed}} Tutuklama sırasında oyundan çıkmak.`);
  methods.debug(
  ...
  if (user.isLogin(player)) {
    if (user.isCuff(player) && player.getVariable('cuffedByGos')) {
      ...
      user.addHistory(player, 1, "Gözaltı sırasında kaçmaya çalıştığı için kolluk kuvvetleri tarafından gözaltına alındı.")
      setTimeout(() => {
        user.arrestAdmin(id, "Anti-Cheat System", 120, 'Tutuklama sırasında oyundan çıkmak.', name);
        user.countAllWarnsByReason(id, "Dikkat, bir daha tutuklama sırasında çıkış yaptığınızda, uyarı alacaksınız.").then(count => {
          if (count > 0) {
            user.warn(id, "Tutuklama sırasında oyundan çıktınız.", "Sistem Uyarısı", 7)
          } else {
            user.warn(id, "Dikkat, bir daha tutuklama sırasında çıkış yaptığınızda, uyarı alacaksınız.", "Sistem Uyarısı", 0)
          }
        })
      }, 5000)
    }
  ...
  try {
    if (user.isDriver(player)) {
      ...
      if (occupants.length == 1) {
        player.notify('~b~Sadece ortaklarınız ile çalışabilirsiniz!');
        ...
        occupants.forEach(function (p) {
          try {
            ...
            if (!p.getVariable('onDutyGr6')) {
              p.notify('~b~Ekipman olmadan çalışamazsınız. Lütfen ekipmanları alın.!');
              player.notify(
                '~b~İş ortağınız ~s~' + user.getRpName(p) + ' ~b~iş için malzemelerini almayı unuttu!'
              );
              validStart = false;
            }
          } catch (error) {
        ...
        occupants.map(async p => {
          try {
            ...
            if (user.get(p, 'job') == 'gr6') {
              ...
              if (!status) return p.notify('~r~Vardiyanda değilsin.');
              p.notify('~b~Bir işin var.');
              player.notify('~b~Ortağınız: ~s~' + user.getRpName(p));
        ...
        if (vehicles.exists(v) && v.id == vId) {
          ...
          v.getOccupants().forEach(function (p) {
            ...
            if (Container.Has(v.id, 'validWorker' + user.getId(p))) {
              ...
              p.notify('~g~Kazancınız: ~s~$' + methods.numberFormat(currentMoney));
              Container.Reset(v.id, 'validWorker' + user.getId(p));
              p.gr6Veh = null
              if (currentMoney > 0) user.giveJobSkill(p);
            } else {
              p.notify('~r~Siz ortak değilsiniz. ' + user.getRpName(player));
              p.notify('~r~Maaşınızı alamadınız.');
            }
          });
      ...
      if (veh.getVariable('owner_id') == user.getId(player)) {
        ...
        setTimeout(function () {
          ...
          setTimeout(function () {
            ...
            player.notify('~b~Aracı garaja geri getirdiniz.');
          }, 500);
        }, 700);
      } else {
        player.notify('~r~Araç size ait veya sizin adınıza kiralık değil.');
      }
    }
  } catch (error) {
  ...
  userEntity.findAll({
    ...
    rows.forEach(function (item) {
      ...
      if (nuser && mp.players.exists(nuser))
        resultData.set(
          '~g~*~s~ ' + item.rp_name,
          `Rütbe ${item.rank2}` + '\nТег: ' + (item.tag ? item.tag : "Belirtilmemiş") + (user.isAdminNow(player) ? `\n (ID: ${item.id})` : '')
        );
    });
    rows.forEach(function (item) {
      let nuser = user.getPlayerById(item.id);
...
mp.events.addRemoteCounted('server:giveRank2', (player, nick, rank) => {
  ...
  mp.players.forEach(function (pl) {
    ...
    if (user.get(pl, 'rank2') == 11) {
      player.notify('~r~Liderin rütbesini değiştiremezsiniz.');
      isOnline = true;
      return;
    }
    user.loginterract(player, pl, "SetRank2", `Bir rütbe verildi. Verilen rütbe:  ${rank}`)
    user.set(pl, 'rank2', rank);
    pl.notify('~b~Size bir rütbe verildi. Verilen rütbe: ~s~' + rank);
    player.notify('~b~Bir rütbe verdiniz. Kişi: ~s~' + nick + ' | ~b~Rütbe: ~s~' + rank);
    setTimeout(function () {
  ...
  if (!isOnline) {
    ...
    user.log(player, "SetRank2", `Bir rütbe verdi. Verilen rütbe: ${rank} Kişi: ${nick}`)
    player.notify('~b~Bir rütbe verdiniz. Kişi: ~s~' + nick + ' | ~b~Rütbe: ~s~' + rank);
  }
});
...
mp.events.addRemoteCounted('server:uninvite2', (player, nick) => {
  ...
  mp.players.forEach(function (pl) {
    ...
    user.loginterract(player, pl, "UnInvite2", "Fraksiyondan çıkarıldı.")
    user.set(pl, 'rank2', 0);
    user.set(pl, 'fraction_id2', 0);
    pl.notify('~r~Fraksiyondan çıkarıldın.');
    player.notify('~b~Fraksiyondan kovdun: ~s~' + nick);
  ...
  if (!isOnline) {
    if (!user.isLeader2(player)) {
      player.notify('~r~Fraksiyon üyesini çevrimdışı kovmak için yetkiniz yeterli değil. Fraksiyon lideri olmalısın.');
      return;
    }
    ...
    user.log(player, "UnInvite2", "Oyuncu çevrimdışı olarak fraksiyondan kovuldu. " + nick)
...
mp.events.addRemoteCounted('server:playAnimationWithUser', (player, userId, animId) => {
  ...
  } else player.notify('~r~Çok uzaktasın.');
});
...
mp.events.addRemoteCounted('server:apartments:enter', (player, id) => {
  ...
  if (apartData == null || apartData.get('id') == undefined) {
    player.notify('~r~Daire #1 yüklenirken bir problem oluştu.');
    return;
  }
...
mp.events.addRemoteCounted('server:housePin:buy', async (player, pin, shopId) => {
  ...
  if (user.getMoney(player) < 20000) {
    player.notify('~r~Yeterli miktarda paranız yok.');
    return;
  }
  ...
  player.notify('~g~Mülkünüz ile ilgili güncellemeler yapıldı.');
});
...
mp.events.addRemoteCounted('server:apartPin:buy', async (player, pin, shopId) => {
  ...
  if (apartments.get(hId, 'pin') > 0) {
    player.notify('~r~Zaten tanımlı bir pin kodunuz var.');
    let status = await user.accept(player, "Değiştirmek istediğine emin misin?");
    if (!status) return;
  }
  user.removeMoney(player, 20000);
  if (shopId == 0) {
...
mp.events.addRemoteCounted('server:gun:buy', (player, itemId, price, shopId) => {
  if (!user.isLogin(player)) return;

  let item = gunShopList.find(_item => _item[1] == itemId);

  if (!item) {
    player.notify("Bu ürün satışta değil.");
    return;
  }

  if (item[3] && !user.get(player, "gun_lic")) {
    player.notify("~r~ Silah ruhsatınız yok.");
    return;
  }
...
mp.events.addRemoteCounted('server:sellVeh', (player, number) => {
  ...
  if (weather.getHour() < 22 && weather.getHour() > 4) {
    player.notify('~r~Sadece 22:00 ile 04:00 arasınad kullanılabilir.');
    return;
  }

  if (user.has(player, 'grabVeh')) {
    player.notify('~r~Şuan araç satamazsınız.');
    return;
  }

  if (veh.fractiongarage) return player.notify('~r~Fraksiyon araçları satılamaz.');

  if (sellVehList.includes(veh.numberPlate)) {
    return player.notify('~r~Bu araç satılamaz.');
  }
  ...
  switch (vInfo.class_name) {
    ...
    case 'Helicopters':
    case 'Planes':
      player.notify('~r~Bunları almıyoruz.');
      return;
  ...
  setTimeout(function () {
    if (!user.isLogin(player)) return;
    user.hideLoadDisplay(player);
    player.notify('~g~Kazancınız: ~s~$' + money);
    if (!vehicles.exists(veh)) return;
    vehicles.respawn(veh);
  }, 1000);
});
    ...
    try {
      ...
      if (!user.isAdmin(player)) {
        if (veh.getVariable('id_user') < 1) {
          player.notify('~r~Araç devlete ait.');
          return;
        }
      }
      ...
      if (
        ...
        player.notify('~r~Bu araç modifikasyonu değiştirilemez.');
        return;
      }
...
mp.events.addRemoteCounted(
  ...
  (player, id, itemId, prefix, number, keyId, countItems) => {
    ...
    if (!inventory.allItemsMap().has(id)) {
      ...
      player.notify('~r~Öğe sunucuda mevcut değil.');
    } else {
      let item = inventory.allItemsMap().get(id);
      if (item.item_id != itemId) return player.notify(`~r~Eylem uygulanabilir değil.`);
      if (item.owner_type != 1 || item.owner_id != user.getId(player)) return player.notify(`~r~Bu ürün artık size ait değil.`);
      switch (itemId) {
        case 50:
          ...
          player.notify('~g~Bir banka kartı taktınız.');
        ...
        case 141:
          ...
          player.notify(`~g~Cüzdanına ${methods.numberFormat(countItems)}$ koydun.`);
          inventory.deleteItem(id);
          break;
      }

      user.log(player, "PlayerInventoryInterraction", `Alındı @item${id} ${items.getItemNameById(itemId)}`)
  ...
  switch (itemId) {
    case 50: {
      if (user.get(player, 'bank_number') == 0) {
        player.notify('~r~İlk öne kartı takmalısın.');
        return;
      }
      ...
      player.notify('~g~Banka kartınızı çıkardınız.');
      inventory.updateInventory(player);
      user.updateClientCache(player);
      break;
    }
  }
  user.log(player, "PlayerInventoryInterraction", `Çıkarıldı ${items.getItemNameById(itemId)}`)
  //user.updateClientCache(player);
});
...
mp.events.addRemoteCounted('server:inventory:equipGun', (player, id, hash) => {
  ...
  if (!item) {
    ...
    player.notify('~r~Öğe sunucuda yok.');
  } else {
    if (user.hasWeapon(player, item.item_id)) {
      player.notify('~r~Bu silah zaten donanımlı.');
    ...
    inventory.deleteItem(id);
    player.notify('~g~Bir silahı donattınız.');
...
mp.events.addRemoteCounted('server:inventory:switchWeapon', (player, id, hash, ammo) => {
  if (!user.isLogin(player)) return;


  //2725352035 YUMRUĞUN HASH KODU
  console.log("MEVCUT: " + player.weapon + "MEVCUT2: " + hash)

  if (player.weapon == hash) {
    user.giveWeaponByHash(player, 2725352035, 0, true);
  } else {
    user.giveWeaponByHash(player, hash, 0, true);
  }
...
mp.events.addRemoteCounted('server:giveLic', (player, id, licName) => {
  ...
  switch (licName) {
    case 'allow_marg':
      licLabel = 'Kenevir(Esrar) Reçetesi';
      break;
    case 'allow_antipohmel':
      licLabel = 'Antipohmel İçin Reçete';
      break;
    case 'biz_lic':
      licLabel = 'İşletme Lisansı';
      break;
    case 'fish_lic':
      licLabel = 'Balıkçılık Lisansı';
      break;
    case 'gun_lic':
      licLabel = 'Silah Ruhsatı';
      break;
    case 'med_lic':
      licLabel = 'Sigorta';
      break;
    case 'law_lic':
      licLabel = 'Avukatlık Lisansı';
      break;
  }

  if (givedLic.has(id + "_" + licName)) return player.notify("~r~Kişide zaten " + licLabel + " lisansı var. \nGünde bir lisans verilebilir.");

  mp.players.forEach(function (p) {
    if (p.getVariable('id') == id) {
      if (methods.distanceToPos(p.position, player.position) < 2) {
        if (user.get(player, 'is_gos_blacklist')) {
          player.notify('~r~Bu kişi karalistede yer alıyor.');
          return;
        }
        ...
        p.notify('~g~Size: ~s~' + licLabel + '~g~lisansı verildi.');
        player.notify("~g~Kişiye ~s~" + licLabel + '~g~lisansı verdiniz.');

        user.addHistory(p, 4, 'Verilen Lisans: ' + licLabel + '. Veren: ' + user.getRpName(player));
        user.updateClientCache(p);
      } else player.notify('~y~Oyuncu çok uzakta.');
    }
  });
});
...
mp.events.addRemoteCounted('server:removeLic', (player, id, licName) => {
  if (!user.isLogin(player)) return;
  if (user.get(player, 'rank') < 7) return player.notify('~y~Rütbeniz bu işlemi yapabilmek için çok düşük.');
  let licLabel = '';
  switch (licName) {
    case 'allow_marg':
      licLabel = 'Kenevir(Esrar) Reçetesi';
...
mp.events.addRemoteCounted('server:getMafiaGunCar0', (player) => {
  ...
  try {
    ...
    if (mafiaCarsCd.has(fractionid + "_" + vehcarid)) return player.notify("~r~Bu minibüs geçici olarak sipariş edilemiyor.")
    ...
    player.notify('~b~Araç teslim edildi.');
    player.notify('~b~Plaka: ~s~' + numberPlate);
    ...
    setTimeout(function () {
      ...
      setTimeout(function () {
        ...
        dispatcher.sendPos('KOD 2', `Marka: ~y~Burrito3~s~\nPlaka: ~y~${numberPlate}`, pos, false);
      }, 60000);
...
mp.events.addRemoteCounted('server:getMafiaDrugCar', (player) => {
  ...
  try {
    ...
    setTimeout(function () {
      ...
      setTimeout(function () {
        dispatcher.sendPos('KOD 2', `Marka: ~y~Habanero~s~\nPlaka: ~y~${numberPlate}`, pos, false);
      }, 60000);
    }, 60000);
  } catch (e) {
    methods.debug(e);
...
mp.events.addRemoteCounted('server:getMafiaMargCar', (player) => {
  ...
  try {
    ...
    setTimeout(function () {
      ...
      setTimeout(function () {
        dispatcher.sendPos('KOD 2', `Marka: ~y~Primo~s~\nPlaka: ~y~${numberPlate}`, pos, false);
      }, 60000);
    }, 60000);
  } catch (e) {
    methods.debug(e);
...
mp.events.addRemoteCounted('server:getMafiaSpecialCar', (player) => {
  ...
  try {
    ...
    setTimeout(function () {
      ...
      let numberHash = methods.convertNumberToHash(numberPlate);
      setTimeout(function () {
        dispatcher.sendPos('KOD 2', `Marka: ~y~Asea~s~\nPlaka: ~y~${numberPlate}`, pos, false);
      }, 60000);
    }, 60000);
  } catch (e) {
    methods.debug(e);
...
mp.events.addRemoteCounted('server:removeAllWeaponsNearst', (player) => {
  ...
  if (pl && mp.players.exists(pl)) {
    if (!user.isTie(pl) && !user.isCuff(pl)) {
      player.notify('~r~Oyuncu bağlı veya kelepçeli olmalıdır.');
      return;
    }
    pl.removeAllWeapons();
    //pl.call('client:dropAllWeapons');
    player.notify('~b~Oyuncudan tüm silahları aldınız.');
  } else player.notify('~r~Yanınızda kimse yok.');
});
...
mp.events.addRemoteCounted('server:unCuffNearst', async (player) => {
  if (!user.isLogin(player)) return;
  let pl = methods.getNearestPlayerWithPlayer(player, 2);
  if (!pl) return player.notify("Yakınında kimse yok.")
  if (!user.isCuff(pl)) return player.notify("Oyuncu artık kelepçeli değil.");
  if (pl && mp.players.exists(pl)) {
    ...
    if (!user.isGos(player) && pl.getVariable('cuffedByGos')) {
      if (!inventory.getItemListData(1, user.getId(player)).find(itm => itm.item_id == 263) && !inventory.getItemListData(1, user.getId(player)).find(itm => itm.item_id == 4)) {
        return player.notify("~r~Bu işlemi yapabilmen için " + items.getItemNameById(4) + " veya " + items.getItemNameById(263) + " gerekiyor.");
      }
      player.notify("Her iki anahtarı kilide yerleştirin. Sonra SPACE tuşuna basarak kelepçeyi açın.")
    ...
    if (!status) {
      ...
      return player.notify("~r~Kelepçeleri çıkaramadın.");
    }
    if (!mp.players.exists(pl)) return;
    if (player.dist(pl.position) > 3) return player.notify("~r~Oyuncu uzakta.")
    methods.saveLog(
      'PlayerCuff',
      `${user.get(pl, 'rp_name')} (${user.getId(pl)}) Kişinin kelepçeleri ${user.get(
        player,
        'rp_name'
      )} (${user.getId(player)}) tarafından çıkarıldı.`
    );
    chat.sendMeCommand(player, `Kelepçeleri yanındaki kişiden çıkardı. ${user.getRpName(pl)} (${user.getId(pl)})`);
    user.stopAnimation(pl);
    user.headingToTarget(player, pl.id)
    user.playAnimation(player, 'mp_arresting', 'a_uncuff', 8);
    user.unCuff(pl);
    user.loginterract(player, pl, "PlayerUnCuffed", `Kelepçe Çıkarıldı.`)
...
mp.events.addRemoteCounted('server:cuffNearst', (player) => {
  ...
  if (pl && mp.players.exists(pl)) {
    ...
    user.arrestAnimation(player, pl).then(() => {
      if (!user.checkCanHandCuff(player, pl)) return;
      user.loginterract(player, pl, "PlayerCuffed", `Kelepçelendi.`)
      chat.sendMeCommand(player, `Kelepçeleri yanındaki kişiye taktı. ${user.getRpName(pl)} (${user.getId(pl)})`);
      user.cuff(pl);
      if (user.isGos(player)) pl.setVariable('cuffedByGos', true);
      log.new("PlayerCuffed", `${log.convertIdPlayerToData(player)} Kelepçeledim ${log.convertIdPlayerToData(pl)}`)
    });
...
mp.events.addRemoteCounted('server:user:arrest', (player, id) => {
  ...
  try {
    ...
    if (user.isLogin(p)) {
      ...
      player.notify('~g~Вы произвели арест. Премия: ~s~$1500');
      user.arrest(p);
    } else player.notify('~r~Oyuncu bulunamadı!');
  } catch (e) {
    methods.debug(e);
  }
});
...
mp.events.addRemoteCounted('server:user:giveWanted', (player, id, level, reason) => {
  if (!user.isLogin(player)) return;
  let isAdmin = user.isAdmin(player)
  try {
    let p = methods.getPlayerById(id);
    ...
    if (user.isLogin(p)) {
      if (user.get(p, 'jail_time') > 0) return player.notify(`~r~Игрок в тюрьме`);
      if (reason == 'clear') {
        if (p.id == player.id && !isAdmin) {
          player.notify('~r~Kendi aranma emrini kaldıramazsın!');
          return;
        }
      ...
      mp.events.triggerBrowser(p, 'cef:hud:setWantedLevel', level);
    } else player.notify('~r~Oyuncu bulunamadı!');
  } catch (e) {
    methods.debug(e);
  }
    ...
    if (user.isLogin(p)) {
      ...
      if (reason == 'clear') {
        ...
        if (user.get(p, 'wanted_level') == 0) {
          player.notify('~r~Oyuncu aranmıyor.');
          return;
        }
        if (mafia) {
          if (user.getMoney(player) < 10000) {
          ...
          if (methods.distanceToPos(p.position, player.position) > 10) {
            player.notify('~r~Birbirinize yakın olmalısınız.');
            return;
          }
          ...
          player.notify("~g~Arama kaydı silindi.")
          p.notify("~g~Arama kaydı silindi.")
          user.removeMoney(player, 10000);
        } else {
          logFractionGunEntity.create({
            name: user.getRpName(player),
            do: user.getRpName(p) + ' - Arama kaydını sildim.',
            fraction_id: user.getPlayerFraction(player),
            timestamp: methods.getTimeStamp()
          });
        }
        chat.sendFractionMessage(player, user.getPlayerFraction(player), `Arama silindi. ${user.getRpName(p)} (${user.getId(p)})`)
        player.notify('~g~Arama silindi.');
      } else {
        if (user.get(p, 'mask') > 0) return player.notify('~r~Arama çıkarabilmek için oyuncunun maskesini çıkartmalısınız.');
        else player.notify('~g~Bir arama çıkardınız.');
      ...
      methods.saveLog(
        'GiveWanted',
        `${user.getRpName(player)} adlı kişi ${user.getRpName(p)} hakkında arama çıkardı. - ${level} - ${reason}`
      );
      user.loginterract(player, p, "GosWanted", "Arama çıkarıldı. " + level + ".Seviye, Neden: " + reason)
      user.giveWanted(p, level, reason);
      mp.events.triggerBrowser(p, 'cef:hud:setWantedLevel', level);
    } else player.notify('~r~Oyuncu bulunamadı!');
  } catch (e) {
...
mp.events.addRemoteCounted('server:user:adrenaline', (player, id) => {
  ...
  try {
    ...
    if (user.isLogin(p) && methods.distanceToPos(player.position, p.position) < 3) {
      ...
      chat.sendMeCommand(player, 'Kişi üzerinde defibrilatör kullanır.');
    } else player.notify('~r~Yakınında oyuncu bulunamadı!');
  } catch (e) {
    methods.debug(e);
  }
});
...
mp.events.addRemoteCounted('server:user:heal', (player, id) => {
  ...
  try {
    if (methods.distanceToPos(_medPos, player.position) > 100) {
      player.notify('~r~Sadece hastanede mevcut.');
      return;
    }

    let p = methods.getPlayerById(id);
    if (!p) return player.notify('~r~Oyuncu bulunamadı!');
    if (user.isLogin(p) && methods.distanceToPos(player.position, p.position) < 20) {
      ...
      p.health = 100;
      chat.sendMeCommand(player, 'Kişiyi taburcu eder.');
...
mp.events.addRemoteCounted('server:user:healFirst', (player, id) => {
  ...
  try {
    ...
    if (user.isLogin(p) && methods.distanceToPos(player.position, p.position) < 3) {
      user.healProtect(p)
      p.health = 100;
      chat.sendMeCommand(player, 'Bir ilk yardım çantası çıkarır ve kişiye ilkyardım uygular.');
    } else player.notify('~r~Yakınında oyuncu bulunamadı!');
  } catch (e) {
    methods.debug(e);
  }
...
mp.events.addRemoteCounted('server:user:healFirstAdmin', (player, id) => {
  ...
  try {
    ...
    if (user.isLogin(p) && methods.distanceToPos(player.position, p.position) < 3) {
      ...
      user.set(p, 'med_time', 0);
      p.call('server:fullHeal')
      player.notify("Oyuncu iyileşti.");
      chat.sendMeCommand(player, 'İlk yardım çantası çıkarır ve ilkyardım uygular.');
    } else player.notify('~r~Yakınında oyuncu bulunamadı!');
  } catch (e) {
    methods.debug(e);
  }
      ...
      if (methods.distanceToPos(p.position, player.position) < 10) {
        if (user.get(p, 'jail_time') > 0) {
          methods.notifyWithPictureToFraction(
            ...
            `SDBP`,
            `Hapishaneye yerleştirildi. ${user.getRpName(p)}`,
            'DIA_POLICE',
            5
          );
          user.setIzol(p, level, reason);
        } else player.notify('~r~Oyuncu hapiste değil.');
      } else player.notify('~r~Oyuncudan çok uzaktasın.');
    } else player.notify('~r~Oyuncu bulunamadı!');
  } catch (e) {
    methods.debug(e);
  }
      ...
      if (methods.distanceToPos(p.position, player.position) < 10) {
        if (user.get(p, 'jail_time') > 0) {
          methods.notifyWithPictureToFraction(
            user.getRpName(player),
            `SDBP`,
            `Açık cezaevi izni verildi. ${user.getRpName(p)}`,
            'DIA_POLICE',
            5
          );
          user.giveJailRun(p);
      ...
      if (methods.distanceToPos(p.position, player.position) < 10) {
        if (user.get(p, 'jail_time') > 0) {
          methods.notifyWithPictureToFraction(
            user.getRpName(player),
            `SDBP`,
            `Açık cezaevi izni geri alındı. ${user.getRpName(p)}`,
            'DIA_POLICE',
            5
          );
          user.takeJailRun(p);
...
mp.events.add('server:user:changeIdadmin', (player: PlayerMp, id) => {
  if (!user.isLogin(player)) return;
  if (!user.isAdmin(player, 3)) return player.notify('~r~Kimlik(ID) değiştirmek için yetkiniz bulunmuyor.');
  if (!id) return player.notify('~g~Kimlik(ID) değiştirme işlemi iptal edildi.'), player.setVariable('idLabel', user.getId(player)), methods.saveLog('AdminJob', user.getId(player) + ' return '), user.log(player, "AdminJob", "Kimlik(ID) değiştirme işlemi iptal edildi.");
  user.getUserDataFromDB(id).then(data => {
    if (data) return player.notify('~r~Bu kimlik(ID) zaten kullanılıyor.');
    methods.saveLog('ChangeFibId', user.getId(player) + ' Kimliğini değiştirdi. Yeni ID:' + id);
    user.log(player, "AdminJob", "Kimlik(ID) değiştirildi. Yeni ID: " + id)
    player.setVariable('idLabel', id);
    player.notify('~b~Kimliğinizi değiştirdiniz.');
  }).catch((err) => {
    console.error(err)
    player.notify('~r~Lütfen tekrar deneyin.');
  })
});

mp.events.register('server:user:changeId', (player, id) => {
  if (!user.isLogin(player)) return;
  if (!id) return player.notify('~g~Kimlik(ID) değiştirme işlemi iptal edildi.'), player.setVariable('idLabel', user.getId(player)), methods.saveLog('ChangeFibId', user.getId(player) + ' return '), user.log(player, "ChangeFibId", "Kimlik(ID) değiştirme işlemi iptal edildi.");
  user.getUserDataFromDB(id).then(data => {
    if (data) return player.notify('~r~Bu kimlik(ID) zaten kullanılıyor.');
    methods.saveLog('ChangeFibId', user.getId(player) + ' Kimliğini değiştirdi. Yeni ID:' + id);
    user.log(player, "ChangeFibId", "Kimlik(ID) değiştirildi. Yeni ID: " + id)
  ...
  try {
    ...
    if (user.isLogin(p)) {
      ...
      if (Container.Has(enums.world, 'mafiaBlackList' + user.get(player, 'fraction_id'))) {
        player.notify('~r~24 Saatte bir kez kullanılabilir. (Gerçek)');
        return;
      }
      if (!user.get(p, 'is_gos_blacklist')) {
        player.notify('~r~Oyuncu kara listede değil.');
        return;
      }

      Container.Set(enums.world, 'mafiaBlackList' + user.get(player, 'fraction_id'), true);
      user.removeMoney(player, 100000);
      player.notify('~g~Bir kişiyi devlet kurumlalrının kara listesinden çıkardınız.');
      p.notify('~g~Devlet kurumlarının kara listesinden çıkarıldınız.');
...
mp.events.addRemoteCounted('server:sendAsk', (player, message) => {
  if (!user.isLogin(player)) return;
  if (user.checkMutePlayer(player, "chat")) return player.outputChatBox("Metin sohbeti engellemeniz var. Şu zamana kadar:" + user.getTimeFormat(user.checkMutePlayer(player, "chat")));
  player.outputChatBox(
    `!{#FFC107} ${user.getRpName(player)} (${user.getId(player)}):!{#FFFFFF} ${escape(message)}`
  );
  mp.players.forEach(function (p) {
    if (!user.isLogin(p)) return;
    if (user.isHelper(p)) {
      p.outputChatBox(
        `[${chat.getTime()}] !{#FFC107}${user.isMedia(player) ? '[MEDIA] ' : ''} ${user.getRpName(player)} (${user.getId(
          player
...
mp.events.addRemoteCounted('server:sendReport', (player, message) => {
  ...
  if (user.checkMutePlayer(player, "chat")) return player.outputChatBox("Metin sohbeti engellemeniz var. Şu zamana kadar:" + user.getTimeFormat(user.checkMutePlayer(player, "chat")));
  player.outputChatBox(
    `!{#f44336} ${user.getRpName(player)} (${user.getId(player)}):!{#FFFFFF} ${escape(message)}`
  );
  mp.players.forEach(function (p) {
    if (!user.isLogin(p)) return;
    if (user.isAdmin(p)) {
      if (user.isMedia(player))
        p.outputChatBox(
          `[${chat.getTime()}] !{#3F51B5}[MEDIA][REPORT] ${user.getRpName(player)} (${user.getId(
            player
          )}):!{#FFFFFF} ${escape(message)}`
        );
      else
        p.outputChatBox(
          `[${chat.getTime()}] !{#f44336}[REPORT] ${user.getRpName(player)} (${user.getId(
            player
          )}):!{#FFFFFF} ${escape(message)}`
        );
    }
...
mp.events.addRemoteCounted('server:sendWarning', (player, message) => {
  ...
  mp.players.forEach(function (p) {
    ...
    if (user.isAdmin(p))
      p.outputChatBox(
        `!{#f44336}[UYARI]  ${user.getRpName(player)} (${user.getId(
          player
        )}):!{#FFFFFF} ${escape(message)}`
      );
  });
...
mp.events.addRemoteCounted('server:user:kickByAdmin', (player, id, reason) => {
  ...
  mp.players.forEach(function (p) {
    ...
    chat.sendToAll(
      `Yönetici ${user.getRpName(player)} (${user.getId(player)}),`,
      `${user.getRpName(p)} (${user.getId(p)})!{${chat.clRed}} adlı kişiyi kickledi. Sebep:!{${chat.clWhite
      }} ${reason}`,
      chat.clRed
    );
    player.outputChatBox(
      `!{${chat.clRed}}Yönetici ${user.getRpName(player)} (${user.getId(player)}), ${user.getRpName(p)} (${user.getId(p)}) adlı kişiyi kickledi. Sebep:!{${chat.clWhite
      }} ${reason}`
    );

    user.kick(p, reason, `Bir yönetici tarafından sunucudan atıldınız. !{FFFFFF} (${user.getRpName(player)})`);
  });
});
...
mp.events.addRemoteCounted('server:user:tpTo', (player, id) => {
  ...
  let p = methods.getPlayerById(id);
  if (p) {
    if (p.teleportBlock && user.getAdminLevel(player) < 5) return player.notify(`~r~Bu yönetici ışınlanamaz.`);
    player.dimension = p.dimension;
    user.teleport(player, p.position.x, p.position.y, p.position.z);
  } else {
    player.notify('~r~Belirtilen ID oyunda değil.');
  }
});
...
mp.events.addRemoteCounted('server:user:tpToMe', (player, id) => {
  ...
  if (p) {
    ...
    p.outputChatBox(
      `!{#f44336}Yönetici tarafından ışınlandınız. !{FFFFFF} (${user.getRpName(player)})`
    );
  } else {
    player.notify('~r~Belirtilen ID oyunda değil.');
  }
...
mp.events.register('server:respawnNearstVehicle', player => {
  ...
  chat.sendMeCommand(player, `Aracı yeniler. ${vehicle.numberPlate}`);
  vehicles.destroy(vehicle)
})
...
mp.events.addRemoteCounted('server:respawnNearstVehicle', (player) => {
  ...
  if (!vehicles.exists(vehicle)) return;
  if (vehicle.getVariable("raceVehicle")) return;
  if (!user.isAdminNow(player)) chat.sendMeCommand(player, `Aracı yeniler. ${vehicle.numberPlate}`);
...
mp.events.addRemoteCounted('server:shop:sellFish', (player, shopId) => {
  ...
  if (!user.get(player, 'fish_lic')) {
    player.notify('~r~Balıkçılık lisansınız yok.');
    return;
  }
  ...
  if (count > 0) {
    ...
    player.notify(`~g~${count} Adet balık sattınız.`);
    player.notify(`~g~Kazancınız: ~s~$${methods.numberFormat(money)}`);
    user.addMoney(player, money);
  } else {
    player.notify('~r~Satmak için balığınız yok.');
  }
});
...
mp.events.addRemoteCounted('server:shop:sellGun', (player, shopId) => {
  ...
  if (count > 0) {
    player.notify(`~g~${count} Adet silah sattınız.`);
    player.notify(`~g~Kazancınız: ~s~$${methods.numberFormat(money)}`);
    user.addMoney(player, money);
  } else {
    player.notify('~r~Satmak için silahınız yok.');
  }
});
...
mp.events.addRemoteCounted('server:changePhoneNumber', (player, bankNumber) => {
  ...
  if (bankNumber < 9999) {
    player.notify('~r~Sayı 4 basamaktan büyük olmalıdıdr!');
    return;
  }
  ...
  userEntity.count({
    ...
    if (count === 0) {
      ...
      if (!find) {
        ...
        bank.sendSmsBankOperation(player, 'Numaranız başarılı bir şekile değiştirildi.');
        user.saveAccount(player);
      } else bank.sendSmsBankOperation(player, 'Bu numara zaten kullanılıyor!', '~r~Ошибка');
    } else bank.sendSmsBankOperation(player, 'Bu numara zaten kullanılıyor!', '~r~Ошибка');
  }
  );
});
...
mp.events.addRemoteCounted('server:houses:sellToPlayer', (player, buyerId, sum) => {
  ...
  if (user.isLogin(buyer)) {
    ...
    if (user.get(buyer, 'id_house') > 0) {
      player.notify('~r~Kullanıcı zaten bir eve sahip.');
      buyer.notify('~r~Zaten bir eve sahipsin.');
      return;
    }
    ...
    buyer.notify('~b~Size bir ev satın alma teklifinde bulunuldu. ' + hInfo.get('address') + ' ' + hInfo.get('id') + ' за ~s~$' + methods.numberFormat(sum));
    player.notify('~b~Bir oyuncuya ev satış teklifinde bulundun.');
  }
});
  ...
  if (user.isLogin(buyer)) {
    ...
    houses.getCountLiveUser(user.get(player, 'id_house'), function (count: number) {
      ...
      if (count == 1 && hInfo.get('price') < 250000) {
        player.notify('~r~Evinizde yer kalmadı!');
        player.notify('~r~Daha fazla arkadaş edinmek için alacağınız evin tutarı en az  $250.000 olmalıdır.');
        return;
      } else if (count == 2 && hInfo.get('price') < 500000) {
        player.notify('~r~Evinizde yer kalmadı!');
        player.notify('~r~Daha fazla arkadaş edinmek için alacağınız evin tutarı en az  $500.000 olmalıdır.');
        return;
      } else if (count == 3 && hInfo.get('price') < 1000000) {
        player.notify('~r~Evinizde yer kalmadı!');
        player.notify('~r~Daha fazla arkadaş edinmek için alacağınız evin tutarı en az  $1.000.000 olmalıdır.');
        return;
      } else if (count == 4 && hInfo.get('price') < 2000000) {
        player.notify('~r~Evinizde yer kalmadı!');
        player.notify('~r~Daha fazla arkadaş edinmek için alacağınız evin tutarı en az  $2.000.000 olmalıdır.');
        return;
      } else if (count == 5 && hInfo.get('price') < 3000000) {
        player.notify('~r~Evinizde yer kalmadı!');
        player.notify('~r~Daha fazla arkadaş edinmek için alacağınız evin tutarı en az  $3.000.000 olmalıdır.');
        return;
      } else if (count == 6 && hInfo.get('price') < 5000000) {
        player.notify('~r~Evinizde yer kalmadı!');
        player.notify('~r~Daha fazla arkadaş edinmek için alacağınız evin tutarı en az  $5.000.000 olmalıdır.');
        return;
      } else if (count == 7 && hInfo.get('price') < 7000000) {
        player.notify('~r~Evinizde yer kalmadı!');
        player.notify('~r~Daha fazla arkadaş edinmek için alacağınız evin tutarı en az  $7.000.000 olmalıdır.');
        return;
      } else if (count == 8 && hInfo.get('price') < 8000000) {
        player.notify('~r~Evinizde yer kalmadı!');
        player.notify('~r~Daha fazla arkadaş edinmek için alacağınız evin tutarı en az  $8.000.000 olmalıdır.');
        return;
      } else if (count == 9 && hInfo.get('price') < 9000000) {
        player.notify('~r~Evinizde yer kalmadı!');
        player.notify('~r~Daha fazla arkadaş edinmek için alacağınız evin tutarı en az  $9.000.000 olmalıdır.');
        return;
      } else if (count == 10 && hInfo.get('price') < 10000000) {
        player.notify('~r~Evinizde yer kalmadı!');
        player.notify('~r~Daha fazla arkadaş edinmek için alacağınız evin tutarı en az  $10.000.000 olmalıdır.');
        return;
      } else if (count == 15 && hInfo.get('price') < 15000000) {
        player.notify('~r~Evinizde yer kalmadı!');
        player.notify('~r~Daha fazla arkadaş edinmek için alacağınız evin tutarı en az  $15.000.000 olmalıdır.');
        return;
      } else if (count == 20 && hInfo.get('price') < 20000000) {
        player.notify('~r~Evinizde yer kalmadı!');
        player.notify('~r~Daha fazla arkadaş ekleyemezsiniz.');
        return;
      }
...
mp.events.addRemoteCounted('server:houses:lawyer:tryaddUser', (player, ownerId, buyerId) => {
  ...
  if (user.isLogin(owner) && user.isLogin(player)) {
    if (user.get(owner, 'id_house') === 0) {
      owner.notify('~r~Bir eve sahip değilsin!');
      player.notify('~r~Oyuncunun bir evi yok!');
      return;
    }

    if (user.getCashMoney(owner) < 10000) {
      owner.notify('~r~Yeterli paranız yok.');
      player.notify('~r~Oyuncunun elinde yeterli para yok.');
      return;
    }
...
mp.events.addRemoteCounted('server:houses:lawyer:addUser', (player, lawyerId, buyerId) => {
  ...
  if (user.isLogin(buyer) && user.isLogin(owner) && user.isLogin(lawyer)) {
    ...
    houses.getCountLiveUser(user.get(owner, 'id_house'), function (count: number) {
      ...
      --count;
      if (count == 1 && hInfo.get('price') < 250000) {
        owner.notify('~r~Evinizde yer kalmadı!');
        owner.notify('~r~Daha fazla arkadaş edinmek için alacağınız evin tutarı en az  $250.000 olmalıdır.');
        return;
      } else if (count == 2 && hInfo.get('price') < 500000) {
        owner.notify('~r~Evinizde yer kalmadı!');
        owner.notify('~r~Daha fazla arkadaş edinmek için alacağınız evin tutarı en az  $500.000 olmalıdır.');
        return;
      } else if (count == 3 && hInfo.get('price') < 1000000) {
        owner.notify('~r~Evinizde yer kalmadı!');
        owner.notify('~r~Daha fazla arkadaş edinmek için alacağınız evin tutarı en az  $1.000.000 olmalıdır.');
        return;
      } else if (count == 4 && hInfo.get('price') < 2000000) {
        owner.notify('~r~Evinizde yer kalmadı!');
        owner.notify('~r~Daha fazla arkadaş edinmek için alacağınız evin tutarı en az  $2.000.000 olmalıdır.');
        return;
      } else if (count == 5 && hInfo.get('price') < 3000000) {
        owner.notify('~r~Evinizde yer kalmadı!');
        owner.notify('~r~Daha fazla arkadaş edinmek için alacağınız evin tutarı en az  $3.000.000 olmalıdır.');
        return;
      } else if (count == 6 && hInfo.get('price') < 5000000) {
        owner.notify('~r~Evinizde yer kalmadı!');
        owner.notify('~r~Daha fazla arkadaş edinmek için alacağınız evin tutarı en az  $5.000.000 olmalıdır.');
        return;
      } else if (count == 7 && hInfo.get('price') < 7000000) {
        owner.notify('~r~Evinizde yer kalmadı!');
        owner.notify('~r~Daha fazla arkadaş edinmek için alacağınız evin tutarı en az  $7.000.000 olmalıdır.');
        return;
      } else if (count == 8 && hInfo.get('price') < 8000000) {
        owner.notify('~r~Evinizde yer kalmadı!');
        owner.notify('~r~Daha fazla arkadaş edinmek için alacağınız evin tutarı en az  $8.000.000 olmalıdır.');
        return;
      } else if (count == 9 && hInfo.get('price') < 9000000) {
        owner.notify('~r~Evinizde yer kalmadı!');
        owner.notify('~r~Daha fazla arkadaş edinmek için alacağınız evin tutarı en az  $9.000.000 olmalıdır.');
        return;
      } else if (count == 10 && hInfo.get('price') < 10000000) {
        owner.notify('~r~Evinizde yer kalmadı!');
        owner.notify('~r~Daha fazla arkadaş edinmek için alacağınız evin tutarı en az  $10.000.000 olmalıdır.');
        return;
      } else if (count == 15 && hInfo.get('price') < 15000000) {
        owner.notify('~r~Evinizde yer kalmadı!');
        owner.notify('~r~Daha fazla arkadaş edinmek için alacağınız evin tutarı en az  $15.000.000 olmalıdır.');
        return;
      } else if (count == 20 && hInfo.get('price') < 20000000) {
        owner.notify('~r~Evinizde yer kalmadı!');
        owner.notify('~r~Daha fazla arkadaş ekleyemezsiniz.');
        return;
      }
  ...
  (player, houseId, sum, sellerId) => {
    ...
    if (user.isLogin(seller)) {
      ...
      if (hId === 0) {
        ...
        seller.notify('~r~Bir eve sahip değilsin!');
        return;
      }
...
mp.events.addRemoteCounted('server:user:showGosDoc', (player, playerRemoteId) => {
  ...
  if (remotePlayer && user.isLogin(remotePlayer)) {
    ...
    if ([2, 3, 7].includes(user.get(player, 'fraction_id'))) {
      ...
      menuData.set('ID', '' + user.getId(player));
      menuData.set('Ad', '' + user.getRpName(player));
      menuData.set('Kuruluş', '' + methods.getFractionName(user.get(player, 'fraction_id')));
      menuData.set(
        'Rütbe',
      ...
      user.showMenu(remotePlayer, 'Belgeler', user.getRpName(player), menuData, true);
    }
  }
});
...
mp.events.addRemoteCounted('server:user:showCardId', (player, playerRemoteId) => {
  ...
  if (remotePlayer && user.isLogin(remotePlayer)) {
    ...
    user.showMenu(remotePlayer, 'Belgeler', user.getRpName(player), menuData);
  }
});
...
mp.events.addRemoteCounted('server:phone:call', (player, number) => {
  if (!user.isLogin(player)) return;
  if (player.getVariable('call')) return player.notify("Zaten biriyle konuşuyorsun.");
  let target = phone.getByNumber(number);
  let callerNumber = phone.getNumber(player);
  if (!target) {
    player.call("client:phone:show", [{ status: 'busy' }]);
    return player.notify("Aradığınız numara yanlış veya telefonu kapalı.")
  }

  if (target == player) {
    player.call("client:phone:show", [{ status: 'busy' }]);
    return player.notify("Kendinizi arayamazsınız!");
  }

  if (target.getVariable('call')) {
    player.call("client:phone:show", [{ status: 'busy' }]);
    return player.notify("Aradığınız kişi meşgul!");
  }

  methods.debug('test');

  target.call("client:phone:show", [{
  ...
  /*
    user.accept(target, "Звонок от " + phone.getNumber(player)).then(status => {
      ...
      target.call("client:phone:show", [{ status: 'call_accepted', callerNumber: callerNumber }]);
      player.call("client:phone:show", [{ status: 'call_accepted' }]);
...
mp.events.addRemoteCounted('server:phone:acceptCall', (target, callerNumber) => {

  const caller = phone.getByNumber(callerNumber);

  caller.enableVoiceTo(target)
  target.enableVoiceTo(caller)
  caller.setVariable('call', callIds);
  target.setVariable('call', callIds);
  caller.callsocket('callStart', [target])
  target.callsocket('callStart', [caller])

  target.call("client:phone:show", [{ status: 'call_accepted', callerNumber: callerNumber }]);
  caller.call("client:phone:show", [{ status: 'call_accepted' }]);
...
mp.events.addRemoteCounted('server:phone:cancelCall', (player) => {
  ...
  target.call("client:phone:show", [{ status: 'call_ended', }]);
  player.call("client:phone:show", [{ status: 'call_ended' }]);
});
...
import { gunShopList } from '../../util/shop';
...
mp.events.addRemoteCounted('server:phone:AddAd', (player, text, rpName, phone, type) => {
  ...
  if (!methods.isTestServer()) {
    const exampleEmbed = new Discord.RichEmbed()
      .setColor('#f00c17')
      .setTitle(`Bildirim [${type}]`)
      .setURL('https://discord.gg/torp')
      .setAuthor(`${phone} [${user.getId(player)}]`)
      .setDescription(text)
      .setTimestamp()
      .setFooter('Hava durumu', 'https://i.imgur.com/qats7HI.png');
...
export const createTempPromocode = (player: PlayerMp, sum: number, dist: number, los: boolean): void => {
  ...
  tempPromocodes.set(code, {
    sum, label: mp.labels.new('~b~Etkinlik bonusu kodu aktifleştirildi. Öldül: ~g~$' + methods.numberFormat(sum) + '\r~p~' + code, player.position, {
...
mp.events.addRemoteCounted('server:activatePromocode', async (player, promocode) => {
  ...
  if (player.spamProtect) return player.notify(`~r~Birkaç saniye bekleyin.`)
  ...
  if (tempPromocodes.has(promocode)) {
    ...
    player.notify(`~g~ ${promocode} Promosyon kodunu kullanarak ${data.sum}$ kazandınız.`)
  ...
  if (alreadyEntered) return player.notify('~r~Bu promosyon kodunu zaten kullanmışsınız.');
  promocodeEntity.findAll({
    ...
    if (rows.length >= 1) {
      rows.forEach((row) => {
        ...
        player.notify(
          `~g~${promocode} Promosyon kodunu kullanarak ${methods.numberFormat(row['bonus'])}$ kazandınız.`
        );
      ...
      promocodeTopListEntity.findAll({
        where: {
          promocode: promocode,
        }, limit: 1
      }).then(rows => {
        if (rows.length >= 1) {
          if (user.get(player, 'promocode') == '') {
            if (user.getLevel(player) <= levelAccess.startPromocode) {
              ...
              player.notify('~g~Bir promosyon kodu girdiniz. Kod: ~s~' + promocode);
            ...
            player.notify('~r~Promosyon kodunu bu seviyede kullanamazsınız. Kullanabileceğiniz seviye(max) : ' + levelAccess.startPromocode);
            return;
          }
          player.notify('~r~Bu promosyon kodunu zaten etkinleştirdiniz.');
        } else {
          player.notify('~r~Geçersiz promosyon kodu girdiniz.');
        }
      }
      );
    }
...
mp.events.addRemoteCounted("custom:norpfload", player => {
  player.notify("~r~Dikkat!!! ~r~Gerekli dokulara sahip değilsiniz");
  player.outputChatBox("Dikkat!!! İstemcinizde normal çalışma için gerekli özel dokulara sahip olmadığınızı gördük. Doğru çalışması için RAGE Multiplayer'ı tamamen kapatmanızı ve sunucuyu yeniden başlatmanızı rica ederiz.")
})

mp.events.addRemoteCounted("server:vehicle:tunmoney:return", async player => {
  if (!player.vehicle) return player.notify("~r~Araçta olmalısın.");
  ...
  if (car) {
    if (car.has('upgrade')) {
      ...
      if (upgrade) {
        ...
        if (resprice == 0) return player.notify("~r~Bu araçta ayar çipiniz yok.");
        for (let id = 0; id < 9; id++) {
        ...
        if (resprice > 0) {
          ...
          player.notify("~g~Paranız iade edildi. Miktar: " + resprice + "$");
          vehicles.respawn(vehicle);
        } else {
          return player.notify("~r~Bu araçta ayar çipiniz yok.");
        }
      }
    }
  }
...
mp.events.add("my:gps", (player: PlayerMp) => {
  let m = menu.new(player, "Mülkiyet");
  ...
  if (user.get(player, "apartment_id") > 0) {
    ...
    if (buildData) {
      ...
      if (typeof buildId == "number") {
        m.newItem({
          name: `Numara ${buildId} #${buildData.get('id')}`,
          more: "Apartman Daireleri",
          onpress: (itm) => {
