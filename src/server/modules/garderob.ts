...
export class dressRoom {
  ...
  delete(player: PlayerMp) {
    if (!user.isAdminNow(player, 6)) return player.notify("~r~Bu işlemi yapabilmek için minimum 6.seviye admin olmalısın.");
    user.accept(player, "Silmek istediğine emin misin?", "~r~Evet", "~g~Hayır").then(status => {
      if (!status) return;
      garderobEntity.destroy({ where: { id: this.id } }).then(() => {
        this.checkpoint.delete();
        garderobPool.delete(this.id);
        player.notify("~g~Giyinme dolabı silindi.");
      })
    })
  }
  constructor(id: number, fraction: number, position: Vector3Mp, dresses: dressItem[], dimension: number) {
    ...
    this.checkpoint = methods.createDynamicCheckpoint(this.position, "Dolabı açmak için ~g~E~w~ tuşuna basın.", player => {
