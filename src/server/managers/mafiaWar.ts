...
export let mafiaWar = {
    ...
    takeBank: function(player:PlayerMp, id:number, sum:number) {
        ...
        if (mafiaWar.get(id, 'money') < sum) {
            player.notify('~r~Bu tutar hesapta bulunmamaktadır');
            return;
        }
        ...
        if (zoneId == id) {
            ...
            player.notify('~y~Bölgenin hesabından para çektiniz');
            user.addMoney(player, sum);
            tablet.openMafiaTer(player)
        }
        else {
            player.notify('~r~Tesislerde bulunmak gereklidir');
        }
    },
    ...
    startWar: function(player:PlayerMp) {
        ...
        if (id == 0) {
            player.notify('~r~Herhangi bir bölgeden çok uzaktasınız');
            return;
        }
    
        if (!mafiaWar.get(id, 'canWar')) {
            player.notify('~r~Bu bölgeyi yakalamak yarın mümkün olacak');
            return;
        }
        if (weather.getHour() < 20 && weather.getHour() > 8) {
            player.notify('~r~Yalnızca IC saatiyle akşam 8 ile sabah 8 arasında kullanılabilir');
            return;
        }
        ...
        if (ownerId > 0) {
            if (dateTime.getDate() % 3) {
                player.notify('~r~Her 3 günde bir mevcuttur (OOC)');
                player.notify('~r~Şöyle ki: Takvimde 3, 6, 9, 12, 15, 18, 21, 24, 27, 30');
                return;
            }
        }
    
        if (dateTime.getHours() < 20) {
            player.notify('~r~Sadece OOC saatiyle akşam 8den sabah 24e kadar kullanılabilir');
            return;
        }
        ...
        switch (id) {
            case 1:
                if (isStartTimer1) {
                    player.notify('~r~Şu anda bölge ele geçirilmiş durumda.');
                    return;
                }
                isStartTimer1 = true;
                timerCounter1 = 900;
                methods.notifyWithPictureToFraction('Bölge için savaş', `Organizasyon`, 'Toprak işgali başladı ~y~Elysian Adası', 'CHAR_DEFAULT', 8);
                methods.notifyWithPictureToFraction('Bölge için savaş', `Organizasyon`, 'Toprak işgali başladı ~y~Elysian Adası', 'CHAR_DEFAULT', 9);
                methods.notifyWithPictureToFraction('Bölge için savaş', `Organizasyon`, 'Toprak işgali başladı ~y~Elysian Adası', 'CHAR_DEFAULT', 10);
                methods.notifyWithPictureToFraction('Bölge için savaş', `Organizasyon`, 'Toprak işgali başladı ~y~Elysian Adası', 'CHAR_DEFAULT', 11);
                break;
            case 2:
                if (isStartTimer2) {
                    player.notify('~r~Şu anda bölge ele geçirilmiş durumda.');
                ...
                isStartTimer2 = true;
                timerCounter2 = 900;
                methods.notifyWithPictureToFraction('Bölge için savaş', `Organizasyon`, 'Toprak işgali başladı ~y~La Puerta', 'CHAR_DEFAULT', 8);
                methods.notifyWithPictureToFraction('Bölge için savaş', `Organizasyon`, 'Toprak işgali başladı ~y~La Puerta', 'CHAR_DEFAULT', 9);
                methods.notifyWithPictureToFraction('Bölge için savaş', `Organizasyon`, 'Toprak işgali başladı ~y~La Puerta', 'CHAR_DEFAULT', 10);
                methods.notifyWithPictureToFraction('Bölge için savaş', `Organizasyon`, 'Toprak işgali başladı ~y~La Puerta', 'CHAR_DEFAULT', 11);
                break;
            case 3:
                if (isStartTimer3) {
                    player.notify('~r~Şu anda bölge ele geçirilmiş durumda.');
                ...
                isStartTimer3 = true;
                timerCounter3 = 900;
                methods.notifyWithPictureToFraction('Bölge için savaş', `Organizasyon`, 'Toprak işgali başladı ~y~Grapevine Havaalanı', 'CHAR_DEFAULT', 8);
                methods.notifyWithPictureToFraction('Bölge için savaş', `Organizasyon`, 'Toprak işgali başladı ~y~Grapevine Havaalanı', 'CHAR_DEFAULT', 9);
                methods.notifyWithPictureToFraction('Bölge için savaş', `Organizasyon`, 'Toprak işgali başladı ~y~Grapevine Havaalanı', 'CHAR_DEFAULT', 10);
                methods.notifyWithPictureToFraction('Bölge için savaş', `Organizasyon`, 'Toprak işgali başladı ~y~Grapevine Havaalanı', 'CHAR_DEFAULT', 11);
                break;
        }
        return true;
    },
    ...
    timer: function() {
        if (isStartTimer1) {
            ...
            if (timerCounter1 < 1) {
                ...
                methods.notifyWithPictureToFraction('Savaşın sonucu', `Elysian Adası`, 'Bölge kontrol altında ' + fractionName, 'CHAR_DEFAULT', 8);
                methods.notifyWithPictureToFraction('Savaşın sonucu', `Elysian Adası`, 'Bölge kontrol altında ' + fractionName, 'CHAR_DEFAULT', 9);
                methods.notifyWithPictureToFraction('Savaşın sonucu', `Elysian Adası`, 'Bölge kontrol altında ' + fractionName, 'CHAR_DEFAULT', 10);
                methods.notifyWithPictureToFraction('Savaşın sonucu', `Elysian Adası`, 'Bölge kontrol altında ' + fractionName, 'CHAR_DEFAULT', 11);
            }
        ...
        if (isStartTimer2) {
            ...
            if (timerCounter2 < 1) {
                ...
                methods.notifyWithPictureToFraction('Savaşın sonucu', `La Puerta`, 'Bölge kontrol altında ' + fractionName, 'CHAR_DEFAULT', 8);
                methods.notifyWithPictureToFraction('Savaşın sonucu', `La Puerta`, 'Bölge kontrol altında ' + fractionName, 'CHAR_DEFAULT', 9);
                methods.notifyWithPictureToFraction('Savaşın sonucu', `La Puerta`, 'Bölge kontrol altında ' + fractionName, 'CHAR_DEFAULT', 10);
                methods.notifyWithPictureToFraction('Savaşın sonucu', `La Puerta`, 'Bölge kontrol altında ' + fractionName, 'CHAR_DEFAULT', 11);
            }
        ...
        if (isStartTimer3) {
            ...
            if (timerCounter3 < 1) {
                ...
                methods.notifyWithPictureToFraction('Savaşın sonucu', `Grapevine Havaalanı`, 'Bölge kontrol altında ' + fractionName, 'CHAR_DEFAULT', 8);
                methods.notifyWithPictureToFraction('Savaşın sonucu', `Grapevine Havaalanı`, 'Bölge kontrol altında ' + fractionName, 'CHAR_DEFAULT', 9);
                methods.notifyWithPictureToFraction('Savaşın sonucu', `Grapevine Havaalanı`, 'Bölge kontrol altında ' + fractionName, 'CHAR_DEFAULT', 10);
                methods.notifyWithPictureToFraction('Savaşın sonucu', `Grapevine Havaalanı`, 'Bölge kontrol altında ' + fractionName, 'CHAR_DEFAULT', 11);
            }
