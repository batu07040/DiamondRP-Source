...
export function restartVoice() {
    if (block) return mp.game.ui.notifications.show('~r~Sesli sohbeti bu kadar sik yeniden baslatamazsiniz');
    block = true;
    setTimeout(() => {
        block = false;
    }, 10000)
    mp.voiceChat.cleanupAndReload(true, true, true);
    mp.game.ui.notifications.show('~g~Sesli sohbet yeniden baslatildi');
}
...
function voiceControllerTick() {
    ...
    mp.players.forEach(nplayer => {
        ...
        if (!phoneRadioController(nplayer)) {
            ...
            let _megaPhoneStatus = user.get("enableMegaPhone");
            let iRange = _megaPhoneStatus ? 50.0 : MaxRange;
            const needSpeak = (dist < iRange) && mp.players.local.dimension == nplayer.dimension
            ...
            if (!needSpeak) {
                ...
                if (nplayer.vehicle && nplayer.vehicle == mp.players.local.vehicle) {
                    ...
                    if (mp.players.local.vehicle) {
                        ...
                        let endVolume = (mp.players.local.hasClearLosTo(nplayer.handle, 17) ? voiceVolume : voiceVolume / 4) - (voiceVolume / (_megaPhoneStatus ? 50.0 : MaxRange)) * (dist + 1.3)
...
setInterval(() => {
    if (megaPhoneKeyPressed) {
        if (mp.keys.isDown(0x51)) return;
        disableMegaPhone();
        return;
    }

    if (radioKeyPressed || mp.players.local.getVariable('radioSpeak')) {
        if (mp.keys.isDown(0xc0)) return;
        disableMicrophoneRadio();
        return;
    }
...
let megaPhoneKeyPressed = false;
const enableMegaPhone = () => {
    if (!user.isLogin())
        return;
    if (user.isCuff())
        return;
    if (user.isDead())
        return;
    let _veh = user.getVehicle();
    if (!_veh) return;
    if (!vehicles.isVehicleSirenValid(_veh.model)) return;
    if (megaPhoneKeyPressed) return;
    ...
    ui.radioSoundOn();
    enableMicrophone();
    megaPhoneKeyPressed = true;
    user.set("enableMegaPhone", true);
    mp.events.callSocket("voice.server.enableMic");

};

const disableMegaPhone = () => {
    if (!user.isLogin())
        return;
    disableMicrophone();
    user.set("enableMegaPhone", false);
    mp.events.callSocket("voice.server.disableMic");
    gui.browser.execute('CEF.hud.setRadio(false)');
    user.stopAllAnimation();
    megaPhoneKeyPressed = false;
};



mp.keys.bind(0xc0, true, enableMicrophoneRadio);
mp.keys.bind(0xc0, false, disableMicrophoneRadio);



mp.keys.bind(0x51, true, enableMegaPhone);
mp.keys.bind(0x51, false, disableMegaPhone);
