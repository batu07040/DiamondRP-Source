...
grab: function() {
  if (weather.getHour() < 22 && weather.getHour() > 6) {
      mp.game.ui.notifications.show('~r~Sadece 22:00 ile 06:00 arasında yapılabilir.');
      return;
  }
  ...
  chests.grabMarkers.forEach(function (item) {
      if (methods.distanceToPos(new mp.Vector3(item[0], item[1], item[2]), mp.players.local.position) < 20) {
          ...
          user.giveWanted(5, 'Silahlı araç soygunundan dolayı aranıyorsunuz.');
          isFind = true;
          return;
      }
  });
  ...
  if (!isFind) {
      ...
      mp.game.ui.notifications.show('~y~Soygun noktasına geldiniz.');
  }
},
