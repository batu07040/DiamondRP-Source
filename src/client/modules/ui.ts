...
let ui = {
  ...
  updateDostavkaTimer: (timerCounter: number) => {
    if (gui.hudLoaded) {
      gui.browser.execute(`CEF.hud.setDostavkaTime(${timerCounter})`);
    }
  },

  showDostavkaTimer: () => {
    if (gui.hudLoaded) {
      gui.browser.execute(`CEF.hud.toggleDostavkaTimer(true)`);
    }
  },

  hideDostavkaTimer: () => {
    if (gui.hudLoaded) {
      gui.browser.execute(`CEF.hud.toggleDostavkaTimer(false)`);
    }
  },
  ...
  drawText3D: (caption: string, x: number, y: number, z: number, scale = 0.3) => {
    if (!mp.game.ui.isHudComponentActive(0)) return false;
