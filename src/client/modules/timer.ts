...
let timer = {
  ...
  setDostavkaTimer: (sec: any) => {
    dostavkaTimer = parseInt(sec);
    if (parseInt(sec) > 0) ui.showDostavkaTimer();
    else ui.hideDostavkaTimer();
  },

  getDostavkaTimer: () => {
    return dostavkaTimer;
  },
  ...
  twoMinTimer: () => {
    ...
    if (veh && mp.vehicles.exists(veh) && veh.getClass() == 18 && !user.isGos()) {
      if (veh.getPedInSeat(-1) == mp.players.local.handle) {
        user.giveWanted(10, 'Devlet aracını çalmaktan aranıyorsunuz.');
        dispatcher.send(`KOD 0`, `Kimliği belirsiz bir kişi devlet aracını çaldı.`);
      }
    }
  ...
  secTimer: () => {
    ...
    if (user.getDrugLevel(drugId) > 0) {
      ...
      if (user.getDrugLevel(drugId) > 210) {
        mp.gui.chat.push(`!{03A9F4}Aşırı dozdan komadasınız.`);
        user.setHeal(0);
      }
    ...
    if (user.has('id')) {
      if (user.currentId - 1000000 != user.get('id')) {
        mp.gui.chat.push(
          `!{03A9F4}Kimlik değiştirme algılandı. Loglar admine gönderildi.`
        );
        user.kickAntiCheat(`Cheat Engine`);
        methods.saveLog('CheatEngine', `Kullanıcı ${user.get('rp_name')} (${user.get('id')}) kimliğini değiştirmeye çalıştı.`);
        return;
      }
    }

    if (user.get('money') < -15000 || user.get('money_bank') < -15000) {
      user.kick(`Anti-Cheat System: Lütfen yönetim ile iletişime geçin.`);
      methods.saveLog('CheaterMoney', `Kullanıcı ${user.get('rp_name')} (${user.get('id')}) para trafiği incelenmelidir.`);
      return;
    }
  ...
  secTimer1: () => {
    if (dostavkaTimer > 0) {
      dostavkaTimer--;
      ui.updateDostavkaTimer(dostavkaTimer);
      if (dostavkaTimer == 0) {
        timer.setDostavkaTimer(0);
      }
    }
    setTimeout(timer.secTimer1, 1000);
  },
