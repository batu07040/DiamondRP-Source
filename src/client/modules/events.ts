...
mp.events.add(
  ...
  (id, name, company, x, y, z, rot, px, py, pz, price) => {
    methods.debug('Event: client:trucker:acceptOffer2');
    trucker.acceptOffer2(id, name, company, x, y, z, px, py, pz, price);
  }
);
...
mp.events.add('dostavkaTimer', (deathTimer: any) => {
  timer.setDostavkaTimer(parseInt(deathTimer));
  // ui.updateDostavkaTimer(deathTimer);
  methods.debug('FGFGFGFGFFFFGFGFFG' + parseInt(deathTimer))
});
...
mp.events.add('playerDeath', async function (player, reason, killer) {
  ...
  mp.events.triggerBrowser('cef:alert:setBigAlert', 'success', escape("Doktor çağırmak için Y tuşuna basın. Çağırmak istemiyorsanız N tuşuna basın."), 7000);
...
mp.events.add('render', () => {
  ...
  mp.game.ui.hideHudComponentThisFrame(19); // GTA TAB SİLAH
  mp.game.ui.hideHudComponentThisFrame(22); // GTA TAB SİLAH
  ...
  mp.game.controls.disableControlAction(32, 14, true);
  mp.game.controls.disableControlAction(32, 15, true);
  mp.game.controls.disableControlAction(32, 16, true);
  mp.game.controls.disableControlAction(32, 17, true);
  ...
  if (user.isLogin()) {
    if (user.get('jail_time')) {
      mp.game.graphics.drawText("Kalan Süre: " + formatTime(user.get('jail_time')), [0.5, 0.9], {
        font: 0,
    ...
    if (user.get('med_time')) {
      mp.game.graphics.drawText("İyileşmenize Kalan Süre: " + formatTime(user.get('med_time')), [0.5, 0.9], {
        font: 0,
        color: [255, 255, 255, 100],
        scale: [0.4, 0.4],
        outline: true,
...
mp.keys.bind(0x31, true, function () {
  ...
  let _weapons = inventory.hasWeapons()
  if (!_weapons || _weapons.length < 1) return;

  let { id, hash, ammoCount } = _weapons[0];

  mp.events.callRemote("server:inventory:switchWeapon", id, hash, ammoCount);
...
mp.keys.bind(0x32, true, function () {
  ...
  let { id, hash, ammoCount } = _weapons[1];
...
mp.keys.bind(0x33, true, function () {
  ...
  if (!user.isLogin()) return;
  let _weapons = inventory.hasWeapons()
  if (!_weapons || _weapons.length < 2) return;

  let { id, hash, ammoCount } = _weapons[2];
...
mp.keys.bind(0x34, true, function () {
  ...
  if (!user.isLogin()) return;
  let _weapons = inventory.hasWeapons()
  if (!_weapons || _weapons.length < 3) return;
  let { id, hash, ammoCount } = _weapons[3];
...
mp.keys.bind(0x35, true, function () {
  ...
  if (!user.isLogin()) return;
  let _weapons = inventory.hasWeapons()
  if (!_weapons || _weapons.length < 4) return;
  let { id, hash, ammoCount } = _weapons[4];
...
mp.keys.bind(0x47, true, function () {
  if (gui.chatActive) return;
  if (!user.isLogin()) return;
  if (!methods.isBlockKeys() && user.getTargetEntityValidate() || !methods.isBlockKeys() && mp.players.local.vehicle && mp.players.local.vehicle.getPedInSeat(-1) === mp.players.local.handle) {
    ui.showOrHideMenu();
  } else {
    return;
  }
...
mp.keys.bind(0x28, true, function () {
  ...
  if (menuList.currentMenu()) return;
  if (gui.currentGui === 'tablet') {
    menuList.currentMenu().close();
  }
  if (gui.currentGui) return;

  if (!user.get('tablet_equip')) return user.notify("~r~Donanımlarınız arasında tablet yok. Lütfen mağazadan bir tablet alın ve donanımlarınıza ekleyin.")
  mp.events.callRemote('tablet:main');
});

mp.keys.bind(0x34, true, function () {
  // FRAKSİYON MENÜSÜ KAPATILDI
  return;
  if (gui.chatActive) return;
  if (!user.isLogin()) return;
...
mp.keys.bind(0x26, true, function () {
  ...
  else if (!(user.get('phone_code') > 0)) return user.notify("~r~Donanımlarınız arasında telefon yok. Lütfen mağazadan bir telefon alın ve donanımlarınıza ekleyin.")
});
  ...
  if (!methods.isBlockKeys()) {
    if (mp.players.local.vehicle) {
      if (seatBelt) {
        seatBelt = false;
        mp.players.local.setConfigFlag(32, true);
        mp.game.ui.notifications.show('~r~Emniyet kemerini çıkardın.');
        chat.sendMeCommand('Emniyet kemerini çıkardı.');
        ui.unbuckle()
      } else {
        seatBelt = true;
        mp.players.local.setConfigFlag(32, false);
        mp.game.ui.notifications.show('~g~Emniyet kemerini taktın.');
        chat.sendMeCommand('Emniyet kemerini taktı.');
        ui.buckle()
      }
      seatBlock = true;
      setTimeout(() => {
...
mp.events.add('client:showId', () => {
  showIds = !showIds;
  if (showIds) mp.game.ui.notifications.show('Oyuncu Kimlikleri(ID): ~g~Etkin');
  else mp.game.ui.notifications.show('Oyuncu Kimlikleri(ID): ~r~Devre Dışı');
});
...
mp.events.add("client:phone:acceptCall", (callerNumber) => {
  phone.acceptCall(
    callerNumber,
  );
});
...
mp.events.add('client:phone:transferBank', (
  prefix, number, amount
) => {
  phone.transferBank(prefix, number, amount);
});
...
mp.events.add('client:phone:show', (addditionaldata) => {
  ...
  if (action == 'settings') {
    ...
    if (action.length > 1) {
      ...
      if (act == 'giveMoney') {
        ...
        if (money < 1) {
          mp.game.ui.notifications.show('~r~1 Dolardan az para transferi yapılamaz.');
          return;
        }
        ...
        if (rpName == '') return;
        mp.events.callRemote('server:user:askDatingToPlayerId', id, rpName);
      } else if (act == 'showGosDoc' || act == 'showCardId' || act == 'showLic') {
        chat.sendMeCommand('Belgelerini gösterdi.');
