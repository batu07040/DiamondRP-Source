...
function switchFly(status:boolean){
  if(status && mp.players.local.vehicle) return mp.game.ui.notifications.show("Uçabilmek için araçtan inmelisin.");
  if (status && user.get('is6Duty') && !methods.isTestServer()) return mp.game.ui.notifications.show("Tahsilat yaparken uçamazsınız.");
  ...
  if(fly.flying){
    mp.game.ui.notifications.show(`<strong>Yonetim</strong><b>Tasinma</b>: WSAD\n<b>Eksen degistirme Z</b>: CTRL + Space\n<b>Hiz degisimi</b>: Q + E\n<b>Ekseni sabitleyin Z</b>: X`);
  }
...
mp.keys.bind(0x58, true, function() {
  ...
  fly.lockZ = !fly.lockZ
  mp.game.ui.notifications.show(fly.lockZ ? 'Eksen Z: ~g~Sabit' : 'Eksen Z: ~r~Sabitlemeden cikarildi');
});
...
export const noClip = function(enable:boolean) {
    try {
        ...
        if (noClipEnabled)
            mp.game.ui.notifications.show(`~b~Tiklayin ~s~H~b~ Kapatmak icin`);
