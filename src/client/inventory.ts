...
const inventory = {
  ...
  hasWeapons: () => {
    ...
    for (let n = 54; n < 138; n++) {
      weapons.hashesMap.forEach((item) => {
        ...
        data = [...data, { id: n, hash: hash, ammoCount, ammoItem, }]
      });
    }
    return data;
  },
  ...
  equipCloth: (id: number, itemId: number, prefix: number, number: number, keyId: number, countItems = 1) => {
    if (prefix != user.getSex()) {
      if (prefix == 1) mp.game.ui.notifications.show('~r~Kadin kiyafetlerini giyemezsiniz');
      else mp.game.ui.notifications.show('~r~Erkek kiyafetlerini giyemezsiniz');
      return;
    }
  ...
  unEquipItem: async (
    ...
    if (isAmoutCheck) {
      ...
      if (items.getItemAmountById(itemId) + amount > inventory.maxAmount(1, user.get('id'))) {
        mp.game.ui.notifications.show('~r~Envanteriniz dolu!');
        inventory.open()
        return;
      }
    }
    ...
    // await sleep();
    if (user.isDead()) return mp.game.ui.notifications.show('~r~Komadasin'), inventory.hide();
    ...
    switch (itemId) {
      ...
      case 7:
        ...
        chat.sendMeCommand('Saatini cikardi');
        break;
      case 8:
        inventory.addItemServer(
          8,
        ...
        mp.game.ui.notifications.show('~g~Telefonu ac');
        break;
      case 42:
        inventory.addItemServer(
          42,
        ...
        mp.game.ui.notifications.show('~g~Ofisin anahtarlarini kenara koy.');
        break;
      case 43:
        inventory.addItemServer(
          43,
        ...
        mp.game.ui.notifications.show('~g~Evin anahtarlarini kenara koy.');
        break;
      case 44:
        inventory.addItemServer(
          44,
        ...
        mp.game.ui.notifications.show('~g~Dairenin anahtarlarini kenara koy.');
        break;
      case 50:
      ...
      case 138:
        if (user.getMoney() < 1) {
          mp.game.ui.notifications.show('~r~Nakit paraniz yok');
          return;
        }
        ...
        mp.game.ui.notifications.show('~g~Envanterinize dolar koydunuz');
        break;
      case 139:
      ...
      case 140:
      case 141:
        let money = <number>await <Promise<any>>UIMenu.Menu.GetUserInput('Miktar', '', 9);

        money = methods.parseInt(money);

        if (user.getMoney() < money) {
          mp.game.ui.notifications.show('~r~O kadar paran yok.');
          return;
        }
        if (money <= 0) {
          mp.game.ui.notifications.show('~r~Nakit paran yok.');
          return;
        }
        ...
        mp.game.ui.notifications.show(`~g~Kaldirdiniz $${methods.numberFormat(money)} Envantere`);
        break;
      case 27:
      case 28:
      case 29:
      ...
      case 153:
        if (type == 9999) {
          ...
          inventory.removeAllAmmo(itemId);
          mp.game.ui.notifications.show('~g~Muhimmati envantere koydun');
        }
        break;
      default:
        if (itemId <= 136 && itemId >= 54) {
          if (type == inventory.types.StockFraction) {
            ...
            weapons.hashesMap.forEach((item) => {
              ...
              mp.game.ui.notifications.show('~g~Silahini kaldir');
            });
          }
        }
    }
  ...
  takeNewItem: async (itemId: number, count = 1) => {
    ...
    inventory.updateAmount(user_id, inventory.types.Player);
    mp.game.ui.notifications.show(`~b~Aldin \"${items.getItemNameById(itemId)}\"`);
    chat.sendMeCommand(`Suradan alinmistir \"${items.getItemNameById(itemId)}\"`);
  },
  ...
  takeItem: async (id: number, itemId: number, ownerType: number, notify = true) => {
    ...
    if (!notify) return;
    mp.game.ui.notifications.show(`~g~Aldin \"${items.getItemNameById(itemId)}\"`);
    chat.sendMeCommand(`Suradan alinmistir \"${items.getItemNameById(itemId)}\"`);
  },

  giveItem: async (id: number, itemId: number, playerId: number, notify = true) => {
    if (itemId == 50) {
      return mp.game.ui.notifications.show("~r~Banka karti transfer edilemez!");
    }
    if (itemId >= 138 && itemId <= 141) {
      return mp.game.ui.notifications.show("~r~Bir tomar para veremezsin!");
    }
    let user_id = user.get('id');
    ...
    if (!notify) return;
    mp.game.ui.notifications.show(`~g~Ilettiniz \"${items.getItemNameById(itemId)}\" Oyuncuya`);
    chat.sendMeCommand(`Tarafindan iletildi \"${items.getItemNameById(itemId)}\" Yaninizdaki kisi`);
  },
  ...
  dropItemToVehicle: async (id: number, itemId: number, number: string, notify = true) => {
    ...
    if (items.getItemAmountById(itemId) + amount > amountMax) {
      mp.game.ui.notifications.show('~r~Bagajda yer yok');
      return;
    }
    ...
    if (!notify) return;
    mp.game.ui.notifications.show(`~g~Yere koydun \"${items.getItemNameById(itemId)}\" Bagajda`);
    chat.sendMeCommand(`Koy \"${items.getItemNameById(itemId)}\" Bagajda`);
  },
  ...
  dropItemToStockFraction: async (id: number, itemId: number, ownerId: number, notify = true) => {
    ...
    if (items.getItemAmountById(itemId) + amount > amountMax) {
      mp.game.ui.notifications.show('~r~Depoda yer yok');
      return;
    }
    ...
    if (!notify) return;
    mp.game.ui.notifications.show(`~g~Yere koy \"${items.getItemNameById(itemId)}\" Stokta`);
    chat.sendMeCommand(`Koy \"${items.getItemNameById(itemId)}\" Stokta`);
  },
  ...
  dropItemToFridge: async (id: number, itemId: number, ownerId: number, notify = true) => {
    ...
    if (items.getItemAmountById(itemId) + amount > amountMax) {
      mp.game.ui.notifications.show('~r~Buzdolabinda yer yok');
      return;
    }
    ...
    mp.game.ui.notifications.show(
      `~g~Yere koy \"${items.getItemNameById(itemId)}\" Buzdolabina`
    );
    chat.sendMeCommand(`Koy \"${items.getItemNameById(itemId)}\" Buzdolabina`);
  },
  ...
  takeDrugItem: async (id: number, itemId: number, countItems: number, notify = true, takeCount = 1) => {
    ...
    if (!notify) return;
    mp.game.ui.notifications.show(`~g~Sen aldin \"${items.getItemNameById(newItemId)}\"`);
    chat.sendMeCommand(`Suradan alinmistir ${takeCount}Gram uyusturucu`);
  },
  ...
  takeCountItem: async (id: number, itemId: number, countItems: number, notify = true, takeCount = 1) => {
    ...
    if (!notify) return;
    mp.game.ui.notifications.show(`~g~Sen aldin \"${items.getItemNameById(newItemId)}\"`);
    chat.sendMeCommand(`Suradan alinmistir ${takeCount}Madde`);
  },
  ...
  startFishing: () => {
    if (!methods.isPlayerInOcean()) {
      mp.game.ui.notifications.show('~r~Okyanusta olmalisin.');
      return;
    }
    if (mp.players.local.isSwimming()) {
      mp.game.ui.notifications.show('~r~Suyun icinde olmamalisiniz.');
      return;
    }
    if (mp.players.local.vehicle) {
      mp.game.ui.notifications.show('~r~Tasimacilikta olmamalisiniz');
      return;
    }
    if (Container.HasLocally(0, 'fish')) {
      mp.game.ui.notifications.show('~r~Zaten balik tutuyorsunuz');
      return;
    }
  ...
  switch (itemId) {
    case 265: {
      if (user.get('torso') != 15) {
        mp.game.ui.notifications.show('~r~Zaten bir govde ile donatilmis durumdasiniz');
        return;
      }
    ...
    case 266: {
      if (user.getSex() == 0) {
        if (user.get('leg') != 61) {
          mp.game.ui.notifications.show('~r~Pantolonuz zaten hazir');
          return;
        }
      } else {
        if (user.get('leg') != 15) {
    ...
    case 267: {
      if (user.getSex() == 0) {
        if (user.get('foot') != 34) {
          mp.game.ui.notifications.show('~r~Ayakkabilariniz zaten hazir');
          return;
        }
      } else {
        if (user.get('foot') != 35) {
    ...
    case 268: {
      if (user.get('accessorie') > 0) {
        mp.game.ui.notifications.show('~r~Zaten bir aksesuariniz var');
        return;
      }
    ...
    case 269: {
      if (user.get('hat') > 0) {
        mp.game.ui.notifications.show('~r~Sapkanizi zaten takmissiniz');
        return;
      }
    ...
    case 270: {
      if (user.get('glasses') > 0) {
        mp.game.ui.notifications.show('~r~Gozlugunuzu zaten takmissiniz');
        return;
      }
    ...
    case 271: {
      if (user.get('ear') > 0) {
        mp.game.ui.notifications.show('~r~Kupelerinizi zaten takmissiniz');
        return;
      }
    ...
    case 272: {
      if (user.get('watch') > 0) {
        mp.game.ui.notifications.show('~r~Zaten bir saatiniz var');
        return;
      }
    ...
    case 273: {
      if (user.get('bracelet') > 0) {
        mp.game.ui.notifications.show('~r~Bilezigi zaten takmissiniz');
        return;
      }
    ...
    case 274: {
      if (user.get('mask') > 0) {
        mp.game.ui.notifications.show('~r~Zaten bir maskeniz var');
        return;
      }

      if (prefix != user.getSex()) {
        if (prefix == 1)
          mp.game.ui.notifications.show('~r~Kadin kiyafetlerini giyemezsiniz');
        else mp.game.ui.notifications.show('~r~Erkek kiyafetlerini giyemezsiniz');
        return;
      }
    ...
    case 7:
      if (!user.get('item_clock')) {
        ...
        chat.sendMeCommand('Saat takmak');

        inventory.deleteItemServer(id);
      } else {
        mp.game.ui.notifications.show('~r~Zaten donanimli saatiniz var');
      }
      break;
    case 8:
      if (user.get('phone_code') == 0) {
        ...
        mp.game.ui.notifications.show('~g~Telefonunuzu donattiniz');
        inventory.deleteItemServer(id);
      } else {
        mp.game.ui.notifications.show('~r~Zaten donanimli bir telefonunuz var');
      }
      break;
    case 42:
      if (user.get('business_id') == 0) {
        ...
        mp.game.ui.notifications.show('~g~Ofisin anahtarlari elinizde');
        inventory.deleteItemServer(id);
      } else {
        mp.game.ui.notifications.show('~r~Ofis icin zaten bir anahtariniz var');
      }
      break;
    case 43:
      if (user.get('id_house') == 0) {
        ...
        mp.game.ui.notifications.show('~g~Evinizin anahtarlari elinizde');
        inventory.deleteItemServer(id);
      } else {
        mp.game.ui.notifications.show('~r~Ev icin zaten bir anahtariniz var');
      }
      break;
    case 44:
      if (user.get('apartment_id') == 0) {
        ...
        mp.game.ui.notifications.show('~g~Daire anahtarlari elinizde');
        inventory.deleteItemServer(id);
      } else {
        mp.game.ui.notifications.show('~r~Daire icin zaten bir anahtariniz var');
      }
      break;
    case 50:
      if (user.get('bank_prefix') == 0) {
        ...
        mp.game.ui.notifications.show('~r~Zaten donanimli bir banka kartiniz var');
      }
      break;
    case 138:
      user.addCashMoney(1);
      mp.game.ui.notifications.show('~g~Cuzdanina 1$ koydun');
      inventory.deleteItemServer(id);
    ...
    case 139:
      ...
      mp.game.ui.notifications.show('~g~Cuzdanina 100$ koydun');
      inventory.deleteItemServer(id);
    ...
    case 153:
      ...
      mp.game.ui.notifications.show('~g~Muhimmatinizi kusandiniz');
      inventory.deleteItemServer(id);
      methods.saveLog(
        'EquipItems',
        `${user.get('rp_name')} (${user.get('id')}) - ${items.getItemNameById(
