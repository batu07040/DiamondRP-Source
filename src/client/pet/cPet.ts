...
import {getRandom} from './cMisc';
import {isInRangeOfPoint} from './cMisc';
...
function findBall(pet) {
    if(pet){
        ...
        if(ball && pweapon != mp.game.joaat('weapon_ball')) {
            mp.game.ui.notifications.showWithPicture("Mascota", "", "Ваш питомец отправляется на поиски мяча", "CHAR_CHOP", 7, true);
           ...
           try{
            ...
            let nx = player.position.x + (getRandom(15,25) * Math.sin(radians));
            let ny = player.position.y + (getRandom(15,25) * Math.cos(radians));
        ...
        else mp.game.ui.notifications.showWithPicture("Mascota", "", "Сначала вы должны бросить мяч", "CHAR_CHOP", 7, true);     
    }
}
...
function sitPet(pet, msg=true) {
    if(pet){
        ...
        if(pet.sit){
            ...
            if(msg) mp.game.ui.notifications.showWithPicture("Mascota", "", "Ваш питомец сел", "CHAR_CHOP", 7, true);
        }
        else {
            mp.game.streaming.requestAnimDict("creatures@rottweiler@amb@world_dog_sitting@exit");
            pet.taskPlayAnim("creatures@rottweiler@amb@world_dog_sitting@exit", "exit", 1, 1.0, -1, 0, 1.0, false, false, false);
            if(msg) mp.game.ui.notifications.showWithPicture("Mascota", "", "Ваш питомец поднялся", "CHAR_CHOP", 7, true);
        }
    }
}
...
function sleepPet(pet, msg=true) {
    if(pet){
        ...
        if(pet.sleep){
            ...
            if(msg) mp.game.ui.notifications.showWithPicture("Mascota", "", "Ваш питомец заснул", "CHAR_CHOP", 7, true);
        }
        else {
            mp.game.streaming.requestAnimDict("creatures@rottweiler@amb@world_dog_sitting@exit");
            pet.taskPlayAnim("creatures@rottweiler@amb@world_dog_sitting@exit", "exit", 1, 1.0, -1, 0, 1.0, false, false, false);
            if(msg) mp.game.ui.notifications.showWithPicture("Mascota", "", "Ваш питомец проснулся", "CHAR_CHOP", 7, true);
        }
    }
}
...
mp.keys.bind(0x5A, true, function() {
    if(!ped) return mp.game.ui.notifications.showWithPicture("Mascota", "", "Сначала вы должны вызвать своего питомца", "CHAR_CHOP", 7, true);
    const pedPos = new mp.Vector3(ped.getCoords(false).x, ped.getCoords(false).y, ped.getCoords(false).z);
    if(!isInRangeOfPoint(player.position, pedPos, 5)) return mp.game.ui.notifications.showWithPicture("Mascota", "", "Вы не достаточно близко к своему питомцу", "CHAR_CHOP", 7, true);
    if(!ped.freeze) return mp.game.ui.notifications.showWithPicture("Mascota", "", "Вы не можете отдавать приказы своему питомцу, пока он находится в движении", "CHAR_CHOP", 7, true);
...
mp.keys.bind(0x42, true, function() {
    ...
    const pedPos = new mp.Vector3(ped.getCoords(false).x, ped.getCoords(false).y, ped.getCoords(false).z);
    if(!isInRangeOfPoint(player.position, pedPos, 5)) return mp.game.ui.notifications.showWithPicture("Mascota", "", "Вы не достаточно близко к своему питомцу", "CHAR_CHOP", 7, true);
    if(ped.sleep) return mp.game.ui.notifications.showWithPicture("Mascota", "", "Ваш питомец спит, вы должны разбудить его", "CHAR_CHOP", 7, true);
    if(!ped.freeze) return mp.game.ui.notifications.showWithPicture("Mascota", "", "Вы не можете отдавать приказы своему питомцу, пока он находится в движении", "CHAR_CHOP", 7, true);
    findBall(ped);
});
...
mp.keys.bind(0x43, true, function() {   
    if(ped) {
        mp.game.ui.notifications.showWithPicture("Mascota", "", "Ваш питомец возвращается домой", "CHAR_CHOP", 7, true);
        ped.destroy();
        ped = false;
    }
    else {
        ...
        mp.game.ui.notifications.showWithPicture("Mascota", "", "Ваш питомец пришел на вашу позицию", "CHAR_CHOP", 7, true);
...
mp.keys.bind(0x46, true, function() {
    if(!ped) return mp.game.ui.notifications.showWithPicture("Mascota", "", "Сначала вы должны вызвать своего питомца", "CHAR_CHOP", 7, true);
    const pedPos = new mp.Vector3(ped.getCoords(false).x, ped.getCoords(false).y, ped.getCoords(false).z);
    if(isInRangeOfPoint(player.position, pedPos, 1.5)) return mp.game.ui.notifications.showWithPicture("Mascota", "", "Вы слишком близко к своему питомцу, отойдите немного дальше", "CHAR_CHOP", 7, true);
    if(!isInRangeOfPoint(player.position, pedPos, 7)) return mp.game.ui.notifications.showWithPicture("Mascota", "", "Вы не достаточно близко к своему питомцу", "CHAR_CHOP", 7, true);
    if(ped.sleep) return mp.game.ui.notifications.showWithPicture("Mascota", "", "Ваш питомец спит, вы должны разбудить его", "CHAR_CHOP", 7, true);
    ped.freeze = !ped.freeze;
    ped.freezePosition(ped.freeze);
    ped.sit = false;
    if(!ped.freeze) mp.game.ui.notifications.showWithPicture("Mascota", "", "Ваш питомец начинает следовать за вами", "CHAR_CHOP", 7, true);
    else {
        mp.game.ui.notifications.showWithPicture("Mascota", "", "Ваш питомец перестает следовать за вами", "CHAR_CHOP", 7, true);
        freezePet(ped, true);
    }
});
...
mp.events.add('render', () => {
    if(ped && !ped.freeze && !ped.gettingBall){
        ...
        if(isInRangeOfPoint(player.position, pedPos, 1.5)){
            freezePet(ped, true);
            if(ballThrown) mp.events.callRemote('pet:getBall');
        }
        else {
            //if we get too far from the pet, it stops and waits for us sitting down
            if(!isInRangeOfPoint(player.position, pedPos, 25)){
                mp.game.ui.notifications.showWithPicture("Mascota", "", "Вы слишком далеко ушли от своего питомца, и он не может вас найти. Он ждет тебя сидя, ищи его.", "CHAR_CHOP", 7, true);
                freezePet(ped, true);
                sitPet(ped, false);
            } else {
                const pGround = mp.game.gameplay.getGroundZFor3dCoord(player.position.x, player.position.y, player.position.z, parseFloat(0), false);
                ...
                let speed;
                if(!isInRangeOfPoint(player.position, pedPos, 4)) speed = 3;
                else speed = 1;
