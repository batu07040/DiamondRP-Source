...
import { cameraRotator } from '../modules/camerarotator';
import {FUNCTION} from '../modules/Rotator'
export {};
...
function createCam() {
  ...
  var vehPosition = new mp.Vector3(showPos[0], showPos[1], showPos[2]); // спавн авто
  cameraRotator.start(camera, vehPosition, vehPosition, new mp.Vector3(-8.0, -5.5, 1.5), 90);
...
mp.events.add({
  ...
  'client:autosalon:testDrive': async () => {
    ...
    user.hideLoadDisplay(500, false);
    user.notify('Aracı test etmek için 60 saniyeniz var.');
    vehicle.engine = true;
    ...
    user.notify('~r~Test sürüşü tamamlandı.');
    user.showLoadDisplay(500)
    await sleep(1000);
    focusCamera()
    testDriveMode = false;
