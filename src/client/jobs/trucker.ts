...
let trucker = {
    ...
    acceptOffer1: function(id:number, name:string, company:string, x:number, y:number, z:number, px:number, py:number, pz:number, price:number) {
        type = 1;
        trucker.notify('Kargoyu belirtilen noktadan alın.');
    ...
    acceptOffer2: function(id:number, name:string, company:string, x:number, y:number, z:number, px:number, py:number, pz:number, price:number) {
        type = 2;
        trucker.notify('Kargoyu belirtilen noktadan alın.');
    ...
    notify: function(text:string, title = 'Информация') {
        ...
        mp.game.ui.notifications.showWithPicture(title, "Kargo taşımacılığı", text, "DIA_DRIVER", 2);
    },
    ...
    workProcess1: function() {
        ...
        setTimeout(function () {
            ...
            trucker.notify('Malları aldınız, müşteriye götürün.');
    ...
    timer: function() {
        if (isSpawn && !isProcess) {
         ...
         if (type == 2 && isProcess == false && methods.distanceToPos(new mp.Vector3(fromPosX, fromPosY, fromPosZ), mp.players.local.position) < 10) {
...
mp.events.add("client:jobs:trucker:isSpawn", (cbSpawn) => {
    if (!cbSpawn) {
        ...
        trucker.notify('~r~Sizden önce bir başkasının kargosu var. Lütfen onun almasını bekleyin.');
        return;
    }
    trucker.notify('~g~Kargonuz yüklendi ve yola çıktı.');
    isSpawn = true;
});
