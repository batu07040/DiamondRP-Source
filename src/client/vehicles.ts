...
let vehicles = {
  ...
  enableAutopilot: () => {
    if (mp.game.invoke(methods.IS_WAYPOINT_ACTIVE)) {
      ...
      mp.game.ui.notifications.show('~g~Otomatik pilot acildi');
    } else {
      mp.players.local.clearTasks();
      mp.game.ui.notifications.show('Once haritada bir konum seçmen gerekli');
    }
  },
  
  disableAutopilot: () => {
    mp.game.ui.notifications.show('~y~Otomatik pilot kapatildi');
    mp.players.local.clearTasks();
  ...
  checkAutopilot: () => {
    if (isAutopilotEnable) {
      if (methods.getWaypointPosition().x == 0) {
        ...
        setTimeout(function() {
          mp.game.ui.notifications.show('~g~Belirttiginiz konuma ulaştiniz');
          mp.game.ui.notifications.show('~y~Otomatik pilot kapatildi');
          mp.players.local.clearTasks();
        }, 8000);
  ...
  //  user.setWaypoint(x, y);
    mp.game.ui.notifications.show('Tiklayiniz ~g~2~s~ Araci acmak icin yanina olmaniz gereklidir');
    mp.game.ui.notifications.show('Arac otoparkta duruyor');
