...
export default class GiveRec extends Component<{ fraction:number, rank:number, remove:boolean}, {current:number}>{
  ...
  send(){
    ...
    if(isNaN(id) || id < 0 || id > 10000000) return CEF.alert.setAlert('error', 'ID doğru değil')
    if (!recLists[this.state.current]) return CEF.alert.setAlert('error', 'Bir tür seçmediniz')
  ...
  render() {
    ...
    return (
      ...
      <div>
        <div>
          <input type="text" className="primary-input wide mb10" ref={this.id} placeholder="Oyuncu Kimliği"/>
        </div>
        <div>
