...
export default class GiveWanted extends Component<{}, { stars: number }>{
  ...
  send() {
    ...
    if (reason.length < 2) return CEF.alert.setAlert('error', 'Aranma nedeni minimum 2 karakterden oluşmalıdır.')
    if (isNaN(id) || id < 0 || id > 10000000) return CEF.alert.setAlert('error', "Geçersiz kullanıcı ID'si girdiniz.")
  ...
  render() {
    return (
      <div className="notepad-content-easy ui-tabs-panel ui-corner-bottom ui-widget-content">
        <h2 className="mini-title">Bir vatandaşı arananlar listesine ekleyin.</h2>
        <div className="notepad-giveWantedBlock">
          <div>
            <input type="text" className="primary-input wide mb10" ref={this.id} placeholder="Arama eklenecek kişinin ID" />
          </div>
          <div></div>
          <div>
        ...
        <textarea ref={this.reason} cols={30} rows={10} className="primary-input mb20" placeholder="Nedenini belirtin." onKeyDown={(e) => {
