    ...
    render() {
        ...
        return <div className="autosale">
            ...
            <div className="autosale-car-image">
                <img src={`http://${resip}:${port}/images/cars-more/${this.state.model || this.state.name}_1.jpg?v=${getRandomInt(1000, 10000)}`} onError={e => {
                    e.currentTarget.hidden = true
                }} />
            </div>
            <div className="autosale-part-title">Bilgi</div>
            <div className="autosale-info">
                {this.drawVehInfo(model, "Modeli", this.state.name)}
                {this.drawVehInfo(number, "Numarasi", this.state.number)}
                {this.state.stage !== "sell" ? <>
                    {this.drawVehInfo(cost, "Durum. Fiyat", this.state.gos+"$")}
                    {this.drawVehInfo(cost, "Fiyat", this.state.price+"$")}
                </> : <></>}
            </div>
            {this.state.stage !== "sell" ? <div className="autosale-info-one">
                {this.drawVehInfo(owner, "Araç Saticisi", this.state.seller)}
            </div> : <></>}
            
            <div className="autosale-part-title">Характеристики</div>
            <div className="autosale-info">
                <span>{this.drawSpecInfo(engine, "Motor", this.state.engine)}</span>
                <span>{this.drawSpecInfo(brake, "Frenler", this.state.brake)}</span>
                <span>{this.drawSpecInfo(transmission, "Transmisyon", this.state.transmission)}</span>
                <span>{this.drawSpecInfo(turbo, "Turbo", this.state.turbo)}</span>
            </div>
