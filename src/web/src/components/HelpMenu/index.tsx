    ...
    loadTicket(id: number) {
        ...
        if (!q.my) {
            ...
            if (dataReport.admin && this.state.admin) {
                ...
                return CEF.alert.setAlert('error', "Bu bildirimi goruntuleme izniniz yok.");
            }
            mp.events.triggerServer('help:loadTicket', q.id);
        }
    }
    sendTicket() {
        if (!this.state.createMenuBlockReason) return CEF.alert.setAlert("error", "Bir bolum secin");
        if (!this.state.createMenuBlockText) return CEF.alert.setAlert("error", "Mesajinizi iletin");
        let dataReport = getReportData(this.state.createMenuBlockReason);
        if (!dataReport) return CEF.alert.setAlert("error", "Secilen bolum yanlis!");
        if (dataReport.isReport) {

            if (!this.state.createMenuBlockReportId && this.state.createMenuBlockReportId !== 0) return CEF.alert.setAlert("error", "Report eden ID belirtin");
            if (isNaN(this.state.createMenuBlockReportId) || this.state.createMenuBlockReportId < 0) return CEF.alert.setAlert("error", "Suclunun kimligi yanlis fark etmediyseniz 0 girin");
        }
    ...
    selectTicket(item: QuestionPrewiew) {
        if (!item.my) {
            ...
            if (dataReport.admin && this.state.admin) {
                ...
                return CEF.alert.setAlert('error', "Bu bileti goruntuleme izniniz yok.");
            }
        }
            ...
            <div className="content">
                        ...
                        <div className="content">
                            <div className="col-sm-7">
                                ...
                                <button className={`btn ${this.state.admin ? '' : 'disabled'}`} onClick={e => {
                                    e.preventDefault();
                                    if (!this.state.admin) return CEF.alert.setAlert("error", "Yalnizca sunucu yoneticileri tarafindan kullanilabilir.")
                                    this.setState({ current: 'admin' })
                                }}>Sikayetler</button>
                                <button className={`btn ${this.state.helper ? '' : 'disabled'}`} onClick={e => {
                                    e.preventDefault();
                                    if (!this.state.helper) return CEF.alert.setAlert("error", "Yalnizca sunucu yardimcilari tarafindan kullanilabilir.")
                            ...
                            this.setState({ createMenuBlock: true })
                                ...
                                <div className="form-group">
                                    ...
                                    {this.state.createMenuBlockReason && getReportData(this.state.createMenuBlockReason).isReport ? <>
                                        <span className="desc">Sikayetci Olunacak Kisinin ID</span>
                                    ...
                                    <span className="desc">Mesaj Girin</span>
                    ...
                    <div className="ticketList" style={{ height: this.state.createMenuBlock ? '48.1%' : '81.5%'}}>
                        {this.state.current == "my" ? this.state.list.map(item => {
                            return <div className={`ticketItem ${this.state.currentTicket && this.state.currentTicket.id == item.id ? 'selecteds' : ''}`} onClick={e => {
                                ...
                                <div className="title">#{item.id} {item.author} <span>({item.authorid}) {getReportLabel(item.category)}</span> {!this.state.readedTickets.includes(item.id) ? <span className="new">Yeni</span> : ''}{item.closed ? <span className="new">Kapali</span> : ''}</div>
                                <div className="desc">{item.text}</div>
                                <div className="row">
                                    <div className="col-md-6 footer">Mesajlarin: {item.messagesCount}</div>
                ...
                <div className="col-md-6">
                    ...
                    {this.state.currentTicket ? <div className="ticketData">
                        ...
                        <div className="ticketItem">
                            <div className="title">#{this.state.currentTicket.id} {this.state.currentTicket.author} <span>({this.state.currentTicket.authorid}) {getReportLabel(this.state.currentTicket.category)}</span> {!this.state.readedTickets.includes(this.state.currentTicket.id) ? <span className="new">Yeni</span> : ''}{this.state.currentTicket.closed ? <span className="new">Kapali</span> : ''}</div>
                        ...
                        {this.state.currentTicket.messages.map((item, index) => {
                            return <div className="ticketItem">
                                <div className="title">#{(index + 1)} {item.username} <span>({item.userid}) {item.userid == this.state.currentTicket.authorid ? 'Yazan' : ''}</span></div>
                            ...
                            Bildirim Kapali
                                ...
                                <div className="ticketItem send">
                                    ...
                                    <div className="col-md-4">
                                        <button className="btn btn-success btn-block">Gonder</button>
                                        <button className="btn btn-danger  btn-block">Bildirimi Kapat</button>
                                    </div>
                                </div>
                            </>}
                        {this.state.admin ? <>
                            <div className="ticketItem footerEl">
                                <div className="title">Bildirimi Yazan Kisi Islemleri {`(ID: ${this.state.currentTicket.authorid} ${this.state.currentTicket.author})`}</div><br/>
                                <button className="btn btn-block btn-info">Kendine Isinla</button>
                                <button className="btn btn-block btn-info">Yanina Git</button>
                            </div>
                            {this.state.currentTicket.is_report ? <>
                                <div className="ticketItem footerEl">
                                    <div className="title">Reportlanan Kisi Islemleri {`(ID: ${this.state.currentTicket.is_report})`}</div><br />
                                    <button className="btn btn-block btn-info">Kendine Isinla</button>
                                    <button className="btn btn-block btn-info">Yanina Git</button>
                                </div>
                            </> : ''}
