...
class Chat extends Component<ChatProps, ChatState> {
  ...
  insertMessageToChat(str: string) {
    ...
    let textResult = String(str).replace(/[&<>"'`=\/]/gi, (s) => {
      ...
      return str + (/[a-zA-Z0-9-ğĞçÇşŞüÜöÖıİ-а-яА-ЯЁё\!\@\#\$\%\^\&\*\(\)\]\[\{\}\,\.\/\_\+\№\;\:\?\\\<\>\`\-]/gi.test(s) ? s : ' ');
    }, '');
  ...
  sendMessage(value: string) {
    ...
    if (value.length > 155) {
      CEF.alert.setAlert('error', "Mesajınız çok uzun 155 karakteri geçmeyen mesaj yazın.")
      return;
    }
            ...
            <div className="chat-hidden">
              <div className="chat-hud-wrap posrev">
                <div className="chat-entered">
                  [{this.state.value[0] == '/' ? 'Komut' : 'Mesaj Gönderin'}]:
                </div>
                <input
                  type="text"
