    ...
    case 'DROP_ITEM':
      ...
      if (payload.toSection == 'trunk' && item.section != 'trunk') {
        if (
          ...
          return CEF.alert.setAlert('warning', 'Bagajiniz dolu!'), state;
        }
      }
      if (payload.toSection != 'trunk' && item.section == 'trunk') {
        if (
          ...
          return CEF.alert.setAlert('warning', 'Envanteriniz dolu!'), state;
        }
      }

      if (payload.toSection == 'clothes' && item.clothGender && item.clothGender != state.floor)
        return CEF.alert.setAlert('warning', 'Farklı cinsiyete ait kiyafetleri giyemezsiniz.'), state;
      ...
      if (payload.count) {
        ...
        if (cancel)
          return CEF.alert.setAlert('warning', 'Bu ogeyi tasiyamazsiniz.'), state;
        countedItem = { ...item };
